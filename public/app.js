...
const ATTACKS=[
  {legacy:'Athena Exclamation',name:'Athena Exclamation',key:'athenaExclamationEnabled',stake:'athenaExclamationStakeCents',min:'athenaExclamationMinEntryCents',max:'athenaExclamationMaxEntryCents',pri:'athenaExclamationPri1R2Enabled',fields:[['athenaExclamationPri1R2TriggerCents','PRI1-R2 arm target','c'],['athenaExclamationFollowUpAttacks','Follow-up Attacks (0-12)',''],['athenaExclamationMinCrashCents','Min crash','c'],['athenaExclamationMinReboundCents','Min rebound','c'],['athenaExclamationMinUpwardTicks','Upward ticks','']],desc:'Optional Bolt Direct attack. Atomic Thunder times the entry. Own band and crash/rebound/ticks still apply. Not a permission source for other attacks.'},
  {legacy:'Scarlet Needle',name:'Scarlet Needle',key:'scarletNeedleEnabled',stake:'scarletNeedleStakeCents',min:'scarletNeedleMinEntryCents',max:'scarletNeedleMaxEntryCents',pri:'scarletNeedlePri1R2Enabled',fields:[['scarletNeedlePri1R2TriggerCents','PRI1-R2 arm target','c'],['scarletNeedleMinCrashCents','Min crash','c'],['scarletNeedleMinReboundCents','Min rebound','c'],['scarletNeedleMinUpwardTicks','Upward ticks','']],desc:'Regular Bolt Direct attack. Uses the shared Infinity Break target.'},
  ...
  {legacy:'Crash Recovery Hunter',name:'Starlight Extinction',key:'crashRecoveryHunterEnabled',stake:'crashRecoveryStakeCents',min:'crashRecoveryMinEntryCents',max:'crashRecoveryMaxEntryCents',pri:'crashRecoveryPri1R2Enabled',fields:[['crashRecoveryPri1R2TriggerCents','PRI profit arm (only if PRI ON)','c'],['crashRecoveryInfinityNetPerOriginalContractCents','Starlight Infinity profit target','c'],['crashRecoveryMinCrashCents','Min crash','c'],['crashRecoveryMinReboundCents','Min rebound','c'],['crashRecoveryMinUpwardTicks','Upward ticks','']],desc:'Not a Mega Wave saint. After a real hunter in the same cosmos dies on stop-loss, Starlight may re-enter that exact ticker if its own crash/rebound/uptick and Infinity target allow.'},
  {legacy:'Recovery Hunter',name:'Crystal Wall',key:'recoveryHunterEnabled',stake:'recoveryStakeCents',min:'recoveryMinEntryCents',max:'recoveryMaxEntryCents',pri:'recoveryPri1R2Enabled',fields:[['recoveryPri1R2TriggerCents','PRI arm target','c'],['recoveryPri1R2TrailCents','PRI trail / giveback','c'],['crystalWallMinCrashCents','Min crash','c'],['crystalWallMinReboundCents','Min rebound','c'],['crystalWallMinUpwardTicks','Upward ticks','']],desc:'Follow-up on the SAME ticker after a real attack closes on Infinity with a win. Not a bolt entry. Proofs unused. Crash/rebound/ticks still apply (0/0/0 = join immediately). PRI trail default 1c and never sells below shared Infinity. Excalibur copies Crystal Wall onto free cosmosses that pass their own CW settings.'},
...
const SHARED=[
 ['maxPositions','Max Attack Positions',''],['maxRepeatsPerMarket','Max fills per attack per market',''],['repeatCooldownMinutes','Wait after that attack closes','min'],['minGameMinutes','Game Clock minimum','min'],['maxGameMinutes','Game Clock maximum','min'],['sessionStopHour','Session stop hour (Madrid)','h'],['sessionStopMinute','Session stop minute','min'],['sessionStartHour','Session start hour (Madrid)','h'],['sessionStartMinute','Session start minute','min'],['maxSpreadCents','Shared Max Spread','c'],['startingCapitalCents','Simulation Starting Capital','c'],['simFeeCents','Simulation Fee / Contract','c'],['recoveryTrackingHours','Recovery Research Tracking','h'],
];
...
function syncCrystalProofStageVisibility(root,count){const n=Math.max(1,Math.min(5,Math.floor(Number(count)||3)));for(const el of (root||document).querySelectorAll('[data-crystal-proof-stage]')){const stage=Math.floor(Number(el.dataset.crystalProofStage)||0);el.style.display=stage>0&&stage<=n?'':'none';}}
function crystalProofStageFieldsHtml(s,count){const n=Math.max(1,Math.min(5,Math.floor(Number(count)||3)));let html='';for(let i=1;i<=5;i+=1){html+=`<div class="crystal-proof-stage" data-crystal-proof-stage="${i}" style="${i>n?'display:none':''}">${inputHtml(`crystalWallProof${i}MinCrashCents`,`Proof ${i} crash`,s.settings?.[`crystalWallProof${i}MinCrashCents`],'c',1,1,99)}${inputHtml(`crystalWallProof${i}MinReboundCents`,`Proof ${i} rebound`,s.settings?.[`crystalWallProof${i}MinReboundCents`],'c',1,1,99)}${inputHtml(`crystalWallProof${i}MinUpwardTicks`,`Proof ${i} ticks`,s.settings?.[`crystalWallProof${i}MinUpwardTicks`],'',1,1,20)}</div>`;}return html;}
function attackControlsHtml(s,a){let h=inputHtml(a.stake,'Stake',s.settings?.[a.stake],'c',1)+inputHtml(a.min,'Min entry',s.settings?.[a.min],'c',1)+inputHtml(a.max,'Max entry',s.settings?.[a.max],'c',1);if(a.pri)h+=`<label>PRI1-R2 Exit Guard<span><input data-setting-bool="${esc(a.pri)}" type="checkbox" ${s.settings?.[a.pri]===true?'checked':''}></span></label>`;for(const [k,l,u] of (a.fields||[]))h+=inputHtml(k,l,s.settings?.[k],u,k.includes('Rate')?0.01:1,k==='crystalWallWinsToTriggerAthena'?1:(/MinCrashCents|MinReboundCents|MinUpwardTicks/.test(k)?0:null),k==='crystalWallWinsToTriggerAthena'?5:(/MinUpwardTicks/.test(k)?20:(/MinCrashCents|MinReboundCents/.test(k)?99:null)));if(a.rays)h+=inputHtml(a.rays,'Max rays',s.settings?.[a.rays],'',1);if(a.repeats)h+=inputHtml(a.repeats,a.repeatLabel||'Max repeats',s.settings?.[a.repeats],'',1);const proofCount=crystalProofCount(s);let authority=a.legacy==='Athena Exclamation'?`BOLT DIRECT: Athena is an optional attack timed by Atomic Thunder. Crystal Wall proofs are not permission. Galactic Explosion ON lets other enabled attacks join the same ticker.`:a.legacy==='Recovery Hunter'?`CRYSTAL WALL FOLLOW-UP: after a real attack banks Infinity, Crystal Wall may enter the same ticker. Own crash/rebound/ticks apply. PRI trail is 1c and must not sell back to scratch. Homepage row shows real twelve-cosmos results.`:a.legacy==='Crash Recovery Hunter'?`STARLIGHT: not a Mega Wave saint. A real hunter stop-loss in this cosmos arms one same-ticker re-entry. Crash/rebound/uptick must pass. Leave PRI OFF and set Starlight Infinity profit target to the cents you want Infinity to take. Tick PRI only if you want the runner instead.`:'BOLT DIRECT: Atomic Thunder times this attack. Own band/crash/rebound/ticks still apply. Galactic OFF = one ticker one hunter. Galactic ON = enabled attacks may join the same ticker, one seat each. PRI ON = PRI1-R2; PRI OFF = Infinity Break. Aurora is loss authority.';const banners='';return `<div class="model-settings">${h}<button class="btn light model-save" data-model-save="${esc(a.legacy)}">Save</button></div>${banners}<div class="attack-authority-note">${authority}</div>`;}
...
function fleetStatFor(s,name){
  const row=(s.fleetAttackResults?.attacks||[]).find((x)=>x.name===name);
  if(row) return row;
  if(name==='Crystal Wall Shadow') return {open:0,closed:0,wins:0,losses:0,winRate:0,pnlCents:0,total:0,avgEntryCents:0,excluded:'crystal_wall_shadow'};
  return statFor(s,name);
}
...
function renderSystem(s){const p=s.performance||{},h=s.health||{},r=s.resourceUsage||{};setText('portfolioValueLabel',s.settings?.mode==='LIVE'?'Kalshi Portfolio':'Portfolio Value');if(s.settings?.mode==='LIVE'&&p.portfolioValueCents==null){setText('portfolioValue','Kalshi —');const el=$('portfolioValue');if(el)el.className='';}else setPnl('portfolioValue',p.portfolioValueCents,true);setPnl('simFreeCash',p.simulationCashCents,true);setText('winRate',pct01(p.winRate));setPnl('unrealized',p.unrealizedCents);setPnl('realized',p.realizedCents);setPnl('profitToday',p.dayRealizedCents);setPnl('profitWeek',p.weekRealizedCents);setPnl('profitMonth',p.monthRealizedCents);setPnl('profitYear',p.yearRealizedCents);setText('openCount',p.open||0);setText('closedPerfCount',p.closed||0);setText('releaseLabel',s.settings?.release||s.release||'R60-COSMO-GREEN');setText('modeBtn',s.settings?.mode||'SIMULATION');$('modeBtn').className=`btn ${s.settings?.mode==='LIVE'?'danger':'light'}`;setText('modeNote',s.settings?.mode==='LIVE'?(s.settings?.liveArmed?'LIVE armed':'LIVE selected, disarmed'):'Paper trading - no real orders.');setText('engineBtn',s.settings?.engineActive?'Stop Engine':'Start Engine');setText('engineStatus',s.settings?.engineActive?'Engine running':'Engine stopped');$('engineStatus').className=`pill ${s.settings?.engineActive?'green':'red'}`;setText('systemStatus',h.degraded?'DEGRADED':'HEALTHY');$('systemStatus').className=`pill ${h.degraded?'red':'green'}`;setText('uptime',duration(Date.now()-Number(h.startedAtMs||Date.now())));const errors=(s.audit||[]).filter(x=>x.level==='error');$('errorWatchdog').classList.toggle('hidden',!errors.length);if(errors.length)$('errorWatchdog').textContent=`Error Watchdog — ${errors.length} recent errors · ${errors[0].event}: ${errors[0].data?.message||''}`;const scan=s.scanner||{};const scanFresh=Number(scan.lastScanMs||0)>0&&Date.now()-Number(scan.lastScanMs)<10*60*1000;setText('entryRunning',s.settings?.engineActive===false?'Stopped':scanFresh?'Running':'Waiting');$('entryRunning').className=`pill ${s.settings?.engineActive===false?'red':scanFresh?'green':'amber'}`;setText('scannerActive',scanFresh?'ACTIVE':'WAITING');$('scannerActive').className=`pill ${scanFresh?'green':'amber'}`;setText('scannerDetail',`Tracked ${scan.tracked||0} · active markets ${scan.activeMarkets||0} · last scan ${scan.lastScanMs?duration(Date.now()-scan.lastScanMs)+' ago':'never'}`);const resourceState=r.pressureState||r.status||'UNKNOWN';setText('resourceStatus',resourceState);$('resourceStatus').className=`pill ${['NORMAL','GREEN'].includes(resourceState)?'green':['WATCH','COMPACT','PRESSURE'].includes(resourceState)?'amber':'red'}`;setText('resourceRss',mib(r.memory?.rssBytes??r.rssBytes));setText('resourceHeapUsed',mib(r.memory?.heapUsedBytes??r.heapUsedBytes));setText('resourceCpu',Number.isFinite(Number(r.cpuPercent))?`${Number(r.cpuPercent).toFixed(1)}%`:'-');setText('resourceUptime',duration((r.uptimeSeconds??r.processUptimeSeconds??0)*1000));setText('connectionPill',h.restOk&&h.websocketFresh?'Connected':'Connection check');$('connectionPill').className=`pill ${h.restOk&&h.websocketFresh?'green':'amber'}`;$('systemNameInput').value=s.settings?.systemName||'SAGITTARIUS';}
function renderGalactic(s){const on=s.settings?.galacticExplosionEnabled===true;setText('galacticStatus',on?'ON':'OFF');$('galacticStatus').className=`pill ${on?'amber':'red'}`;const b=$('galacticExplosionToggle');if(b){b.textContent=on?'ON':'OFF';b.dataset.enabled=String(on);b.className=`galactic-toggle ${on?'enabled':'disabled'}`;}const rozan=s.settings?.rozanHyakuRyuHaEnabled===true;if($('rozanStatus')){setText('rozanStatus',rozan?'ON':'OFF');$('rozanStatus').className=`pill ${rozan?'amber':'red'}`;}const r=$('rozanHyakuRyuHaToggle');if(r){r.textContent=rozan?'ON':'OFF';r.dataset.enabled=String(rozan);r.className=`galactic-toggle ${rozan?'enabled':'disabled'}`;}const blade=s.settings?.excaliburEnabled===true;if($('excaliburStatus')){setText('excaliburStatus',blade?'ON':'OFF');$('excaliburStatus').className=`pill ${blade?'amber':'red'}`;}const x=$('excaliburToggle');if(x){x.textContent=blade?'ON':'OFF';x.dataset.enabled=String(blade);x.className=`galactic-toggle ${blade?'enabled':'disabled'}`;}
  const gateOn=s.settings?.gameClockEntryGateEnabled===true;
  if($('gameClockGateStatus')){setText('gameClockGateStatus',gateOn?'ON':'OFF');$('gameClockGateStatus').className=`pill ${gateOn?'amber':'red'}`;}
  const g=$('gameClockGateToggle');if(g){g.textContent=gateOn?'ON':'OFF';g.dataset.enabled=String(gateOn);g.className=`galactic-toggle ${gateOn?'enabled':'disabled'}`;}
  const shutOn=s.settings?.sessionShutdownEnabled===true;
  if($('sessionShutdownStatus')){setText('sessionShutdownStatus',shutOn?'ON':'OFF');$('sessionShutdownStatus').className=`pill ${shutOn?'amber':'red'}`;}
  const sh=$('sessionShutdownToggle');if(sh){sh.textContent=shutOn?'ON':'OFF';sh.dataset.enabled=String(shutOn);sh.className=`galactic-toggle ${shutOn?'enabled':'disabled'}`;}
  const sess=s.riskControls?.operatorSession||s.operatorSession||null;
  if($('madridClockLabel')) setText('madridClockLabel', sess?.wall?.clockLabel || s.madridClockLabel || '—');
  if($('sessionWindowLabel')){
    const stopH=String(s.settings?.sessionStopHour??1).padStart(2,'0');
    const stopM=String(s.settings?.sessionStopMinute??0).padStart(2,'0');
    const startH=String(s.settings?.sessionStartHour??7).padStart(2,'0');
    const startM=String(s.settings?.sessionStartMinute??0).padStart(2,'0');
    setText('sessionWindowLabel', `${stopH}:${stopM} → ${startH}:${startM} Europe/Madrid`);
  }
  if($('sessionPhaseLabel')) setText('sessionPhaseLabel', sess?.reason || (shutOn?'session_armed':'session_shutdown_disabled'));
}
function controlFingerprint(s){try{return JSON.stringify({settings:s.settings||{},conceptStats:s.conceptStats||[],fleetAttackResults:s.fleetAttackResults||{},gemini:s.gemini||{},crystalWallShadow:s.crystalWallShadow||{}});}catch{return String(Date.now());}}
...
$('galacticExplosionToggle').onclick=async()=>{const on=$('galacticExplosionToggle').dataset.enabled==='true';if(!on&&!confirm('Enable Galactic Explosion? Saints still wait for a profitable Athena close. They do not enter while Athena is open.'))return;try{render(await patchSettingsVerified({galacticExplosionEnabled:!on}));}catch(e){msg(e.message,true);}};
$('rozanHyakuRyuHaToggle')?.addEventListener('click',async()=>{const on=$('rozanHyakuRyuHaToggle').dataset.enabled==='true';if(!on&&!confirm('Enable Rozan Hyaku Ryu Ha? All twelve cosmosses may enter the same exact ticker. Excalibur will turn off.'))return;try{render(await patchSettingsVerified(on?{rozanHyakuRyuHaEnabled:false}:{rozanHyakuRyuHaEnabled:true,excaliburEnabled:false}));}catch(e){msg(e.message,true);}});
$('excaliburToggle')?.addEventListener('click',async()=>{const on=$('excaliburToggle').dataset.enabled==='true';if(!on&&!confirm('Enable Excalibur? One authorized Athena fire unlocks that ticker. Each cosmos must still print its own Crystal Wall before it opens. Rozan will turn off.'))return;try{render(await patchSettingsVerified(on?{excaliburEnabled:false}:{excaliburEnabled:true,rozanHyakuRyuHaEnabled:false}));}catch(e){msg(e.message,true);}});
$('gameClockGateToggle')?.addEventListener('click',async()=>{const on=$('gameClockGateToggle').dataset.enabled==='true';try{render(await patchSettingsVerified({gameClockEntryGateEnabled:!on}));msg('Game Clock entry gate saved on host + 12 cosmosses.');}catch(e){msg(e.message,true);}});
$('sessionShutdownToggle')?.addEventListener('click',async()=>{const on=$('sessionShutdownToggle').dataset.enabled==='true';try{render(await patchSettingsVerified({sessionShutdownEnabled:!on}));msg('Night shutdown saved. Madrid wall clock is the leading clock.');}catch(e){msg(e.message,true);}});
...
$('resetDashboardBtn').onclick=async()=>{if(!confirm('Zero the homepage counters from now. Rows stay in the database. Learning is not deleted.'))return;try{await post('/api/reset-dashboard');await load();msg('Dashboard baseline reset.');}catch(e){msg(e.message,true);}};
$('resetSimulationBtn').onclick=async()=>{if(!confirm('Archive EVERY simulation trade on all twelve cosmos for this account? Homepage open/closed tables go empty. Learning stays.'))return;try{await post('/api/reset-simulation');await load();msg('Simulation book archived. Tables should be empty.');}catch(e){msg(e.message,true);}};
...
$('subsetApplyBtn')?.addEventListener('click',async()=>{
  ...
  try{
    const patch={crystalWallMinCrashCents:crash,crystalWallMinReboundCents:rebound,crystalWallMinUpwardTicks:ticks};
    for(let i=1;i<=5;i+=1){patch[`crystalWallProof${i}MinCrashCents`]=crash;patch[`crystalWallProof${i}MinReboundCents`]=rebound;patch[`crystalWallProof${i}MinUpwardTicks`]=ticks;}
