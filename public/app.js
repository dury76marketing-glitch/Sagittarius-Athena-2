...
$('galacticExplosionToggle').onclick=async()=>{const on=$('galacticExplosionToggle').dataset.enabled==='true';if(!on&&!confirm('Enable Galactic Explosion? When Athena opens, enabled follow-up Saints of that same cosmos may enter the same ticker with her. They still need their own confirmation. Same-Attack duplicates stay prohibited.'))return;try{render(await patchSettingsVerified({galacticExplosionEnabled:!on}));}catch(e){msg(e.message,true);}};
