...
function crystalProofStageFieldsHtml(s,count){const n=Math.max(1,Math.min(5,Math.floor(Number(count)||3)));let html='';for(let i=1;i<=5;i+=1){html+=`<div class="crystal-proof-stage" data-crystal-proof-stage="${i}" style="${i>n?'display:none':''}"><strong>Crystal Wall Proof ${i}</strong>${inputHtml(`crystalWallProof${i}MinCrashCents`,`Proof ${i} crash`,s.settings?.[`crystalWallProof${i}MinCrashCents`],'c',1,1,99)}${inputHtml(`crystalWallProof${i}MinReboundCents`,`Proof ${i} rebound`,s.settings?.[`crystalWallProof${i}MinReboundCents`],'c',1,1,99)}${inputHtml(`crystalWallProof${i}MinUpwardTicks`,`Proof ${i} upward ticks`,s.settings?.[`crystalWallProof${i}MinUpwardTicks`],'',1,1,20)}</div>`;}return html;}
