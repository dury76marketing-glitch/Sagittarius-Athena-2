...
const ATTACKS=[
  ...
  {legacy:'Recovery Hunter',name:'Crystal Wall',key:'recoveryHunterEnabled',stake:'recoveryStakeCents',min:'recoveryMinEntryCents',max:'recoveryMaxEntryCents',fields:[['crystalWallWinsToTriggerAthena','Winners to trigger Athena (1-5)','']],desc:'Independent CI1 crash-rebound SHADOW proof Attack. Proof 1–5 crash/rebound/ticks below are the live rules. The old single Crystal Wall crash/rebound/ticks is only a fallback if a proof key is missing.'},
...
function fleetStatFor(s,name){
  ...
  if(row) return row;
  if(name==='Recovery Hunter' || name==='Crystal Wall Shadow') return {open:0,closed:0,wins:0,losses:0,winRate:0,pnlCents:0,total:0,avgEntryCents:0,excluded:'crystal_wall_shadow'};
...
function bindBoard(id){const b=$(id);b.oninput=e=>{const i=e.target.closest('[data-setting-key]');if(i){drafts.set(i.dataset.settingKey,i.value);if(i.dataset.settingKey==='crystalWallWinsToTriggerAthena')syncCrystalProofStageVisibility(b,i.value);}};b.onclick=async e=>{const save=e.target.closest('[data-model-save]');if(save){const row=save.closest('tr'),data={};for(const i of row.querySelectorAll('[data-setting-key]'))data[i.dataset.settingKey]=Number(i.value);for(const i of row.querySelectorAll('[data-setting-bool]'))data[i.dataset.settingBool]=i.checked===true;if(save.dataset.modelSave==='Recovery Hunter'&&Number.isFinite(Number(data.crystalWallProof1MinCrashCents))){data.crystalWallMinCrashCents=Number(data.crystalWallProof1MinCrashCents);data.crystalWallMinReboundCents=Number(data.crystalWallProof1MinReboundCents);data.crystalWallMinUpwardTicks=Number(data.crystalWallProof1MinUpwardTicks);}save.disabled=true;try{const fresh=await patchSettingsVerified(data);for(const k of Object.keys(data))drafts.delete(k);render(fresh);msg(`${save.dataset.modelSave} settings saved.`);}catch(x){msg(x.message,true);}finally{save.disabled=false;}return;}const btn=e.target.closest('[data-model-key]');if(!btn)return;const key=btn.dataset.modelKey,enable=btn.dataset.enabled!=='true';btn.disabled=true;try{const fresh=await patchSettingsVerified({[key]:enable});render(fresh);msg(`${btn.dataset.modelName} ${enable?'enabled':'disabled'}.`);}catch(x){msg(x.message,true);}finally{btn.disabled=false;}};}
