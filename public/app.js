...
function renderGalactic(s){const on=s.settings?.galacticExplosionEnabled===true;setText('galacticStatus',on?'ON':'OFF');$('galacticStatus').className=`pill ${on?'amber':'red'}`;const b=$('galacticExplosionToggle');if(b){b.textContent=on?'ON':'OFF';b.dataset.enabled=String(on);b.className=`galactic-toggle ${on?'enabled':'disabled'}`;}const rozan=s.settings?.rozanHyakuRyuHaEnabled===true;if($('rozanStatus')){setText('rozanStatus',rozan?'ON':'OFF');$('rozanStatus').className=`pill ${rozan?'amber':'red'}`;}const r=$('rozanHyakuRyuHaToggle');if(r){r.textContent=rozan?'ON':'OFF';r.dataset.enabled=String(rozan);r.className=`galactic-toggle ${rozan?'enabled':'disabled'}`;}}
...
$('rozanHyakuRyuHaToggle')?.addEventListener('click',async()=>{const on=$('rozanHyakuRyuHaToggle').dataset.enabled==='true';if(!on&&!confirm('Enable Rozan Hyaku Ryu Ha? All twelve cosmosses may enter the same exact ticker.'))return;try{render(await patchSettingsVerified({rozanHyakuRyuHaEnabled:!on}));}catch(e){msg(e.message,true);}});
