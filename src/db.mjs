...
export class Database {
  ...
  async acquireFleetTickerLock(ticker) {
    const client = await this.lockPool.connect();
    const key = `fleet|ticker|${String(ticker||'')}`;
    try {
  ...
  async openFleetHunterEntries(){
    const r=await this.pool.query("select * from sag_entries where archived=false and concept_name = any($1::text[]) and status in ('open','entry_pending','exit_pending','pending_recovery') order by opened_at_ms asc",[PORTFOLIO_CONCEPT_NAMES]);
    return r.rows.map(rowEntry);
  }
  async openFleetHunterEntriesByTicker(ticker){
    const r=await this.pool.query("select * from sag_entries where ticker=$1 and archived=false and concept_name = any($2::text[]) and status in ('open','entry_pending','exit_pending','pending_recovery') order by opened_at_ms asc",[String(ticker),PORTFOLIO_CONCEPT_NAMES]);
    return r.rows.map(rowEntry);
  }
  ...
  async init() {
    await this.pool.query(`
      ...
      create index if not exists sag_entries_system_sim_active on sag_entries(system_name,mode) where archived=false;
  ...
  async loadSettingsByKey(key, defaults){
    const r=await this.pool.query('select value from sag_settings where key=$1',[String(key)]);
    return sanitizeRuntimeSettings(r.rows[0]?.value||{},defaults);
  }
  async saveSettingsByKey(key, settings){
    const clean=sanitizeRuntimeSettings(settings);
    await this.pool.query('insert into sag_settings(key,value) values($1,$2) on conflict(key) do update set value=excluded.value,updated_at=now()',[String(key),clean]);
    return clean;
  }
  ...
  async loadCosmosSettings(id, defaults){
    ...
    return this.loadSettingsByKey(cosmosSettingsKey(cosmosId), defaults);
  }
  async saveCosmosSettings(id, settings){
    const cosmosId=normalizeCosmosId(id);
    if(!cosmosId) throw new Error(`unknown_cosmos:${id}`);
    return this.saveSettingsByKey(cosmosSettingsKey(cosmosId), settings);
  }
  async ensureCosmosSettingsRows(defaults){
    const master=await this.loadSettings(defaults);
    for (const id of COSMOS_IDS) {
      const key=cosmosSettingsKey(id);
      const r=await this.pool.query('select 1 from sag_settings where key=$1',[key]);
      if(!r.rowCount) await this.saveSettingsByKey(key, master);
    }
    const fleet=await this.pool.query('select 1 from sag_settings where key=$1',[CONSTELLATION.fleetSettingsKey]);
    if(!fleet.rowCount) await this.saveSettingsByKey(CONSTELLATION.fleetSettingsKey, master);
    return master;
  }
  async loadSoulLedger(){const r=await this.pool.query("select value from sag_settings where key='athena_soul'");return r.rows[0]?.value||null;}
  async saveSoulLedger(value){await this.pool.query("insert into sag_settings(key,value) values('athena_soul',$1) on conflict(key) do update set value=excluded.value,updated_at=now()",[value||{}]);}
  ...
  async insertEntry(e){
    if(!String(e?.systemName||'').trim())throw new Error('entry_system_name_required');
    const cols=[
  ...
  async migrateSimulationCrystalWallToShadow(systemName,{shadowConceptName='Crystal Wall Shadow',release=null,policyRevision=null}={}){
    // R69.2 migration is deliberately SIMULATION-only. Historical/open LIVE
    // Recovery Hunter rows may represent broker inventory and must remain in
    // the real portfolio/protection domain until their ordinary exit completes.
    const r=await this.pool.query(`update sag_entries set
      concept_name=$2,
      entry_config=coalesce(entry_config,'{}'::jsonb) || jsonb_build_object(
        'conceptName',$2,
        'shadowAttack',jsonb_build_object(
          'version','CRYSTAL-WALL-V3','policyRevision',coalesce($4,entry_config#>>'{crystalWall,policyRevision}'),
          'brokerOrderAuthority',false,'portfolioCapitalAuthority',false,'simulationPortfolioCapitalAuthority',false,
          'migratedFromRealSimulation',true
        ),
        'virtualInfinity',coalesce(entry_config->'virtualInfinity',entry_config->'infinityBreak','{}'::jsonb),
        'shadowMigration',jsonb_build_object('release',$3,'migratedAtMs',$5,'fromConcept','Recovery Hunter')
      ),
      updated_at_ms=$5
      where system_name=$1 and mode='SIMULATION' and archived=false and concept_name='Recovery Hunter'
        and entry_config#>>'{crystalWall,version}'='CRYSTAL-WALL-V3'`,
      [String(systemName),String(shadowConceptName),release==null?null:String(release),policyRevision==null?null:String(policyRevision),Date.now()]);
    return Number(r.rowCount||0);
  }
  async entries(systemName,{limit=5000,includeArchived=false}={}){
    if(!String(systemName||'').trim())throw new Error('entries_system_name_required');
    const r=await this.pool.query(`select * from sag_entries where system_name=$1 ${includeArchived?'':'and archived=false'} order by opened_at_ms desc limit $2`,[systemName,limit]);
    return r.rows.map(rowEntry);
  }
  async entriesAllCosmoses({limit=5000,includeArchived=false}={}){
    const out=[];
    for(const id of COSMOS_IDS) out.push(...await this.entries(id,{limit,includeArchived}));
    return out;
  }
  async hydrateAllCosmosEntries({limit=5000,includeArchived=false}={}){
    const books={};
    for(const id of COSMOS_IDS) books[id]=await this.entries(id,{limit,includeArchived});
    return books;
  }
  ...
  // R68/OPI1: dashboard/log readers intentionally project only the scalar
  // ledger truth and the small JSON fragments needed by the UI. The previous
  // select-* state path hydrated large Athena/guard/research graphs every SSE
  // cycle, coupling operator responsiveness to historical JSON size.
  dashboardEntryProjectionSql(){return `id,system_name,owner_id,concept_name,source_feeder,source_trade_id,ticker,event_ticker,market_title,watchdog_model,mode,status,
    ...
    jsonb_build_object(
      'aurora',coalesce(entry_config->'aurora','{}'::jsonb),
      'infinityBreak',coalesce(entry_config->'infinityBreak','{}'::jsonb),
      'profitAuthority',entry_config->'profitAuthority',
      'simFeeCents',entry_config->'simFeeCents',
      'virtualInfinity',coalesce(entry_config->'virtualInfinity','{}'::jsonb),
      'dragonSource',case when entry_config#>'{dragonSource,signalPriceCents}' is null then '{}'::jsonb else jsonb_build_object('signalPriceCents',entry_config#>'{dragonSource,signalPriceCents}') end,
      'phoenixSource',case when entry_config#>'{phoenixSource,signalAskCents}' is null then '{}'::jsonb else jsonb_build_object('signalAskCents',entry_config#>'{phoenixSource,signalAskCents}') end,
      'goldenDragonSource',case when entry_config#>'{goldenDragonSource,signalPriceCents}' is null then '{}'::jsonb else jsonb_build_object('signalPriceCents',entry_config#>'{goldenDragonSource,signalPriceCents}') end
    ) as entry_config,
    jsonb_build_object(
      'atomicThunderBoltId',feeder_state->'atomicThunderBoltId',
      'athenaSelectedAttack',feeder_state->'athenaSelectedAttack',
      'realEntryId',feeder_state->'realEntryId',
      'justiceArrowEntryId',feeder_state->'justiceArrowEntryId'
    ) as feeder_state,
    '{}'::jsonb as stop_guard_state,'{}'::jsonb as profit_guard_state,'{}'::jsonb as apex_profit_guard_state,
    jsonb_build_object(
      'latestMarketPriceCents',post_exit_state->'latestMarketPriceCents',
      'deltaFromExitCents',post_exit_state->'deltaFromExitCents',
      'bestExecutableBidCents',post_exit_state->'bestExecutableBidCents',
      'bestDeltaFromExitCents',post_exit_state->'bestDeltaFromExitCents',
      'missedUpsideNetCents',post_exit_state->'missedUpsideNetCents',
      'worstExecutableBidCents',post_exit_state->'worstExecutableBidCents',
      'worstDeltaFromExitCents',post_exit_state->'worstDeltaFromExitCents',
  ...
  async performanceAggregate(systemName,{resetTimestampMs=0}={}){
    const reset=Math.max(0,Number(resetTimestampMs)||0);
    // PNL1: restore the original dashboard period metrics from durable closed
    // Execution-Attack P/L. Calendar boundaries follow Europe/Madrid, matching
    // the operator clock shown on the board. The dashboard reset timestamp is
    // still authoritative, so no period can reach behind the active baseline.
    const r=await this.pool.query(`with bounds as (select
        (extract(epoch from (date_trunc('day',now() at time zone 'Europe/Madrid') at time zone 'Europe/Madrid'))*1000)::bigint as day_ms,
        (extract(epoch from (date_trunc('week',now() at time zone 'Europe/Madrid') at time zone 'Europe/Madrid'))*1000)::bigint as week_ms,
        (extract(epoch from (date_trunc('month',now() at time zone 'Europe/Madrid') at time zone 'Europe/Madrid'))*1000)::bigint as month_ms,
        (extract(epoch from (date_trunc('year',now() at time zone 'Europe/Madrid') at time zone 'Europe/Madrid'))*1000)::bigint as year_ms
      ) select
      count(*) filter(where concept_name = any($2::text[]) and status='closed' and ($3::bigint=0 or closed_at_ms >= $3))::int as closed_hunters,
      count(*) filter(where concept_name = any($2::text[]) and status in ('open','entry_pending','exit_pending','pending_recovery'))::int as open_hunters,
      count(*) filter(where concept_name = any($2::text[]) and status='closed' and pnl_cents>0 and ($3::bigint=0 or closed_at_ms >= $3))::int as wins,
      ...
      count(*) filter(where concept_name = any($2::text[]) and status='closed' and pnl_cents=0 and ($3::bigint=0 or closed_at_ms >= $3))::int as scratches,
      coalesce(sum(pnl_cents) filter(where concept_name = any($2::text[]) and status='closed' and ($3::bigint=0 or closed_at_ms >= $3)),0)::double precision as closed_realized_cents,
      coalesce(sum(pnl_cents) filter(where concept_name = any($2::text[]) and status='closed' and closed_at_ms >= greatest($3::bigint,bounds.day_ms)),0)::double precision as day_realized_cents,
      coalesce(sum(pnl_cents) filter(where concept_name = any($2::text[]) and status='closed' and closed_at_ms >= greatest($3::bigint,bounds.week_ms)),0)::double precision as week_realized_cents,
      coalesce(sum(pnl_cents) filter(where concept_name = any($2::text[]) and status='closed' and closed_at_ms >= greatest($3::bigint,bounds.month_ms)),0)::double precision as month_realized_cents,
  ...
  async conceptStatsAggregate(systemName,{resetTimestampMs=0}={}){
    ...
    const r=await this.pool.query(`with portfolio as (
      select concept_name,
        ...
        coalesce(avg(current_price_cents),0)::double precision as avg_current_cents,
        coalesce(avg(volume_24h),0)::double precision as avg_liquidity
      from sag_entries where system_name=$1 and archived=false and concept_name = any($2::text[])
        and ($4::bigint=0 or status in ('open','entry_pending','exit_pending','pending_recovery') or coalesce(closed_at_ms,0) >= $4)
      group by concept_name
    ), signals as (
      ...
      select source_feeder,
        ...
        coalesce(sum(pnl_cents) filter(where status='closed'),0)::double precision as pnl_cents
      from sag_entries where system_name=$1 and archived=false and concept_name = any($2::text[]) and source_feeder = any($3::text[])
        and ($4::bigint=0 or status in ('open','entry_pending','exit_pending','pending_recovery') or coalesce(closed_at_ms,0) >= $4)
      group by source_feeder
    ) select
      ...
      coalesce((select jsonb_agg(to_jsonb(l)) from linked l),'[]'::jsonb) as linked`,[String(systemName),PORTFOLIO_CONCEPT_NAMES,FEEDER_CONCEPT_NAMES,reset]);
  ...
  async entriesByConceptTicker(systemName,conceptName,ticker,{limit=200,includeArchived=false}={}){const lim=Math.max(1,Math.min(2000,Math.floor(Number(limit)||200)));const r=await this.pool.query(`select * from sag_entries where system_name=$1 and concept_name=$2 and ticker=$3 ${includeArchived?'':'and archived=false'} order by opened_at_ms desc limit $4`,[String(systemName),String(conceptName),String(ticker),lim]);return r.rows.map(rowEntry);}
  ...
  async archiveSimulation(systemName){const r=await this.pool.query("update sag_entries set archived=true,updated_at_ms=$2 where system_name=$1 and mode='SIMULATION' and archived=false",[systemName,Date.now()]);return r.rowCount||0;}
  async archiveSimulationFleet({ownerId,mode='SIMULATION'}={}){
    const owner=String(ownerId||'');
    if(!owner) throw new Error('archive_simulation_fleet_owner_required');
    const r=await this.pool.query("update sag_entries set archived=true,updated_at_ms=$3 where owner_id=$1 and mode=$2 and archived=false",[owner,String(mode||'SIMULATION'),Date.now()]);
    return r.rowCount||0;
  }
  ...
  async atomicThunderStats(systemName){
    // R68/OPI1: aggregate the complete harvest history inside PostgreSQL. The
    // previous dashboard reader hydrated every closed Atomic Thunder row plus
    // its profit-episode JSON on each state refresh, making observability cost
    // grow forever with account history.
    ...
    const harvests=await this.pool.query(`with raw as (
      select e.pnl_cents::double precision as actual,e.opened_at_ms,e.closed_at_ms,
        case when coalesce(p.state->>'postExitMinExecutableNetCents','') ~ '^-?[0-9]+([.][0-9]+)?$' then (p.state->>'postExitMinExecutableNetCents')::double precision end as post_min,
        case when coalesce(p.state->>'terminalNetCents','') ~ '^-?[0-9]+([.][0-9]+)?$' then (p.state->>'terminalNetCents')::double precision end as terminal,
        case when coalesce(p.state->>'postExitMaxExecutableNetCents','') ~ '^-?[0-9]+([.][0-9]+)?$' then (p.state->>'postExitMaxExecutableNetCents')::double precision end as post_max,
        coalesce((p.state->>'trackingComplete')::boolean,false) as tracking_complete
      from sag_entries e left join sag_profit_episodes_v1 p on p.id=e.id
      where e.system_name=$1 and e.archived=false and e.status='closed' and e.close_reason='atomic_thunder_cashout'
    ), calc as (
      select *,
        case when post_min is null then terminal when terminal is null then post_min else least(post_min,terminal) end as worst,
        greatest(0,coalesce(post_max,0),coalesce(terminal,0)) as best
      from raw
    ) select count(*)::int as harvest_count,coalesce(sum(actual),0)::double precision as realized_pnl_cents,
      coalesce(sum(greatest(0,closed_at_ms-opened_at_ms)),0)::double precision as total_time_ms,
      coalesce(sum(case when worst is not null and worst<actual then greatest(0,actual-worst) else 0 end),0)::double precision as avoided_loss_cents,
      count(*) filter(where worst is not null and worst<actual and worst<0)::int as losses_avoided,
      coalesce(sum(greatest(0,best-actual)),0)::double precision as forgone_upside_cents,
      count(*) filter(where tracking_complete)::int as research_complete
      from calc`,[systemName]);
    ...
    const h=harvests.rows[0]||{};const harvestCount=Number(h.harvest_count||0),realizedPnlCents=Number(h.realized_pnl_cents||0),totalTimeMs=Number(h.total_time_ms||0);
    return {
      observedHunters:Number(byEvent.observing?.count||0),opportunitiesDetected:Number(byEvent.opportunity_detected?.count||0),harvestsExecuted:harvestCount,
      invalidOpportunitiesBlocked:Number(byEvent.invalid_opportunity_blocked?.count||0),confirmationResets:Number(byEvent.confirmation_reset?.count||0),
      realizedPnlCents,averageProfitCents:harvestCount?realizedPnlCents/harvestCount:0,averageTimeToHarvestMs:harvestCount?totalTimeMs/harvestCount:0,
      lossesAvoided:Number(h.losses_avoided||0),avoidedLossCents:Number(h.avoided_loss_cents||0),forgoneUpsideCents:Number(h.forgone_upside_cents||0),researchComplete:Number(h.research_complete||0),
