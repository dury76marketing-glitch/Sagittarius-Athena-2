...
export class StrategyEngine {
  ...
  async hunterEntryPolicyDecision(concept, q, { requireClock = true, includeCooldown = true, stage = 'policy', crystalWallOverlay = false, megaWaveAuthorized = false } = {}) {
    ...
    return {ok:true,reason:'qualified',...eventState,cooldownApplied:includeCooldown&&crystalWallOverlay!==true,eventCapBypassed:crystalWallOverlay===true&&eventState.eventCapBlocked,crystalWallOverlay:crystalWallOverlay===true,megaWaveAuthorized:megaWaveAuthorized===true};
  }
  ...
  async createHunter(concept, q, stakeCents, _legacyStopLossCents = 0, { sourceFeeder = null, sourceTradeId = null, sourceEntryConfig = null, recoverySourceSnapshot = null, crystalWallSourceSnapshot = null, justiceArrowSourceSnapshot = null, crashSourceSnapshot = null, entryQualificationSnapshot = null, athenaFireCommand = null, crystalWallFireCommand = null, justiceArrowFireCommand = null, megaWaveAuthorization = null, legacyCompatibility = false } = {}) {
    ...
    try {
      ...
      if(concept==='Athena Exclamation'&&!megaWaveAthenaEntry){
      ...
      if(MEGA_WAVE.downstreamSaints.includes(concept)&&!megaWaveSaintEntry){
      ...
      if(newGenerationEntry){
        ...
        if(concept==='Sagittarius Justice Arrow'&&!justiceArrowIndependentEntry&&!megaWaveSaintEntry){
      ...
      if(independentCrashRecoveryEntry){
        ...
        if(megaWaveAthenaEntry||megaWaveSaintEntry){
  ...
  async executeMegaWaveSaint(q,parentEntry,authorization,watch={}){
    ...
    if(String(durableParent?.id||'')!==String(authorization.parentEntryId)||String(durableParent?.systemName||'')!==String(s.systemName)||String(durableParent?.ownerId||'')!==String(s.ownerId)||String(durableParent?.mode||'')!==String(s.mode||'')||String(durableParent?.conceptName||'')!=='Athena Exclamation'||String(durableParent?.ticker||'')!==ticker||String(durableParent?.status||'')!=='closed'||Number(durableParent?.remainingCount||0)>1e-9||!(Number(durableParent?.pnlCents||0)>0)||String(parentMw?.version||'')!==MEGA_WAVE.version||String(parentMw?.policyRevision||'')!==MEGA_WAVE.policyRevision)return null;
