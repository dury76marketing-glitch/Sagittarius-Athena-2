...
// BOR1-R2 localizes an unprovable legacy ownership mismatch only after the
// authoritative broker has repeatedly reported exactly zero inventory and the
// complete Sagittarius client-prefix history proves this row has no own SELL
// receipt. The row is quarantined, never economically closed.
const BROKER_ORPHAN_ZERO_CONFIRMATIONS = 3;
const BROKER_ORPHAN_ZERO_MIN_SPAN_MS = 5_000;
const BROKER_ORPHAN_HISTORY_RECHECK_MS = 2_000;
const BROKER_ORPHAN_STATUS = 'reconciliation_orphaned';
const INFINITY_OBSERVING_AUDIT_MIN_INTERVAL_MS = 5_000;
...
function infinityBreakTotalEntryFee(entry, settings) {
  const stored=n(entry?.entryFeeCents);
  if(stored>0)return stored;
  const qty=Math.max(0,n(entry?.count));
  if(!(qty>0))return 0;
  if(String(entry?.mode||settings?.mode||'SIMULATION').toUpperCase()==='LIVE')
    return kalshiGeneralTakerFeeEstimateCents({count:qty,priceCents:n(entry?.entryPriceCents)});
  return Math.max(0,n(settings?.simFeeCents))*qty;
}

function infinityBreakAllocatedEntryFee(entry, filled, settings) {
  const original=Math.max(0,n(entry?.count));
  const qty=Math.max(0,n(filled));
  if(!(original>0)||!(qty>0))return 0;
  return infinityBreakTotalEntryFee(entry,settings)*(qty/original);
}

function infinityBreakExpectedExitFee(entry, count, priceCents, settings, consumedLevels=null) {
  const qty=Math.max(0,n(count));
  if(!(qty>0))return 0;
  if(String(entry?.mode||settings?.mode||'SIMULATION').toUpperCase()==='LIVE'){
    if(Array.isArray(consumedLevels)&&consumedLevels.length){
      return consumedLevels.reduce((sum,level)=>sum+kalshiGeneralTakerFeeEstimateCents({count:Math.max(0,n(level?.count)),priceCents:n(level?.priceCents)}),0);
    }
    return kalshiGeneralTakerFeeEstimateCents({count:qty,priceCents:n(priceCents)});
  }
  return Math.max(0,n(settings?.simFeeCents))*qty;
}
...
export class ProfitGuard {
  constructor({ db, kalshi, market, learning, athena = null, getSettings, onOpportunityCompleted = null, onPositionClosed = null }) {
    ...
    // IB1-R2: bounded, in-memory near-target wake metadata. This never grants
    // exit authority; it only lets the engine bypass the normal 125ms debounce
    // when an already-protected Infinity position is close to its frozen target.
    this.infinityBreakFastWake = new Map();
    // BOR1-R2 confirmation state is deliberately process-local. Restarting does
    // not inherit a quarantine decision; authoritative zero inventory + no own
    // exit receipt must be observed again before a local ledger row is isolated.
    this.brokerOrphanCandidates = new Map();
    // Infinity fast wake stays immediate, but low-value observing telemetry is
    // bounded independently so protection cannot turn into a hot DB write loop.
    this.infinityObservingAudit = new Map();
  ...
  pruneRuntimeState(activeEntries = []) {
    ...
    const activeTickers=new Set((activeEntries||[]).map((e)=>String(e?.ticker||'')).filter(Boolean));
    for(const ticker of [...this.infinityBreakFastWake.keys()])if(!activeTickers.has(String(ticker)))this.infinityBreakFastWake.delete(ticker);
    for(const id of [...this.brokerOrphanCandidates.keys()])if(!activeIds.has(String(id)))this.brokerOrphanCandidates.delete(id);
    for(const id of [...this.infinityObservingAudit.keys()])if(!activeIds.has(String(id)))this.infinityObservingAudit.delete(id);
  }
  ...
  resourceSnapshot() {
    return {
      ...
      infinityBreakFastWakeTickers:this.infinityBreakFastWake.size,
      infinityBreakPolicyRevision:INFINITY_BREAK.policyRevision,
      brokerOrphanCandidates:this.brokerOrphanCandidates.size,
      infinityObservingAuditStates:this.infinityObservingAudit.size,
  ...
  async liveTickerOwnershipSnapshot(entry) {
    ...
    // Reconciliation is owner-wide, not system-name-wide. A shared database may
    // legitimately hold several Sagittarius system names for the same owner;
    // every active LIVE Hunter row on the ticker must be covered before a SELL.
    let rows=[];
    if(typeof this.db.liveOpenHunterEntries==='function') rows=await this.db.liveOpenHunterEntries(s.ownerId);
    else if(typeof this.db.openHunterEntriesByTicker==='function') rows=await this.db.openHunterEntriesByTicker(s.systemName,entry.ticker);
    else if(typeof this.db.openEntriesByTicker==='function') rows=await this.db.openEntriesByTicker(s.systemName,entry.ticker);
    const owned=(rows||[]).filter((e)=>String(e.ticker||'')===String(entry.ticker||'')&&PORTFOLIO_CONCEPTS.has(e.conceptName)&&String(e.ownerId||'')===String(s.ownerId||'')&&String(e.mode||'')==='LIVE'&&['open','entry_pending','exit_pending','pending_recovery'].includes(String(e.status||'')));
    ...
    const brokerCount=(positions||[]).filter((p)=>String(p.ticker||p.market_ticker||'')===String(entry.ticker||'')).reduce((sum,p)=>{
  ...
  async recoverHistoricalExitFill(entry, { auditContext='ownership_reconciliation' }={}) {
    if(!entry||String(entry.mode||'')!=='LIVE'||remainingCount(entry)<=1e-9)return{recovered:false,reason:'not_recoverable'};
    if(entry.exitClientOrderId)return{recovered:false,reason:'exact_exit_receipt_present'};
    if(typeof this.kalshi?.clientOrderPrefix!=='function'||typeof this.kalshi?.inspectOrderHistoryByClientPrefix!=='function')return{recovered:false,reason:'broker_exit_history_reader_unavailable'};
    const s=this.getSettings();
    const prefix=this.kalshi.clientOrderPrefix(s.ownerId,entry.id,'exit');
    const minTsSec=Math.max(0,Math.floor(n(entry.openedAtMs)/1000)-120);
    const inspected=await this.kalshi.inspectOrderHistoryByClientPrefix(entry.ticker,prefix,minTsSec).catch((error)=>({found:false,authoritative:false,orders:[],error:String(error?.message||error)}));
    if(!inspected?.authoritative){
      await this.audit('live_exit_orphan_history_inconclusive',{id:entry.id,ticker:entry.ticker,clientOrderPrefix:prefix,auditContext,message:inspected?.error||null},'warning');
      return{recovered:false,reason:'broker_exit_history_inconclusive'};
    }
    if(n(inspected.fillCount)<=0){
      const pendingOrders=(inspected.orders||[]).filter((o)=>!terminalOrder(o));
      if(pendingOrders.length)return{recovered:false,reason:'broker_exit_history_pending',authoritativeHistory:true};
      return{recovered:false,reason:'no_authoritative_exit_fill',authoritativeHistory:true};
    }

    const persistedFilled=n(entry.exitFilledCount);
    const persistedNotional=n(entry.exitNotionalCents);
    const persistedFee=n(entry.exitFeeCents);
    const historicalFilled=n(inspected.fillCount);
    const historicalNotional=n(inspected.notionalCents);
    const historicalFee=n(inspected.feePaidCents);
    const missingFill=historicalFilled-persistedFilled;
    const missingNotional=historicalNotional-persistedNotional;
    const missingFee=historicalFee-persistedFee;
    if(missingFill<=1e-9)return{recovered:false,reason:'exit_history_already_persisted'};
    if(missingFill>remainingCount(entry)+1e-6||missingNotional<-1e-6||missingFee<-1e-6||!Number.isFinite(missingNotional)||missingNotional<=0){
      await this.audit('live_exit_orphan_history_conflict',{id:entry.id,ticker:entry.ticker,clientOrderPrefix:prefix,auditContext,historicalFilled,persistedFilled,remaining:remainingCount(entry),historicalNotional,persistedNotional,historicalFee,persistedFee},'error');
      return{recovered:false,reason:'broker_exit_history_conflict'};
    }
    const fillPriceCents=missingNotional/missingFill;
    if(!Number.isFinite(fillPriceCents)||fillPriceCents<0||fillPriceCents>100)return{recovered:false,reason:'broker_exit_history_invalid_price'};
    const lastFilled=[...(inspected.orders||[])].filter((o)=>n(o.fillCount)>0).at(-1)||null;
    // The historical client prefix proves Sagittarius itself submitted the SELL,
    // but an old release may not have durably retained the strategy reason.
    // Use a neutral reconciliation reason unless a durable order receipt exists;
    // this prevents falsely firing profit-continuation side effects.
    const decision={action:'broker_reconciled_exit',reason:'broker_reconciled_exit',peakPriceCents:entry.peakPriceCents,stopPriceCents:entry.stopPriceCents};
    const applied=await this.applyExitFill(entry,decision,{fillCount:missingFill,fillPriceCents,exitFeeCents:Math.max(0,missingFee),orderId:lastFilled?.orderId||entry.exitOrderId||null,clearClientId:true});
    await this.audit('live_exit_orphan_fill_recovered',{id:entry.id,ticker:entry.ticker,concept:entry.conceptName,auditContext,clientOrderPrefix:prefix,missingFill,fillPriceCents,exitFeeCents:Math.max(0,missingFee),historicalFilled,persistedFilled,remainingAfter:applied.remaining,closed:applied.closed,orders:(inspected.orders||[]).map((o)=>({orderId:o.orderId,clientOrderId:o.clientOrderId,fillCount:o.fillCount,status:o.status}))},'warning');
    return{recovered:true,...applied};
  }

  async quarantineZeroInventoryOrphan(entry,{ticker,brokerCount,observation,nowMs=Date.now()}={}) {
    if(!entry||String(entry.mode||'')!=='LIVE'||remainingCount(entry)<=1e-9)return{quarantined:false,reason:'not_quarantinable'};
    if(brokerCount===null||brokerCount===undefined||brokerCount==='')return{quarantined:false,reason:'broker_inventory_unavailable'};
    const authoritativeBrokerCount=Number(brokerCount);
    if(!Number.isFinite(authoritativeBrokerCount))return{quarantined:false,reason:'broker_inventory_unavailable'};
    if(Math.abs(authoritativeBrokerCount)>1e-9)return{quarantined:false,reason:'broker_inventory_nonzero'};
    if(entry.exitClientOrderId)return{quarantined:false,reason:'exact_exit_receipt_present'};
    const state=observation||this.brokerOrphanCandidates.get(entry.id);
    if(!state||n(state.confirmations)<BROKER_ORPHAN_ZERO_CONFIRMATIONS||n(nowMs)-n(state.firstObservedAtMs)<BROKER_ORPHAN_ZERO_MIN_SPAN_MS)return{quarantined:false,reason:'zero_inventory_confirmation_pending'};
    const fresh=typeof this.db.entryById==='function'?await this.db.entryById(entry.id):entry;
    if(!fresh||remainingCount(fresh)<=1e-9||!['open','exit_pending','pending_recovery'].includes(String(fresh.status||''))||fresh.exitClientOrderId)return{quarantined:false,reason:'entry_changed_before_quarantine'};
    const priorPostExit=fresh.postExitState&&typeof fresh.postExitState==='object'?fresh.postExitState:{};
    const quarantine={
      version:'BOR1',policyRevision:'BOR1-R2-ZERO-INVENTORY-ORPHAN-QUARANTINE',
      reason:'authoritative_broker_zero_no_own_exit_fill',ticker:String(ticker||fresh.ticker||''),
      brokerCount:0,remainingCount:remainingCount(fresh),originalStatus:String(fresh.status||''),
      confirmations:n(state.confirmations),firstObservedAtMs:n(state.firstObservedAtMs),lastObservedAtMs:n(state.lastObservedAtMs),
      quarantinedAtMs:n(nowMs),economicClosure:false,pnlInferred:false,exitPriceInferred:false,
    };
    await this.db.updateEntry(fresh.id,{
      status:BROKER_ORPHAN_STATUS,
      closeReason:'broker_zero_inventory_orphan_quarantined',
      postExitState:{...priorPostExit,brokerOwnershipQuarantine:quarantine},
      updatedAtMs:n(nowMs),
    });
    this.brokerOrphanCandidates.delete(fresh.id);
    this.atomicThunderStates.delete(fresh.id);
    this.states.delete(fresh.id);
    this.infinityObservingAudit.delete(fresh.id);
    await this.audit('live_broker_ownership_orphan_quarantined',{
      id:fresh.id,ticker:fresh.ticker,concept:fresh.conceptName,brokerCount:0,remainingCount:remainingCount(fresh),
      confirmations:quarantine.confirmations,firstObservedAtMs:quarantine.firstObservedAtMs,lastObservedAtMs:quarantine.lastObservedAtMs,
      policyRevision:quarantine.policyRevision,economicClosure:false,pnlInferred:false,exitPriceInferred:false,
    },'warning');
    return{quarantined:true,id:fresh.id,status:BROKER_ORPHAN_STATUS,remaining:remainingCount(fresh)};
  }

  async reconcileBrokerOwnershipShortfall({ticker,brokerCount=null,entries=[],nowMs=Date.now()}={}) {
    const candidates=(entries||[]).filter((e)=>e&&String(e.mode||'')==='LIVE'&&String(e.ticker||'')===String(ticker||'')&&remainingCount(e)>1e-9);
    let recovered=0;
    const authoritativeNoFill=[];
    for(const original of candidates){
      let entry=typeof this.db.entryById==='function'?await this.db.entryById(original.id):original;
      if(!entry||remainingCount(entry)<=1e-9||!['open','exit_pending','pending_recovery'].includes(String(entry.status||'')))continue;
      let out=null;
      if(entry.exitClientOrderId){
        this.brokerOrphanCandidates.delete(entry.id);
        const reason=String(entry.closeReason||'broker_reconciled_exit');
        out=await this.reconcileExistingExitIntent(entry,{action:reason,reason,peakPriceCents:entry.peakPriceCents,stopPriceCents:entry.stopPriceCents}).catch(()=>null);
      }else{
        const prior=this.brokerOrphanCandidates.get(entry.id);
        const historyDue=!prior||n(nowMs)-n(prior.lastHistoryCheckedAtMs)>=BROKER_ORPHAN_HISTORY_RECHECK_MS;
        if(historyDue){
          out=await this.recoverHistoricalExitFill(entry,{auditContext:'broker_reconciliation'}).catch(()=>null);
          if(out?.reason==='no_authoritative_exit_fill'&&out?.authoritativeHistory){
            const next={
              firstObservedAtMs:prior?.firstObservedAtMs??n(nowMs),lastObservedAtMs:n(nowMs),lastHistoryCheckedAtMs:n(nowMs),
              confirmations:n(prior?.confirmations)+1,brokerCount:n(brokerCount),ticker:String(ticker||entry.ticker||''),
            };
            this.brokerOrphanCandidates.set(entry.id,next);
            authoritativeNoFill.push({entry,observation:next});
          }else if(!out?.recovered&&!out?.closed&&n(out?.filled)<=0){
            this.brokerOrphanCandidates.delete(entry.id);
          }
        }
      }
      if(out?.closed||n(out?.filled)>0||out?.recovered)recovered+=1;
    }
    if(recovered>0){
      // A recovered fill changes the ledger. Never quarantine any sibling row
      // from the stale pre-recovery broker snapshot; the engine must re-read
      // both authorities first.
      for(const x of candidates)this.brokerOrphanCandidates.delete(x.id);
      await this.audit('live_broker_ownership_shortfall_recovery',{ticker,brokerCount,recovered,candidates:candidates.length},'warning');
      return{recovered:true,recoveredEntries:recovered,quarantined:false,quarantinedEntries:0,candidates:candidates.length};
    }

    let quarantined=0;
    // Positive-but-insufficient broker inventory is ambiguous ownership and must
    // remain globally fail-closed. Only repeated exact ZERO inventory can be
    // localized, because there is no broker exposure left for this local row to
    // protect or accidentally sell.
    const authoritativeBrokerCount=(brokerCount===null||brokerCount===undefined||brokerCount==='')?NaN:Number(brokerCount);
    if(Number.isFinite(authoritativeBrokerCount)&&Math.abs(authoritativeBrokerCount)<=1e-9){
      for(const {entry,observation} of authoritativeNoFill){
        const out=await this.quarantineZeroInventoryOrphan(entry,{ticker,brokerCount,observation,nowMs});
        if(out?.quarantined)quarantined+=1;
      }
    }else{
      for(const x of candidates)this.brokerOrphanCandidates.delete(x.id);
    }
    return{recovered:false,recoveredEntries:0,quarantined:quarantined>0,quarantinedEntries:quarantined,candidates:candidates.length};
  }
  ...
  infinityBreakExecutableNetCents(entry, count, executableBidCents, settings) {
    const qty=Math.max(0,n(count));
    if(qty<=1e-9||!Number.isFinite(Number(executableBidCents)))return null;
    const entryFee=infinityBreakAllocatedEntryFee(entry,qty,settings);
    const expectedExitFee=infinityBreakExpectedExitFee(entry,qty,executableBidCents,settings);
    return n(entry?.pnlCents)+(n(executableBidCents)-n(entry?.entryPriceCents))*qty-entryFee-expectedExitFee;
  }

  infinityBreakTargetPriceCents(entry, count, settings, targetNetCents = 0) {
    const qty=Math.max(1e-9,n(count));
    // LIVE Kalshi exit fees are price-dependent. Search the integer contract
    // prices exactly rather than reusing the SIM linear approximation.
    for(let px=1;px<=99;px+=1){
      const net=this.infinityBreakExecutableNetCents(entry,qty,px,settings);
      if(n(net,-Infinity)+1e-9>=n(targetNetCents))return px;
    }
    return 100;
  }

  infinityBreakFastWakeThresholdCents(entry, settings=this.getSettings()) {
    if(!this.isInfinityBreakTrade(entry))return null;
    const remain=remainingCount(entry);
    if(!(remain>0))return null;
    return this.infinityBreakTargetPriceCents(entry,remain,settings,this.atomicThunderTargetNetCents(entry,settings));
  }

  registerInfinityBreakFastWake(entry, settings=this.getSettings()) {
    if(!entry?.ticker||!this.isInfinityBreakTrade(entry))return false;
    const target=this.infinityBreakFastWakeThresholdCents(entry,settings);
    if(!Number.isFinite(Number(target)))return false;
    const ticker=String(entry.ticker);
    const prior=this.infinityBreakFastWake.get(ticker);
    // Multiple Galactic Explosion rows may share a ticker. Wake for the lowest
    // near-target threshold so no owned Infinity position is starved.
    this.infinityBreakFastWake.set(ticker,prior==null?target:Math.min(n(prior,target),target));
    return true;
  }

  shouldFastWakeInfinity(ticker, q=null) {
    const threshold=this.infinityBreakFastWake.get(String(ticker||''));
    if(!Number.isFinite(Number(threshold)))return false;
    const quote=q||this.market?.getQuote?.(ticker)||null;
    const bid=n(quote?.yesBid);
    return bid>0&&bid+2+1e-9>=n(threshold);
  }

  async assessInfinityBreakExecution(entry, q, settings=this.getSettings(), {ensureFresh=true}={}) {
    const policy=this.atomicThunderPolicy(entry,settings);
    const remain=remainingCount(entry);
    const targetNetCents=this.atomicThunderTargetNetCents(entry,settings);
    const exitFloorCents=this.infinityBreakTargetPriceCents(entry,remain,settings,targetNetCents);
    if(!(remain>0))return{ok:false,qualifies:false,reason:'no_remaining_position',policy,targetNetCents,exitFloorCents,remaining:remain};
    if(ensureFresh){
      if(typeof this.market?.ensureFreshBook!=='function')return{ok:false,qualifies:false,reason:'fresh_book_revalidation_unavailable',policy,targetNetCents,exitFloorCents,remaining:remain};
      try{
        const refreshed=await this.market.ensureFreshBook(entry.ticker,policy.maximumBookAgeMs);
        if(!refreshed)return{ok:false,qualifies:false,reason:'fresh_book_revalidation_failed',policy,targetNetCents,exitFloorCents,remaining:remain};
      }catch(error){
        return{ok:false,qualifies:false,reason:'fresh_book_revalidation_failed',policy,targetNetCents,exitFloorCents,remaining:remain,message:String(error?.message||error)};
      }
    }
    const freshQ=this.market.getQuote(entry.ticker)||q;
    const book=this.market.getBook?.(entry.ticker);
    ...
    if(!validFresh)return{ok:false,qualifies:false,reason:'stale_or_invalid_book',policy,q:freshQ,targetNetCents,exitFloorCents,remaining:remain,bookMs,bookAgeMs,quoteAgeMs};
    ...
    const executableNetCents=fullExecutable?n(entry.pnlCents)+(n(executableBidCents)-n(entry.entryPriceCents))*remain-infinityBreakAllocatedEntryFee(entry,remain,settings)-infinityBreakExpectedExitFee(entry,remain,executableBidCents,settings,exec?.consumedLevels):null;
    const qualifies=fullExecutable&&n(executableNetCents,-Infinity)+1e-9>=targetNetCents;
    const lowestConsumedCents=fullExecutable?n(exec?.lowestConsumedCents,executableBidCents):null;
    const expectedExitFeeCents=fullExecutable?infinityBreakExpectedExitFee(entry,remain,executableBidCents,settings,exec?.consumedLevels):null;
    const entryFeeCents=infinityBreakAllocatedEntryFee(entry,remain,settings);
    return{ok:true,qualifies,reason:!fullExecutable?'insufficient_full_position_depth':qualifies?'target_met':'net_below_threshold',policy,q:freshQ,targetNetCents,exitFloorCents,remaining:remain,bookMs,bookAgeMs,quoteAgeMs,exec,fullExecutable,executableBidCents,executableNetCents,lowestConsumedCents,expectedExitFeeCents,entryFeeCents};
  }
  ...
  isMegaWaveAthenaTrade(entry) {
    if (!entry || String(entry?.conceptName || '') !== String(MEGA_WAVE.athenaConcept)) return false;
    const mw=entry?.entryConfig?.athenaExclamation?.megaWaveAuthorization || entry?.entryConfig?.megaWave || {};
    return String(mw?.version || '') === String(MEGA_WAVE.version)
      && String(mw?.policyRevision || '') === String(MEGA_WAVE.policyRevision);
  }
  ...
  shouldNotifyPositionClosed(entry, decision = null) {
    ...
    if (this.isMegaWaveAthenaTrade(entry)) return true;
    // Preserve the pre-Mega-Wave callback contract for all other concepts.
    return Boolean(decision && this.isInfinityBreakExitDecision(entry, decision));
  }
  ...
  atomicThunderPolicy(entry, settings = this.getSettings()) {
    if (this.isInfinityBreakTrade(entry)) {
      ...
      return {
        ...
        minimumNetPerOriginalContractCents:Math.max(0.01,n(frozen?.minimumNetPerOriginalContractCents,INFINITY_BREAK.defaultMinimumNetPerOriginalContractCents)),
  ...
  atomicThunderExitFloorCents(entry, settings = this.getSettings()) {
    const remain=remainingCount(entry),target=this.atomicThunderTargetNetCents(entry,settings);
    return this.isInfinityBreakTrade(entry)
      ? this.infinityBreakTargetPriceCents(entry,remain,settings,target)
      : this.priceForAggregateNetTargetCents(entry,remain,settings,target);
  }

  async recordAtomicThunderEvent(entry, eventType, data = {}, eventSuffix = '') {
    if (this.isInfinityBreakTrade(entry)) {
      if(eventType==='observing'&&entry?.id){
        const now=Date.now();
        const prior=n(this.infinityObservingAudit.get(entry.id));
        if(prior>0&&now-prior<INFINITY_OBSERVING_AUDIT_MIN_INTERVAL_MS)return false;
        this.infinityObservingAudit.set(entry.id,now);
      }
      await this.audit(`infinity_break_${eventType}`,{id:entry?.id||null,ticker:entry?.ticker||null,...(data||{})}).catch(()=>{});
      return true;
    }
  ...
  atomicThunderTelemetry(entry, assessment = null) {
    ...
    const out={profitAuthority:authority,profitAuthorityPolicyRevision:revision,profitHarvestState:a.triggered?`${prefix}_READY`:n(state.confirmations)>0?'PROFIT_CONFIRMING':'OBSERVING',profitHarvestConfirmations:n(state.confirmations),profitHarvestRequiredConfirmations:n(a.policy?.requiredFreshConfirmations,n(state.requiredFreshConfirmations,2)),profitHarvestExecutableNetCents:Number.isFinite(Number(a.executableNetCents))?n(a.executableNetCents):null,profitHarvestTargetNetCents:Number.isFinite(Number(a.targetNetCents))?n(a.targetNetCents):null,profitHarvestExitFloorCents:Number.isFinite(Number(a.exitFloorCents))?n(a.exitFloorCents):null,profitHarvestExecutableBidCents:Number.isFinite(Number(a.executableBidCents))?n(a.executableBidCents):null,profitHarvestLowestConsumedCents:Number.isFinite(Number(a.lowestConsumedCents))?n(a.lowestConsumedCents):null,profitHarvestExpectedExitFeeCents:Number.isFinite(Number(a.expectedExitFeeCents))?n(a.expectedExitFeeCents):null,profitHarvestEntryFeeCents:Number.isFinite(Number(a.entryFeeCents))?n(a.entryFeeCents):null,profitHarvestBookMs:Number.isFinite(Number(a.bookMs))?n(a.bookMs):null};
    if(isInfinity)return{...out,infinityBreak:INFINITY_BREAK.version,infinityBreakPolicyRevision:INFINITY_BREAK.policyRevision,infinityBreakState:out.profitHarvestState,infinityBreakConfirmations:out.profitHarvestConfirmations,infinityBreakRequiredConfirmations:out.profitHarvestRequiredConfirmations,infinityBreakExecutableNetCents:out.profitHarvestExecutableNetCents,infinityBreakTargetNetCents:out.profitHarvestTargetNetCents,infinityBreakExitFloorCents:out.profitHarvestExitFloorCents,infinityBreakExecutableBidCents:out.profitHarvestExecutableBidCents,infinityBreakLowestConsumedCents:out.profitHarvestLowestConsumedCents,infinityBreakExpectedExitFeeCents:out.profitHarvestExpectedExitFeeCents,infinityBreakEntryFeeCents:out.profitHarvestEntryFeeCents,infinityBreakBookMs:out.profitHarvestBookMs};
  ...
  async evaluateAtomicThunder(entry, q, settings) {
    ...
    const fullExecutable = Boolean(exec?.full && n(exec?.filled) + 1e-9 >= remain && n(exec?.avgCents) > 0);
    const executableBidCents = fullExecutable ? n(exec.avgCents) : null;
    const infinityBreak = this.isInfinityBreakTrade(entry);
    const executableNetCents = fullExecutable
      ? (infinityBreak
        ? n(entry.pnlCents)+(n(executableBidCents)-n(entry.entryPriceCents))*remain-infinityBreakAllocatedEntryFee(entry,remain,settings)-infinityBreakExpectedExitFee(entry,remain,executableBidCents,settings,exec?.consumedLevels)
        : this.aggregateExecutableNetCents(entry, remain, executableBidCents, settings))
      : null;
    const lowestConsumedCents = fullExecutable ? n(exec?.lowestConsumedCents,executableBidCents) : null;
    const expectedExitFeeCents = fullExecutable
      ? (infinityBreak ? infinityBreakExpectedExitFee(entry, remain, executableBidCents, settings,exec?.consumedLevels) : n(settings.simFeeCents) * remain)
      : null;
    const entryFeeCents = infinityBreak ? infinityBreakAllocatedEntryFee(entry, remain, settings) : allocatedEntryFee(entry, remain, settings);
    const qualifies = fullExecutable && n(executableNetCents, -Infinity) + 1e-9 >= targetNetCents;
    ...
    if (!qualifies) {
      ...
      return { eligible:true, triggered:false, policy, q:freshQ, reason:!fullExecutable?'insufficient_full_position_depth':'net_below_threshold', targetNetCents, exitFloorCents, executableNetCents, executableBidCents, lowestConsumedCents, expectedExitFeeCents, entryFeeCents, fullExecutable, bookMs };
    }
    ...
    return { eligible:true, triggered, policy, q:freshQ, targetNetCents, exitFloorCents, executableNetCents, executableBidCents, lowestConsumedCents, expectedExitFeeCents, entryFeeCents, fullExecutable:true, confirmations, bookMs, state };
  }
  ...
  async evaluateProtectedRunnerR2(entry, q, settings) {
    ...
    if (!state) {
      if (executableNetCents + 1e-9 < configuredActivationTargetNetCents) {
        return { q:freshQ, state:null, armed:false, committed:false, triggered:false, executable:true, executableFull:true, executableBidCents, executableNetCents, breakEvenPriceCents, capitalLatchTargetNetCents, configuredActivationTargetNetCents, profitFloorArmTargetNetCents };
      }
      let profile = {
  ...
  async applyExitFill(entry, decision, { fillCount, fillPriceCents, exitFeeCents = 0, orderId = null, clearClientId = true }) {
    ...
    if (infinityBreakExit) {
      const prior=entry?.profitGuardState?.version===INFINITY_BREAK.version?entry.profitGuardState:{};
      nextInfinityState={
        ...prior,version:INFINITY_BREAK.version,policyRevision:INFINITY_BREAK.policyRevision,
        phase:remaining<=1e-9?'INFINITY_BREAK_EXIT_FILLED':'INFINITY_BREAK_EXIT_PARTIAL',
        targetNetCents:n(decision.infinityBreakTargetNetCents,prior.targetNetCents),
        executableNetCents:n(decision.infinityBreakExecutableNetCents,prior.executableNetCents),
        executableBidCents:n(decision.infinityBreakExecutableBidCents,prior.executableBidCents),
        lowestConsumedCents:n(decision.infinityBreakLowestConsumedCents,prior.lowestConsumedCents),
        expectedExitFeeCents:n(decision.infinityBreakExpectedExitFeeCents,prior.expectedExitFeeCents),
        entryFeeCents:n(decision.infinityBreakEntryFeeCents,prior.entryFeeCents),
        bookMs:n(decision.infinityBreakBookMs,prior.bookMs),
        revalidatedAtMs:n(decision.infinityBreakRevalidatedAtMs,prior.revalidatedAtMs),
        exitFilledCount,exitVwapCents:averageExit,realizedNetCents:pnl,remainingCount:remaining,
        lastFillPriceCents:px,lastFillCount:filled,exitOrderId:orderId||entry.exitOrderId||null,
        ...(remaining<=1e-9?{exitFilledAtMs:closedAtMs}:{}),updatedAtMs:Date.now(),
      };
    }
    const patch = {
      ...
      ...(nextX1State ? { profitGuardState:nextX1State } : nextPri1State ? { profitGuardState:nextPri1State } : nextInfinityState ? { profitGuardState:nextInfinityState } : {}),
      updatedAtMs: Date.now(),
    };
    await this.db.updateEntry(entry.id, patch);

    const closedEntry = { ...entry, ...patch };
    // The economic close is durable before the strategic handoff. Never make
    // Mega Wave depend on which profit executor produced that profitable close.
    if(remaining<=1e-9&&this.shouldNotifyPositionClosed(closedEntry,decision))this.notifyPositionClosed(closedEntry);
    ...
    if (atomicThunderExit) {
      ...
      const targetNetCents=infinityBreakExit?n(decision.infinityBreakTargetNetCents):n(decision.atomicThunderTargetNetCents);
      const executableNetCents=infinityBreakExit?n(decision.infinityBreakExecutableNetCents):n(decision.atomicThunderExecutableNetCents);
      const executableBidCents=infinityBreakExit?n(decision.infinityBreakExecutableBidCents):n(decision.atomicThunderExecutableBidCents);
      await this.recordAtomicThunderEvent(closedEntry, eventType, {
        ...
        targetNetCents,executableNetCents,executableBidCents,
        lowestConsumedCents:infinityBreakExit?n(decision.infinityBreakLowestConsumedCents):null,
        expectedExitFeeCents:infinityBreakExit?n(decision.infinityBreakExpectedExitFeeCents):null,
        revalidatedBookMs:infinityBreakExit?n(decision.infinityBreakBookMs):null,
      });
      await this.audit(infinityBreakExit
        ...
        id:entry.id, ticker:entry.ticker, concept:entry.conceptName, filled, remaining,
        fillPriceCents:px, exitVwapCents:averageExit, realizedNetCents:pnl,
        ...(infinityBreakExit?{targetNetCents,executableNetCents,executableBidCents,lowestConsumedCents:n(decision.infinityBreakLowestConsumedCents),expectedExitFeeCents:n(decision.infinityBreakExpectedExitFeeCents),entryFeeCents:n(decision.infinityBreakEntryFeeCents),revalidatedBookMs:n(decision.infinityBreakBookMs)}:{}),
      });
  ...
  async closeAtSettlement(entry, decision) {
    ...
    if (remain <= 1e-9) {
      ...
      await this.db.updateEntry(entry.id, patch);
      const closedEntry={...entry,...patch};
      if(this.shouldNotifyPositionClosed(closedEntry,decision))this.notifyPositionClosed(closedEntry);
  ...
  async simulationExit(entry, decision, q) {
    ...
    if (profitProtectedExit && !infinityBreakExit && bid < exitFloorCents) {
      if (atomicThunderExit) {
    ...
    let infinityRevalidation=null;
    if(infinityBreakExit){
      infinityRevalidation=await this.assessInfinityBreakExecution(entry,freshQ,s,{ensureFresh:true});
      if(!infinityRevalidation?.qualifies){
        this.atomicThunderStates.delete(entry.id);
        await this.audit('infinity_break_exit_window_lost',{
          id:entry.id,ticker:entry.ticker,mode:'SIMULATION',bidCents:n(infinityRevalidation?.q?.yesBid,bid),
          profitFloorCents:n(infinityRevalidation?.exitFloorCents,exitFloorCents),remaining:remainingCount(entry),
          targetNetCents:n(infinityRevalidation?.targetNetCents),executableNetCents:infinityRevalidation?.executableNetCents??null,
          executableBidCents:infinityRevalidation?.executableBidCents??null,lowestConsumedCents:infinityRevalidation?.lowestConsumedCents??null,
          reason:infinityRevalidation?.reason||'revalidation_failed'
        },'warning');
        return{closed:false,pending:false,remaining:remainingCount(entry),skipped:'infinity_break_revalidation_failed'};
      }
    }
    const exec = infinityBreakExit ? infinityRevalidation.exec : this.market.executableBid(entry.ticker, remainingCount(entry), exitFloorCents);
    ...
    const executionPrice=n(exec.avgCents,bid);
    const fee = infinityBreakExit
      ? infinityBreakExpectedExitFee(entry,executionCount,executionPrice,s)
      : n(s.simFeeCents) * executionCount;
    const effectiveDecision=infinityBreakExit?{...decision,
      infinityBreakTargetNetCents:n(infinityRevalidation?.targetNetCents,decision.infinityBreakTargetNetCents),
      infinityBreakExecutableNetCents:n(infinityRevalidation?.executableNetCents,decision.infinityBreakExecutableNetCents),
      infinityBreakExecutableBidCents:n(infinityRevalidation?.executableBidCents,executionPrice),
      infinityBreakLowestConsumedCents:n(infinityRevalidation?.lowestConsumedCents,exec.lowestConsumedCents),
      infinityBreakExpectedExitFeeCents:n(infinityRevalidation?.expectedExitFeeCents,fee),
      infinityBreakEntryFeeCents:n(infinityRevalidation?.entryFeeCents),
      infinityBreakBookMs:n(infinityRevalidation?.bookMs,bookMs),
      infinityBreakRevalidatedAtMs:Date.now(),
    }:decision;
    const result = await this.applyExitFill(entry, effectiveDecision, {
      fillCount: executionCount,
      fillPriceCents: executionPrice,
      exitFeeCents: fee,
      clearClientId: true,
    });
  ...
  async reconcileExistingExitIntent(entry, decision) {
    const persistedInfinity=entry?.profitGuardState?.version===INFINITY_BREAK.version?entry.profitGuardState:null;
    const infinityPending=this.isInfinityBreakTrade(entry)&&String(entry?.closeReason||decision?.reason||'')==='infinity_break';
    const effectiveDecision=infinityPending&&persistedInfinity?{...decision,
      infinityBreak:INFINITY_BREAK.version,infinityBreakPolicyRevision:INFINITY_BREAK.policyRevision,
      infinityBreakTargetNetCents:n(persistedInfinity.targetNetCents,decision?.infinityBreakTargetNetCents),
      infinityBreakExecutableNetCents:n(persistedInfinity.executableNetCents,decision?.infinityBreakExecutableNetCents),
      infinityBreakExecutableBidCents:n(persistedInfinity.executableBidCents,decision?.infinityBreakExecutableBidCents),
      infinityBreakLowestConsumedCents:n(persistedInfinity.lowestConsumedCents,decision?.infinityBreakLowestConsumedCents),
      infinityBreakExpectedExitFeeCents:n(persistedInfinity.expectedExitFeeCents,decision?.infinityBreakExpectedExitFeeCents),
      infinityBreakEntryFeeCents:n(persistedInfinity.entryFeeCents,decision?.infinityBreakEntryFeeCents),
      infinityBreakBookMs:n(persistedInfinity.bookMs,decision?.infinityBreakBookMs),
      infinityBreakRevalidatedAtMs:n(persistedInfinity.revalidatedAtMs,decision?.infinityBreakRevalidatedAtMs),
    }:decision;
    const inspected = await this.kalshi.inspectOrderByClientId(
      entry.ticker,
    ...
    const truth = inspected.order;
    if (truth?.fillCount > 0) {
      const fillPx=n(truth.averageFillPriceCents,entry.currentPriceCents);
      const exitFee = Number.isFinite(Number(truth.feePaidCents))
        ? Number(truth.feePaidCents)
        : infinityPending ? infinityBreakExpectedExitFee(entry,truth.fillCount,fillPx,this.getSettings()) : n(this.getSettings().simFeeCents) * truth.fillCount;
      return this.applyExitFill(entry, effectiveDecision, {
        fillCount: truth.fillCount,
        fillPriceCents: fillPx,
    ...
    this.setState(entry, { ...effectiveDecision, action: 'committed_exit', guardState: 'EXIT_PENDING_RECONCILIATION' });
  ...
  async _liveExit(entry, decision) {
    ...
    const exec = this.market.executableBid(entry.ticker, remain, infinityBreakExit ? 1 : exitFloorCents);
    ...
    const preliminaryInfinityNet = infinityBreakExit && exec?.full
      ? n(entry.pnlCents)+(n(exec.avgCents)-n(entry.entryPriceCents))*remain-infinityBreakAllocatedEntryFee(entry,remain,s)-infinityBreakExpectedExitFee(entry,remain,n(exec.avgCents),s,exec?.consumedLevels)
      : null;
    const preliminaryInfinityTarget = infinityBreakExit ? this.atomicThunderTargetNetCents(entry,s) : null;
    if(infinityBreakExit && (!exec?.full || n(exec.filled)+1e-9<remain || n(preliminaryInfinityNet,-Infinity)+1e-9<n(preliminaryInfinityTarget))){
      this.atomicThunderStates.delete(entry.id);
    ...
    let ownership=await this.liveTickerOwnershipSnapshot(entry);
    if(!ownership.ok&&ownership.reason==='broker_below_owned_ledger'){
      // Before blocking, reconstruct only exact Sagittarius-owned historical
      // exit receipts. Never infer a fill from aggregate broker quantity alone.
      const repaired=await this.reconcileBrokerOwnershipShortfall({ticker:entry.ticker,brokerCount:ownership.brokerCount,entries:ownership.ownedEntries||[entry]});
      if(repaired.recovered||repaired.quarantined){
        const current=typeof this.db.entryById==='function'?await this.db.entryById(entry.id):entry;
        if(!current||remainingCount(current)<=1e-9||String(current.status||'')==='closed')return{closed:true,remaining:0,reconciled:true};
        if(String(current.status||'')===BROKER_ORPHAN_STATUS){
          return{closed:false,pending:false,remaining:remainingCount(current),quarantined:true,skipped:'broker_ownership_orphan_quarantined'};
        }
        entry=current;
    ...
    if(!ownership.ok){
      ...
      // Automatic exits must not manufacture a broker-commit lifecycle state
      // when no mutation receipt exists. Explicit operator emergency/manual
      // intent remains sticky by design so it executes as soon as ownership is
      // provable; that intent is not treated as evidence that a SELL was sent.
      const explicitOperatorIntent=['emergency_exit','manual_cashout'].includes(String(decision?.reason||''));
      if(explicitOperatorIntent&&String(entry.status||'')!=='exit_pending'){
        const now=Date.now();
        await this.db.updateEntry(entry.id,{status:'exit_pending',closeReason:decision.reason,updatedAtMs:now});
        entry={...entry,status:'exit_pending',closeReason:decision.reason,updatedAtMs:now};
      }
      return{closed:false,pending:String(entry.status||'')==='exit_pending',remaining:remainingCount(entry),skipped:ownership.reason};
    }
    ...
    const durableIntentAtMs=Date.now();
    await this.db.updateEntry(entry.id, {
      status: 'exit_pending', exitClientOrderId: client, closeReason: decision.reason,
      peakPriceCents: decision.peakPriceCents ?? entry.peakPriceCents,
      stopPriceCents: decision.stopPriceCents ?? entry.stopPriceCents,
      ...(infinityBreakExit?{profitGuardState:{
        version:INFINITY_BREAK.version,policyRevision:INFINITY_BREAK.policyRevision,phase:'INFINITY_BREAK_EXIT_INTENT_DURABLE',
        targetNetCents:n(decision.infinityBreakTargetNetCents,preliminaryInfinityTarget),
        executableNetCents:n(decision.infinityBreakExecutableNetCents,preliminaryInfinityNet),
        executableBidCents:n(decision.infinityBreakExecutableBidCents,exec?.avgCents),
        lowestConsumedCents:n(decision.infinityBreakLowestConsumedCents,exec?.lowestConsumedCents),
        committedAtMs:durableIntentAtMs,updatedAtMs:durableIntentAtMs,
      }}:{}),
      updatedAtMs: durableIntentAtMs,
    });

    // IB1-R2 final authority boundary: after ownership proof and after the exact
    // client mutation receipt is durable, reacquire a fresh book and recompute
    // the SAME aggregate full-position net covenant used by detection. If the
    // window disappeared, clear the unsent intent and fail closed.
    let infinityRevalidation=null;
    let effectiveDecision=decision;
    if(infinityBreakExit){
      infinityRevalidation=await this.assessInfinityBreakExecution(entry,q,s,{ensureFresh:true});
      if(!infinityRevalidation?.qualifies||!Number.isFinite(Number(infinityRevalidation?.lowestConsumedCents))){
        this.atomicThunderStates.delete(entry.id);
        const lostAtMs=Date.now();
        await this.db.updateEntry(entry.id,{status:'open',exitClientOrderId:null,closeReason:null,profitGuardState:{version:INFINITY_BREAK.version,policyRevision:INFINITY_BREAK.policyRevision,phase:'INFINITY_BREAK_WINDOW_LOST_BEFORE_SUBMIT',targetNetCents:n(infinityRevalidation?.targetNetCents,preliminaryInfinityTarget),executableNetCents:infinityRevalidation?.executableNetCents??null,executableBidCents:infinityRevalidation?.executableBidCents??null,lowestConsumedCents:infinityRevalidation?.lowestConsumedCents??null,updatedAtMs:lostAtMs},updatedAtMs:lostAtMs});
        await this.audit('infinity_break_exit_window_lost',{id:entry.id,ticker:entry.ticker,mode:'LIVE',bidCents:n(infinityRevalidation?.q?.yesBid,bid),profitFloorCents:n(infinityRevalidation?.exitFloorCents,exitFloorCents),remaining:remain,targetNetCents:n(infinityRevalidation?.targetNetCents,preliminaryInfinityTarget),executableNetCents:infinityRevalidation?.executableNetCents??null,executableBidCents:infinityRevalidation?.executableBidCents??null,lowestConsumedCents:infinityRevalidation?.lowestConsumedCents??null,reason:infinityRevalidation?.reason||'revalidation_failed',stage:'immediately_before_submit'},'warning');
        return{closed:false,pending:false,remaining:remain,skipped:'infinity_break_revalidation_failed'};
      }
      effectiveDecision={...decision,
        infinityBreakTargetNetCents:n(infinityRevalidation.targetNetCents),
        infinityBreakExecutableNetCents:n(infinityRevalidation.executableNetCents),
        infinityBreakExecutableBidCents:n(infinityRevalidation.executableBidCents),
        infinityBreakLowestConsumedCents:n(infinityRevalidation.lowestConsumedCents),
        infinityBreakExpectedExitFeeCents:n(infinityRevalidation.expectedExitFeeCents),
        infinityBreakEntryFeeCents:n(infinityRevalidation.entryFeeCents),
        infinityBreakBookMs:n(infinityRevalidation.bookMs),
        infinityBreakRevalidatedAtMs:Date.now(),
      };
      await this.db.updateEntry(entry.id,{profitGuardState:{version:INFINITY_BREAK.version,policyRevision:INFINITY_BREAK.policyRevision,phase:'INFINITY_BREAK_EXIT_REVALIDATED',targetNetCents:effectiveDecision.infinityBreakTargetNetCents,executableNetCents:effectiveDecision.infinityBreakExecutableNetCents,executableBidCents:effectiveDecision.infinityBreakExecutableBidCents,lowestConsumedCents:effectiveDecision.infinityBreakLowestConsumedCents,expectedExitFeeCents:effectiveDecision.infinityBreakExpectedExitFeeCents,entryFeeCents:effectiveDecision.infinityBreakEntryFeeCents,bookMs:effectiveDecision.infinityBreakBookMs,revalidatedAtMs:effectiveDecision.infinityBreakRevalidatedAtMs,updatedAtMs:Date.now()},updatedAtMs:Date.now()});
    }
    const submitted = { ...entry, status: 'exit_pending', exitClientOrderId: client, closeReason: decision.reason, profitGuardState:infinityBreakExit?{version:INFINITY_BREAK.version,policyRevision:INFINITY_BREAK.policyRevision,phase:'INFINITY_BREAK_EXIT_REVALIDATED',targetNetCents:effectiveDecision.infinityBreakTargetNetCents,executableNetCents:effectiveDecision.infinityBreakExecutableNetCents,executableBidCents:effectiveDecision.infinityBreakExecutableBidCents,lowestConsumedCents:effectiveDecision.infinityBreakLowestConsumedCents,expectedExitFeeCents:effectiveDecision.infinityBreakExpectedExitFeeCents,entryFeeCents:effectiveDecision.infinityBreakEntryFeeCents,bookMs:effectiveDecision.infinityBreakBookMs,revalidatedAtMs:effectiveDecision.infinityBreakRevalidatedAtMs,updatedAtMs:Date.now()}:entry.profitGuardState, updatedAtMs: Date.now() };
    // IB1-R2 uses the lowest level actually consumed by the qualifying full-book
    // sweep as the IOC limit. Other profit authorities preserve their own floors.
    const orderLimitCents = infinityBreakExit ? n(infinityRevalidation.lowestConsumedCents) : profitProtectedExit ? exitFloorCents : bid;
    const routedExchangeIndex=entry?.entryConfig?.brokerRouting?.exchangeIndex;
    const quoteExchangeIndex=q?.exchangeIndex;
    ...
    if (r.fillCount > 0) {
      ...
      const exitFee = Number.isFinite(Number(r.feePaidCents)) ? Number(r.feePaidCents) : (infinityBreakExit ? infinityBreakExpectedExitFee(entry,r.fillCount,fillPx,s) : n(s.simFeeCents) * r.fillCount);
      const applied = await this.applyExitFill(submitted, effectiveDecision, {
  ...
  async _protect(entry) {
    const s = this.getSettings();
    if(this.isInfinityBreakTrade(entry))this.registerInfinityBreakFastWake(entry,s);
    ...
    if (entry.status === 'exit_pending' && (entry.closeReason === 'atomic_thunder_cashout' || entry.closeReason === 'infinity_break') && !entry.exitClientOrderId && !finalMarket(q)) {
      ...
      const infinityAssessment=infinityBreakPending?await this.assessInfinityBreakExecution(entry,atQ,s,{ensureFresh:false}):null;
      ...
      const atExec = infinityBreakPending
        ? infinityAssessment?.exec||null
        : atBookFresh && atBid >= atFloor ? this.market.executableBid(entry.ticker, remainingCount(entry), atFloor) : null;
      const atNet = infinityBreakPending
        ? infinityAssessment?.executableNetCents??null
        : atExec?.full ? this.aggregateExecutableNetCents(entry, remainingCount(entry), n(atExec.avgCents), s) : null;
      const atWindowValid=infinityBreakPending
        ? Boolean(infinityAssessment?.qualifies)
        : Boolean(atExec?.full&&n(atExec.filled)+1e-9>=remainingCount(entry)&&n(atNet,-Infinity)+1e-9>=atTarget);
      if (!atWindowValid) {
        ...
        await this.audit(infinityBreakPending?'infinity_break_exit_released_after_window':'atomic_thunder_exit_released_after_window', {
          ...
          executableBidCents:infinityBreakPending?infinityAssessment?.executableBidCents??null:atExec?.avgCents??null,
          lowestConsumedCents:infinityBreakPending?infinityAssessment?.lowestConsumedCents??null:atExec?.lowestConsumedCents??null,
    ...
    if ((this.isInfinityBreakTrade(entry) || this.isAtomicThunderTrade(entry)) && this.atomicThunderPolicy(entry, s).enabled) {
      ...
      if (at?.triggered) {
        ...
        await this.audit(this.isInfinityBreakTrade(entry)?'infinity_break_committed':'atomic_thunder_harvest_committed', {
          ...
          lowestConsumedCents:Number.isFinite(Number(at.lowestConsumedCents))?n(at.lowestConsumedCents):null,
          expectedExitFeeCents:Number.isFinite(Number(at.expectedExitFeeCents))?n(at.expectedExitFeeCents):null,
          entryFeeCents:Number.isFinite(Number(at.entryFeeCents))?n(at.entryFeeCents):null,bookMs:n(at.bookMs),
  ...
  async protectTicker(ticker) {
    ...
    const entries = typeof this.db.openFleetHunterEntriesByTicker === 'function'
  ...
  async sweep() {
    try {
      ...
      const entries = typeof this.db.openFleetHunterEntries === 'function'
        ? await this.db.openFleetHunterEntries()
        : typeof this.db.openHunterEntries === 'function'
      ...
      this.infinityBreakFastWake = new Map();
      for(const e of entries)if(this.isInfinityBreakTrade(e))this.registerInfinityBreakFastWake(e,s);
      for (const e of entries) await this.protect(e);
      return entries.length;
    } catch (e) {
