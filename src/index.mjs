...
await new Promise((resolveListening, rejectListening) => {
  ...
  const onListening = () => { server.off('error', onError); resolveListening(); };
  const onError = (error) => { server.off('listening', onListening); rejectListening(error); };
...
async function bootLoop() {
  while (!stopping && !runtime.engine) {
    ...
    let candidate = null;
    try {
      const { SagittariusEngine } = await import('./engine.mjs');
