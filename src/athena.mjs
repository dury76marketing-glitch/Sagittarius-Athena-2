...
// ---------------------------------------------------------------------------
import { ATHENA_COMMANDER, ATHENA_SOUL, ARAYASHIKI, ATOMIC_THUNDER_BOLT, ATOMIC_THUNDER_PATTERN_GUARDIAN, INFINITY_BREAK, SCARLET_NEEDLE, LIGHTNING_PLASMA, ATHENA_EXCLAMATION, EXECUTION_ATTACK_DISPLAY, PORTFOLIO_CONCEPTS } from './doctrine.mjs';
...
function economicsFor({entry=null,profitEpisode=null,outcome=null,fireCommand=null}={}){
  ...
  const frozenTarget=Math.max(0,commandFinite(entry?.entryConfig?.infinityBreak?.minimumNetPerOriginalContractCents,commandFinite(fireCommand?.economicTarget?.netPerOriginalContractCents,0)));
...
function addProfileObservation(profiles,key,{label,pnlCents=0,maeCents=null,origin='legacy',economics=null}={}){
  ...
  if(economics){
    ...
    for(const threshold of ECONOMIC_TARGET_THRESHOLDS){
      ...
      else if(['infinity_break','atomic_thunder_cashout'].includes(reason)&&frozenTarget>0&&threshold<=frozenTarget+1e-9){known=true;hit=true;}
...
function economicLegacyLabel(e){
  ...
  const frozenTarget=commandFinite(e?.entryConfig?.infinityBreak?.minimumNetPerOriginalContractCents,e?.entryConfig?.atomicThunder?.minimumNetPerOriginalContractCents??5);
...
function featureFit(concept,f={},context={}){
  ...
  if(concept==='Lightning Plasma'){const independent=commandFinite(context?.fieldContext?.independentEventCount,cosmos);return commandClamp(45+Math.min(26,independent*8)+Math.min(12,Math.max(0,v15)*12)+Math.min(8,up*2)-Math.min(12,lower*3));}
  if(concept==='Athena Exclamation'){const saints=commandFinite(context?.fieldContext?.goldSaintCount,0);if(saints<3)return 0;return commandClamp(45+Math.min(24,(saints-2)*8)+Math.min(10,Math.max(0,v15)*10)+Math.min(6,up*1.5)-Math.min(12,lower*2.5));}
  return 0;
}
...
export function rankAthenaAttacks(bolt,settings={},memory=null,context={}){
  ...
  for(const concept of COMMAND_ATTACKS){
    ...
    if(concept==='Lightning Plasma'){
      const field=context?.fieldContext||{},independent=commandFinite(field.independentEventCount,0),minCosmos=Math.max(2,commandFinite(field.minCosmos,LIGHTNING_PLASMA.minCosmos)),minStrikes=Math.max(2,commandFinite(field.minStrikes,LIGHTNING_PLASMA.minStrikes));
      if(field.lightningPlasmaQualified!==true||field.currentTickerEligible!==true||independent<minCosmos||independent<minStrikes)continue;
    }
    if(concept==='Athena Exclamation'){
      const candidate=context?.fieldContext?.athenaExclamationCandidate||null;
      if(!candidate||commandFinite(candidate.saintCount,0)<ATHENA_EXCLAMATION.minimumSaints)continue;
    }
...
const SOUL_OPENERS=new Set(ATHENA_SOUL.openers);
const SOUL_WIN=new Set(ATHENA_SOUL.winSaints);
const SOUL_WOUND=new Set(ATHENA_SOUL.woundSaints);
function soulEntry(v){const n=Number(v);return Number.isFinite(n)?n:0;}
function soulRoom(concept,entryCents){
  const e=soulEntry(entryCents),c=String(concept||'');
  if(e<=ATHENA_SOUL.graveMaxCents)return{ok:false,room:'grave',reason:'soul_grave'};
  if(c==='Momentum Hunter'){
    const ok=(e>=56&&e<=69)||(e>=80&&e<=89);
    return{ok,room:e>=80?'crown':'spine',reason:ok?'ok':'soul_wrong_room'};
  }
  if(c==='Wave Surfer'||c==='Justice Arrow'){
    const ok=e>=80&&e<=89;
    return{ok,room:'crown',reason:ok?'ok':'soul_wrong_room'};
  }
  if(c==='Scarlet Needle'){
    const ok=e>=50&&e<=99;
    return{ok,room:'handoff',reason:ok?'ok':'soul_wrong_room'};
  }
  if(c==='Crash Recovery Hunter'||c==='Lightning Plasma'||c==='Crystal Wall'){
    const ok=e>=56&&e<=69;
    return{ok,room:'spine',reason:ok?'ok':'soul_wrong_room'};
  }
  return{ok:false,room:'unknown',reason:'soul_unknown_saint'};
}
function soulScript(concept,source=''){
  const c=String(concept||'');
  if(c==='Momentum Hunter'||c==='Wave Surfer'){
    return{openingSaint:c,winSaint:'Scarlet Needle',woundSaint:'Crystal Wall'};
  }
  if(c==='Crash Recovery Hunter')return{openingSaint:c,winSaint:null,woundSaint:'Lightning Plasma'};
  return{openingSaint:c,winSaint:null,woundSaint:null};
}

export class AthenaSoul{
  constructor(){
    this.version=ATHENA_SOUL.version;
    this.stories=new Map();
    this.ledger={opened:0,checkmate:0,abort:0,wound:0,realizedCents:0};
  }
  snapshot(){
    const open=[...this.stories.values()].filter((s)=>s.status==='OPENING'||s.status==='CHAPTER2');
    const color=this.ledger.realizedCents>=0?'GREEN':'RED';
    return{version:ATHENA_SOUL.version,policyRevision:ATHENA_SOUL.policyRevision,shieldIsDisplayOnly:true,color,openStories:open.length,ledger:{...this.ledger},stories:open.map((s)=>({storyId:s.storyId,ticker:s.ticker,openingSaint:s.openingSaint,winSaint:s.winSaint,woundSaint:s.woundSaint,status:s.status}))};
  }
  admitChapter1({ticker,concept,entryCents,boltId,source=''}={}){
    const exact=String(ticker||'');
    if(!exact)return{ok:false,reason:'soul_ticker_required'};
    if(this.stories.has(exact))return{ok:false,reason:'soul_same_board'};
    if(!SOUL_OPENERS.has(String(concept||'')))return{ok:false,reason:'soul_not_opener'};
    const room=soulRoom(concept,entryCents);
    if(!room.ok)return{ok:false,reason:room.reason,room:room.room};
    const script=soulScript(concept,source);
    const story={storyId:`${exact}:${boltId||Date.now()}`,ticker:exact,boltId:boltId||null,entryCents:soulEntry(entryCents),status:'OPENING',...script};
    this.stories.set(exact,story);
    this.ledger.opened+=1;
    return{ok:true,reason:'soul_chapter1_sealed',story};
  }
  admitChapter2({ticker,concept,printCents}={}){
    const story=this.stories.get(String(ticker||''));
    if(!story)return{ok:false,reason:'soul_chapter2_no_parent'};
    const c=String(concept||'');
    const room=soulRoom(c,printCents);
    if(!room.ok)return{ok:false,reason:room.reason};
    if(SOUL_WIN.has(c)){
      if(story.winSaint!==c)return{ok:false,reason:'soul_script_mismatch'};
      if(story.status==='CHAPTER2'&&story.chapter2Saint)return{ok:false,reason:'soul_second_win_saint'};
      story.status='CHAPTER2';story.chapter2Saint=c;
      return{ok:true,reason:'soul_chapter2_win',story};
    }
    if(SOUL_WOUND.has(c)){
      if(story.woundSaint!==c)return{ok:false,reason:'soul_script_mismatch'};
      if(story.chapter2Saint)return{ok:false,reason:'soul_second_reserve'};
      const print=soulEntry(printCents);
      if(print<=ATHENA_SOUL.graveMaxCents)return{ok:false,reason:'soul_grave'};
      story.status='CHAPTER2';story.chapter2Saint=c;
      return{ok:true,reason:'soul_chapter2_wound',story};
    }
    return{ok:false,reason:'soul_script_mismatch'};
  }
  closeChapter({ticker,reason='',printCents=0,pnlCents=0}={}){
    const exact=String(ticker||'');
    const story=this.stories.get(exact);
    if(!story)return{ok:false,reason:'soul_no_story'};
    const print=soulEntry(printCents);
    const pnl=soulEntry(pnlCents);
    this.ledger.realizedCents+=pnl;
    if(print<=ATHENA_SOUL.graveMaxCents||reason==='grave'||reason==='clock'){
      story.status='ABORT';this.ledger.abort+=1;this.stories.delete(exact);
      return{ok:true,reason:'soul_abort',story};
    }
    if(reason==='hard_stop_loss'||reason==='another_dimension_aurora'){
      this.ledger.wound+=1;
      return{ok:true,reason:'soul_wound_alive',story,woundSaint:story.woundSaint};
    }
    story.status='CHECKMATE';this.ledger.checkmate+=1;this.stories.delete(exact);
    return{ok:true,reason:'soul_checkmate',story};
  }
  serialize(){
    return{version:this.version,ledger:{...this.ledger},stories:[...this.stories.values()]};
  }
  hydrate(raw){
    if(!raw||typeof raw!=='object')return this;
    this.ledger={opened:Number(raw.ledger?.opened||0),checkmate:Number(raw.ledger?.checkmate||0),abort:Number(raw.ledger?.abort||0),wound:Number(raw.ledger?.wound||0),realizedCents:Number(raw.ledger?.realizedCents||0)};
    this.stories=new Map();
    for(const story of Array.isArray(raw.stories)?raw.stories:[]){
      if(story?.ticker)this.stories.set(String(story.ticker),story);
    }
    return this;
  }
}
...
export class AthenaCommander{
  constructor({db,systemName='SAGITTARIUS',sourceRelease='',getSettings=()=>({}),audit=async()=>{},legacyAthena=null}={}){
    this.db=db;this.systemName=systemName;this.sourceRelease=sourceRelease;this.getSettings=getSettings;this.audit=audit;this.legacyAthena=legacyAthena;this.memory=compileAthenaAttackMemory();this.loadedAtMs=0;this.decisions=0;this.fire=0;this.watch=0;this.reject=0;this.expired=0;this.survivalCertified=0;this.survivalRejected=0;this.lastSurvivalCertificate=null;this.lastDecision=null;this.trainedEpisodeIds=new Set();this.knownEntryIds=new Set();this.knownProfitEpisodeIds=new Set();this.memoryRefreshPromise=null;this.memoryMutationVersion=0;this.memoryLoad={state:'IDLE',startedAtMs:0,completedAtMs:0,lastError:null,attempts:0,discardedStaleLoads:0};this.soul=new AthenaSoul();
  }
  async persistSoul(){
    if(typeof this.db?.saveSoulLedger!=='function')return false;
    await this.db.saveSoulLedger(this.soul.serialize());
    return true;
  }
  async restoreSoul(){
    if(typeof this.db?.loadSoulLedger!=='function')return false;
    const raw=await this.db.loadSoulLedger();
    if(raw)this.soul.hydrate(raw);
    return true;
  }
  ...
  async decide(bolt,context={}){
    ...
    if(!bolt||!bolt.id){reason='bolt_required';this.reject+=1;}
      ...
      if(immediate){
        // Soul is display/script only. Operator bands, clock, book, capital and
        // hard execution safety remain authoritative. A Soul room/script miss
        // must never convert a ranked Cosmo-GREEN attack into REJECT.
        decision='FIRE';reason='cosmo_green_best_attack';this.fire+=1;
        if(settings.athenaSoulEnabled===true){
          const entry=commandFinite(bolt.features?.askCents,0)||commandFinite(bolt.greenTrigger?.currentExecutableBidCents,0);
          const admitted=this.soul.admitChapter1({ticker:bolt.ticker,concept:immediate.concept,entryCents:entry,boltId:bolt.id,source:bolt.greenTrigger?.cosmo||''});
          if(admitted.ok){
            context=Object.assign({},context,{soulStory:admitted.story,soulAdmission:admitted});
            this.persistSoul().catch(()=>{});
          }else{
            context=Object.assign({},context,{soulStory:null,soulAdmission:admitted});
          }
        }
      }
      else this.reject+=1;
    ...
    let persisted=false;
    if(typeof this.db?.upsertOpportunityEpisode==='function'&&bolt?.id){try{await this.db.upsertOpportunityEpisode({id:bolt.id,systemName:String(settings.systemName||this.systemName),sourceRelease:this.sourceRelease,cohortId:String(settings.resetTimestampMs||''),ticker:bolt.ticker,eventTicker:bolt.eventTicker,side:'YES',sport:bolt.sport||bolt.features?.sport||'Unknown',boltAtMs:bolt.detectedAtMs,boltSnapshot:bolt,athenaDecision:result,fireCommand:fireCommand||{},attackSelected:best?.concept||null,updatedAtMs:now});persisted=true;}catch(error){await this.audit('athena_a3_decision_persistence_failed',{boltId:bolt.id,ticker:bolt.ticker,decision,message:String(error?.message||error)},'error').catch(()=>{});}}
