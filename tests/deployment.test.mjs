...
test('deployment restores the proven Railway source-build contract without Dockerfile dependence',async()=>{const pkg=JSON.parse(await readFile(resolve(root,'package.json'),'utf8'));assert.equal(pkg.scripts.start,'node src/index.mjs');assert.equal(pkg.scripts.start.includes('dist/'),false);const railway=JSON.parse(await readFile(resolve(root,'railway.json'),'utf8'));assert.equal(railway.build.builder,'RAILPACK');assert.equal(railway.build.buildCommand,'npm test && npm run check');assert.equal(railway.deploy.startCommand,'npm start');assert.equal(railway.deploy.healthcheckPath,'/health');assert.equal(railway.deploy.healthcheckTimeout,180);await assert.rejects(readFile(resolve(root,'Dockerfile'),'utf8'),/ENOENT/);await assert.rejects(readFile(resolve(root,'.npmrc'),'utf8'),/ENOENT/);});
const R56_PROJECT_SOURCE_FILE_BUDGET=57; // TC1-P1 adds src/constellation.mjs + tests/constellation.test.mjs. Generated dependency files remain excluded.
test("R56 project stays within the explicit compact authored source-file budget even when npm generates package-lock.json",async()=>{const {RELEASE,originalSettings,CANONICAL_NUMERIC_SETTINGS,CANONICAL_BOOLEAN_SETTINGS,sanitizeRuntimeSettings,normalizeStartupExecutionMode}=await import('../src/config.mjs');const D=await import('../src/doctrine.mjs');const {readFile,readdir,stat}=await import('node:fs/promises');assert.equal(RELEASE,'SAGITTARIUS-SE1-R2-STOP-LOSS-NOTIFY-STARLIGHT-2026-09-23');const root=new URL('../',import.meta.url);async function walk(u){let out=[];for(const ent of await readdir(u,{withFileTypes:true})){if(ent.name==='node_modules'||ent.name==='.git'||ent.name==='package-lock.json')continue;const x=new URL(ent.name+(ent.isDirectory()?'/':''),u);if(ent.isDirectory())out.push(...await walk(x));else out.push(x);}return out;}const all=await walk(root);assert.equal(all.length,R56_PROJECT_SOURCE_FILE_BUDGET);assert.ok(all.some(x=>x.pathname.endsWith('/src/opportunity.mjs')));assert.ok(all.some(x=>x.pathname.endsWith('/src/authority.mjs')));assert.ok(all.some(x=>x.pathname.endsWith('/tests/legacy/r51.legacy.mjs')));});
...
test("R45 dashboard contains the four-layer Mega Wave architecture plus operational research sections",async()=>{const {RELEASE,originalSettings,CANONICAL_NUMERIC_SETTINGS,CANONICAL_BOOLEAN_SETTINGS,sanitizeRuntimeSettings,normalizeStartupExecutionMode}=await import('../src/config.mjs');const D=await import('../src/doctrine.mjs');const {readFile,readdir,stat}=await import('node:fs/promises');const html=await readFile(new URL('../public/index.html',import.meta.url),'utf8');const app=await readFile(new URL('../public/app.js',import.meta.url),'utf8');assert.ok(html.includes('ATOMIC THUNDER BOLT'));assert.ok(html.includes('ATHENA'));assert.ok(html.includes('INFINITY BREAK'));assert.ok(html.includes('AURORA EXECUTION'));assert.ok(html.includes('COSMO UNIVERSE'));assert.ok(app.includes('auroraDamageControlPercent'));assert.ok(app.includes('lightningPlasmaMaxStrikes'));for(const required of ['momentumMinCrashCents','waveMinCrashCents','crashRecoveryMinCrashCents','crashRecoveryMinUpwardTicks'])assert.equal(app.includes(required),true,required);assert.equal(app.includes('waveMinFeederFavorableMoveCents'),false);assert.equal(app.includes('momentumMinRiseCents'),false);assert.equal(app.includes('recoveryMinReboundCents'),false,'Crystal Wall keeps its dedicated crystalWallMinReboundCents control');});
...
test('HTTP listener is truly live before trading-engine module load and boot retries are retained',async()=>{const src=await readFile(resolve(root,'src/index.mjs'),'utf8');assert.equal(src.includes("import { SagittariusEngine } from './engine.mjs'"),false,'trading engine must not be a static pre-listener import');const listenAt=src.indexOf('startServer(runtime');const listeningAt=src.indexOf("server.once('listening'");const dynamicImportAt=src.indexOf("await import('./engine.mjs')");const engineAt=src.indexOf('new SagittariusEngine()');assert.ok(listenAt>=0&&listeningAt>listenAt&&dynamicImportAt>listeningAt&&engineAt>dynamicImportAt,'HTTP listener must be confirmed live before engine module load/initialization');assert.ok(src.includes('while (!stopping && !runtime.engine)'));assert.ok(src.includes('await sleep(5000)'));});
...
test('R54 RGM3 pressure defers heavyweight historical exports without invoking DB-backed collectors',async()=>{
  ...
  const engine={
    ...
    resourceUsageSnapshot:()=>({version:'RGM5',pressureState:'HARD_RESEARCH_SHED',researchDeferred:true,workload:{version:'DBPI2'}}),
  ...
  assert.match(log,/RGM5 is protecting trading memory/);
});
...
test('R61 startup has no Scarlet retracement-arm hydration path or incomplete-history dependency',async()=>{
  ...
  assert.equal(athena.summary().scarletNeedle.managedBy,'ENGINE_THIRD_CRYSTAL_WALL_PROOF_HANDOFF');
...
test('Mega Wave dashboard exposes Triple Crystal to Athena and 0-12 multi-Saint release controls with fresh assets',async()=>{
  ...
  for(const key of ['athenaExclamationFollowUpAttacks','scarletNeedleEnabled','scarletNeedleStakeCents','scarletNeedleMinEntryCents','scarletNeedleMaxEntryCents','recoveryPri1R2TrailCents','maxRepeatsPerMarket','repeatCooldownMinutes'])assert.ok(app.includes(key),key);
  ...
  assert.ok(html.includes('Atomic Thunder is the only timing signal')||html.includes('BOLT DIRECT'));
  assert.ok(html.includes('Galactic Explosion ON lets other enabled attacks join')||html.includes('other enabled attacks may join'));
  assert.ok(engine.includes("version:'MEGA-WAVE-EPC1'"));
  assert.ok(engine.includes("scarletNeedleContinuationAuthority:false"));
  assert.ok(engine.includes('scarletNeedleMegaWaveDownstreamAuthority:this.settings.scarletNeedleEnabled===true'));
  assert.ok(engine.includes("noBoltNoAttackExceptions:[MEGA_WAVE.entryAuthority,MEGA_WAVE.downstreamAuthority,'CRYSTAL_WALL_V3_SHADOW_AUTHORITY']"));
  assert.ok(engine.includes('enabledMegaWaveSaints'));
  assert.ok(html.includes('/styles.css?v=MEGAWAVE-RWY-HF1'));
  assert.ok(html.includes('/app.js?v=MEGAWAVE-RWY-HF1'));
  ...
  try{
    for(const path of ['/','/app.js?v=MEGAWAVE-RWY-HF1','/styles.css?v=MEGAWAVE-RWY-HF1']){
    ...
    const js=await (await fetch(`http://127.0.0.1:${port}/app.js?v=MEGAWAVE-RWY-HF1`)).text();assert.ok(js.includes("name:'Scarlet Needle'"));assert.ok(js.includes('athenaExclamationFollowUpAttacks'));
...
test('R55 Phoenix is event-driven, reference-only, TTL-bounded and cannot contaminate frozen Athena B2-R2 context',async()=>{
  ...
  assert.ok(config.includes("SAGITTARIUS-SE1-R2-STOP-LOSS-NOTIFY-STARLIGHT-2026-09-23"));
...
test('R60 Athena receives COSMO_GREEN directly; Arayashiki and ATB2 remain non-authority research only',async()=>{
  ...
  assert.ok(engine.includes("eventClockTradingAuthority: false"));
  assert.ok(engine.includes("ECA1-RETIRED-TELEMETRY-ONLY"));
...
test('PNL1 restores Madrid day/week/month/year realized P&L metrics from durable closed Execution Attacks',async()=>{
  const {Database}=await import('../src/db.mjs');
  let capturedSql='';let capturedParams=null;
  const fake={pool:{query:async(sql,params)=>{capturedSql=sql;capturedParams=params;return{rows:[{closed_hunters:4,open_hunters:1,wins:3,losses:1,scratches:0,closed_realized_cents:1250,day_realized_cents:125,week_realized_cents:350,month_realized_cents:800,year_realized_cents:1250,partial_realized_cents:0,simulation_ledger_pnl_cents:0}]};}}};
  const out=await Database.prototype.performanceAggregate.call(fake,'SAGITTARIUS',{resetTimestampMs:123});
  assert.equal(out.day_realized_cents,125);
  assert.equal(out.week_realized_cents,350);
  assert.equal(out.month_realized_cents,800);
  assert.equal(out.year_realized_cents,1250);
  assert.equal(capturedParams[0],'SAGITTARIUS');
  assert.equal(capturedParams[2],123);
  assert.ok(capturedSql.includes("date_trunc('day',now() at time zone 'Europe/Madrid')"));
  assert.ok(capturedSql.includes("date_trunc('week',now() at time zone 'Europe/Madrid')"));
  assert.ok(capturedSql.includes("greatest($3::bigint,bounds.day_ms)"),'dashboard reset must remain authoritative over period history');
  assert.ok(capturedSql.includes("concept_name = any($2::text[]) and status='closed'"),'period P/L must remain scoped to closed Execution Attacks');

  const engineSrc=await readFile(resolve(root,'src/engine.mjs'),'utf8');
  const html=await readFile(resolve(root,'public/index.html'),'utf8');
  const app=await readFile(resolve(root,'public/app.js'),'utf8');
  for(const field of ['dayRealizedCents','weekRealizedCents','monthRealizedCents','yearRealizedCents'])assert.ok(engineSrc.includes(field),`${field} missing from state/performance path`);
  for(const [id,label] of [['profitToday','Profit Today'],['profitWeek','Profit This Week'],['profitMonth','Profit This Month'],['profitYear','Profit This Year']]){assert.ok(html.includes(`id="${id}"`),`${id} dashboard metric missing`);assert.ok(html.includes(label),`${label} label missing`);assert.ok(app.includes(`setPnl('${id}'`),`${id} renderer missing`);}
});

test('PNL1 engine operational performance carries database period aggregates unchanged into dashboard-ready camelCase fields',async()=>{
  const {SagittariusEngine}=await import('../src/engine.mjs');
  const engine=Object.create(SagittariusEngine.prototype);
  engine.settings={systemName:'SAGITTARIUS',resetTimestampMs:0,startingCapitalCents:1_000_000,simFeeCents:2};
  engine.db={
    performanceAggregate:async()=>({closed_realized_cents:1250,partial_realized_cents:0,day_realized_cents:125,week_realized_cents:350,month_realized_cents:800,year_realized_cents:1250,simulation_ledger_pnl_cents:0,wins:3,losses:1,scratches:0,closed_hunters:4,open_hunters:0}),
    openEntries:async()=>[],recentClosedHunters:async()=>[],conceptStatsAggregate:async()=>({portfolio:[],signals:[],linked:[]}),
  };
  const p=await engine.performance();
  assert.equal(p.dayRealizedCents,125);
  assert.equal(p.weekRealizedCents,350);
  assert.equal(p.monthRealizedCents,800);
  assert.equal(p.yearRealizedCents,1250);
  assert.equal(p.hunterRealizedCents,1250);
  assert.equal(p.portfolioValueCents,1_001_250);
});
