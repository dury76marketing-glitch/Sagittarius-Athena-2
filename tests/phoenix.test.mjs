...
test('R63-MHF1 genuine SID gap invalidates the entire stream and recovers by bounded get_snapshot without reconnect',()=>{
  ...
  assert.deepEqual(sent[0].params.sids,[9]);assert.equal(Object.hasOwn(sent[0].params,'sid'),false);
...
test('R63-MHF1-HF3 REST fallback buffers WS deltas and replays a delayed recovery snapshot without an SID mismatch storm',()=>{
  const hub=hubForMarketTruth();seedTruth(hub,'T');hub.setWanted(['T']);
  const sent=[];hub.ws={readyState:1,send:x=>sent.push(JSON.parse(x)),close:()=>{}};hub.connected=true;
  hub.handle(snap('T',21,1));
  hub.books.set('T',{ticker:'T',yesBids:[{priceCents:79,count:10}],noBids:[{priceCents:20,count:10}],updatedAtMs:1_100,source:'REST',sid:null,seq:null,sequenceValid:true,invalidReason:null});hub.applyBook('T');
  for(let seq=2;seq<=101;seq++)hub.handle(delta('T',21,seq,'yes','0.79',1,1_000+seq));
  let integrity=hub.resourceSnapshot().bookIntegrity;
  assert.equal(integrity.sidMismatch,0,'REST-to-WS handoff is not an SID mismatch');
  assert.equal(integrity.pendingSnapshots,1);assert.equal(sent.length,1);assert.equal(sent[0].params.action,'get_snapshot');
  assert.equal(hub.getQuote('T').bookInvalid,true);
  hub.handle(snap('T',21,50,2_000));
  integrity=hub.resourceSnapshot().bookIntegrity;
  assert.equal(integrity.pendingSnapshots,0);assert.equal(integrity.recoveringStreams,0);assert.equal(integrity.sidMismatch,0);
  assert.equal(hub.getQuote('T').bookInvalid,false);assert.equal(hub.getBook('T').sequenceValid,true);assert.equal(hub.getBook('T').seq,101);
  assert.ok(integrity.recoveryReplayedDeltas>=51);
});

test('R63-MHF1-HF4 a ticker entering recovery behind an in-flight snapshot gets its own snapshot request instead of starving',()=>{
  const hub=hubForMarketTruth();for(const t of ['A','B'])seedTruth(hub,t);hub.setWanted(['A','B']);
  const sent=[];hub.ws={readyState:1,send:x=>sent.push(JSON.parse(x)),close:()=>{}};hub.connected=true;
  hub.handle(snap('A',31,1));hub.handle(snap('B',31,2));
  hub.books.set('A',{ticker:'A',yesBids:[{priceCents:79,count:10}],noBids:[{priceCents:20,count:10}],updatedAtMs:1_100,source:'REST',sid:null,seq:null,sequenceValid:true,invalidReason:null});hub.applyBook('A');
  hub.handle(delta('A',31,3));
  assert.equal(sent.length,1);assert.deepEqual(sent[0].params.market_tickers,['A']);assert.deepEqual(sent[0].params.sids,[31]);
  hub.books.set('B',{ticker:'B',yesBids:[{priceCents:79,count:10}],noBids:[{priceCents:20,count:10}],updatedAtMs:1_200,source:'REST',sid:null,seq:null,sequenceValid:true,invalidReason:null});hub.applyBook('B');
  hub.handle(delta('B',31,4));
  assert.equal(sent.length,2,'B must not wait for A snapshot completion');assert.deepEqual(sent[1].params.market_tickers,['B']);assert.deepEqual(sent[1].params.sids,[31]);
  let integrity=hub.resourceSnapshot().bookIntegrity;assert.equal(integrity.pendingSnapshots,2);assert.equal(integrity.sidMismatch,0);
  hub.handle(snap('A',31,5));hub.handle(snap('B',31,6));
  integrity=hub.resourceSnapshot().bookIntegrity;assert.equal(integrity.pendingSnapshots,0);assert.equal(integrity.recoveringStreams,0);assert.equal(integrity.sidMismatch,0);
});

test('R63-MHF1-HF4 get_snapshot errors release the affected ticker and retry with canonical sids syntax',()=>{
  const hub=hubForMarketTruth();seedTruth(hub,'A');hub.setWanted(['A']);
  const sent=[];hub.ws={readyState:1,send:x=>sent.push(JSON.parse(x)),close:()=>{}};hub.connected=true;
  hub.handle(snap('A',41,1));
  hub.books.set('A',{ticker:'A',yesBids:[{priceCents:79,count:10}],noBids:[{priceCents:20,count:10}],updatedAtMs:1_100,source:'REST',sid:null,seq:null,sequenceValid:true,invalidReason:null});hub.applyBook('A');
  hub.handle(delta('A',41,2));assert.equal(sent.length,1);
  const first=sent[0];hub.handle({id:first.id,type:'error',msg:{code:11,msg:'Invalid parameter'}});
  assert.equal(sent.length,2,'recovery command failure must not strand the ticker');assert.notEqual(sent[1].id,first.id);assert.deepEqual(sent[1].params.sids,[41]);
  const integrity=hub.resourceSnapshot().bookIntegrity;assert.equal(integrity.pendingSnapshots,1);assert.equal(integrity.recoverySnapshotErrors,1);
});

test('R63-MHF1-HF4 stalled recovery snapshots time out and retry instead of buffering forever',()=>{
  const hub=hubForMarketTruth();seedTruth(hub,'A');hub.setWanted(['A']);
  const sent=[];hub.ws={readyState:1,send:x=>sent.push(JSON.parse(x)),close:()=>{}};hub.connected=true;
  hub.handle(snap('A',51,1));
  hub.books.set('A',{ticker:'A',yesBids:[{priceCents:79,count:10}],noBids:[{priceCents:20,count:10}],updatedAtMs:1_100,source:'REST',sid:null,seq:null,sequenceValid:true,invalidReason:null});hub.applyBook('A');
  hub.handle(delta('A',51,2));assert.equal(sent.length,1);
  const state=hub.streamState(51);const requestedAt=state.snapshotInFlight.get('A').requestedAtMs;
  hub.maintainRecovery(requestedAt+5_001);
  assert.equal(sent.length,2,'timeout must produce a fresh snapshot request');assert.equal(hub.resourceSnapshot().bookIntegrity.recoverySnapshotTimeouts,1);
});

test('R63-MHF1-HF4 repeated snapshot command failures fail closed into one clean websocket reconnect',()=>{
  const hub=hubForMarketTruth();seedTruth(hub,'A');hub.setWanted(['A']);
  const sent=[];let closes=0;hub.ws={readyState:1,send:x=>sent.push(JSON.parse(x)),close:()=>{closes+=1;}};hub.connected=true;
  hub.handle(snap('A',61,1));
  hub.books.set('A',{ticker:'A',yesBids:[{priceCents:79,count:10}],noBids:[{priceCents:20,count:10}],updatedAtMs:1_100,source:'REST',sid:null,seq:null,sequenceValid:true,invalidReason:null});hub.applyBook('A');
  hub.handle(delta('A',61,2));
  for(let i=0;i<3;i++){
    const cmd=sent.at(-1);hub.handle({id:cmd.id,type:'error',msg:{code:18,msg:'Command timeout'}});
  }
  const integrity=hub.resourceSnapshot().bookIntegrity;
  assert.equal(closes,1);assert.equal(integrity.recoveryReconnects,1);assert.equal(integrity.recoverySnapshotErrors,3);assert.equal(hub.getQuote('A').bookInvalid,true);
});
