...
test('CW3 migration removes old double-stake aliases and installs the fixed recovery profile',async()=>{const C=await import('../src/config.mjs');const old={hunterStakeCents:21000,stakeCents:22000,feederStakeCents:3000,recoveryBaseStakeCents:10000,sagittariusEnabled:true,waveStopLossCents:35};const s=C.sanitizeRuntimeSettings(old);assert.equal(s.momentumStakeCents,21000);assert.equal(s.waveStakeCents,22000);assert.equal(s.pegasusReferenceStakeCents,3000);assert.equal(s.recoveryStakeCents,500);assert.equal(s.recoveryMinEntryCents,10);assert.equal(s.recoveryMaxEntryCents,50);assert.equal(s.crystalWallMinCrashCents,15);assert.equal(s.crystalWallMinReboundCents,5);assert.equal(s.crystalWallMinUpwardTicks,2);assert.equal(Object.hasOwn(s,'recoveryBaseStakeCents'),false);assert.equal(Object.hasOwn(s,'sagittariusEnabled'),false);assert.equal(Object.hasOwn(s,'waveStopLossCents'),false);});
