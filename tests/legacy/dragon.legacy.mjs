...
test('Dragon materializes one reference-only ghost per exact CI1 episode and never places a broker order',async()=>{
  ...
  assert.equal(e.conceptName,'Dragon');assert.equal(e.entryPriceCents,86,'Dragon shadow entry must use the executable signal ask, not the historical trough');assert.equal(e.entryConfig.dragonSource.troughCents,70);assert.equal(e.entryOrderId??null,null);assert.equal(e.stopLossCents,0);
...
test('CW3 Cosmo routing keeps Crystal Wall out of ordinary Cosmo selection and declares its independent authority',()=>{
  ...
  assert.equal(COSMO_ROUTING.defaultFutureInitialEntryConsumer,true);assert.equal(COSMO_ROUTING.sourceDoesNotAuthorizeEntry,true);
});
