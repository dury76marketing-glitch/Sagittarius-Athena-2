...
test('LIVE restart continuity remains disarmed until hard readiness is clean, then restores LIVE arm and requests an immediate scan',async()=>{
  const engine=Object.create(SagittariusEngine.prototype);
  engine.running=true;engine.startupLiveResumeRequested=true;engine.liveAutoRearmTimer=null;engine.liveAutoRearmPromise=null;engine.liveAutoRearmLastReason=null;engine.scanRequested=false;
  engine.settings=settings({mode:'LIVE',liveArmed:false});
  engine.health={restOk:true,websocketFresh:true,reconciliationOk:true,protectionOk:true,protectionFresh:true,goldenEyeOk:true,scannerFresh:false,degraded:true};
  engine.db={audits:[],async audit(level,event,data){this.audits.push({level,event,data});}};
  engine.deploymentAllowsLiveTrading=()=>true;engine.testConnection=async()=>({ok:true});engine.reconcileBroker=async()=>true;engine.runProtectionSweep=async()=>true;
  engine.recomputeHealth=()=>{engine.health.degraded=!(engine.health.restOk&&engine.health.websocketFresh&&engine.health.reconciliationOk&&engine.health.protectionOk&&engine.health.protectionFresh&&engine.health.goldenEyeOk&&engine.health.scannerFresh);};
  engine.invalidateStateSnapshot=()=>{};engine.requestScan=()=>{engine.scanRequested=true;};
  assert.equal(await engine.attemptLiveAutoRearm(),false);assert.equal(engine.settings.liveArmed,false);assert.equal(engine.startupLiveResumeRequested,true);assert.ok(engine.db.audits.some(x=>x.event==='live_restart_rearm_waiting'&&x.data.reason==='scanner_freshness'));
  engine.health.scannerFresh=true;
  assert.equal(await engine.attemptLiveAutoRearm(),true);assert.equal(engine.settings.liveArmed,true);assert.equal(engine.startupLiveResumeRequested,false);assert.equal(engine.scanRequested,true);assert.ok(engine.db.audits.some(x=>x.event==='live_restart_rearm_restored'));
});

test('LIVE restart continuity fails closed on broker reconciliation mismatch and never arms',async()=>{
  const engine=Object.create(SagittariusEngine.prototype);engine.running=true;engine.startupLiveResumeRequested=true;engine.liveAutoRearmTimer=null;engine.liveAutoRearmPromise=null;engine.liveAutoRearmLastReason=null;engine.settings=settings({mode:'LIVE',liveArmed:false});
  engine.health={restOk:true,websocketFresh:true,reconciliationOk:false,protectionOk:true,protectionFresh:true,goldenEyeOk:true,scannerFresh:true,degraded:true};engine.db={audits:[],async audit(level,event,data){this.audits.push({level,event,data});}};
  engine.deploymentAllowsLiveTrading=()=>true;engine.testConnection=async()=>({ok:true});engine.reconcileBroker=async()=>false;engine.runProtectionSweep=async()=>{throw new Error('must_not_reach_protection');};engine.recomputeHealth=()=>{};engine.invalidateStateSnapshot=()=>{};engine.requestScan=()=>{};
  assert.equal(await engine.attemptLiveAutoRearm(),false);assert.equal(engine.settings.liveArmed,false);assert.equal(engine.startupLiveResumeRequested,true);assert.ok(engine.db.audits.some(x=>x.event==='live_restart_rearm_waiting'&&x.data.reason==='broker_reconciliation_not_clean'));
});

test('operator switch to SIMULATION wins a race against restart auto-rearm',async()=>{
  const engine=Object.create(SagittariusEngine.prototype);engine.running=true;engine.startupLiveResumeRequested=true;engine.liveAutoRearmTimer=null;engine.liveAutoRearmPromise=null;engine.liveAutoRearmLastReason=null;engine.settings=settings({mode:'LIVE',liveArmed:false});
  engine.health={restOk:true,websocketFresh:true,reconciliationOk:true,protectionOk:true,protectionFresh:true,goldenEyeOk:true,scannerFresh:true,degraded:false};engine.db={audits:[],async audit(level,event,data){this.audits.push({level,event,data});}};
  engine.deploymentAllowsLiveTrading=()=>true;engine.testConnection=async()=>({ok:true});engine.reconcileBroker=async()=>true;engine.runProtectionSweep=async()=>{engine.settings={...engine.settings,mode:'SIMULATION',liveArmed:false};engine.startupLiveResumeRequested=false;};engine.recomputeHealth=()=>{engine.health.degraded=false;};engine.invalidateStateSnapshot=()=>{};engine.requestScan=()=>{};
  assert.equal(await engine.attemptLiveAutoRearm(),false);assert.equal(engine.settings.mode,'SIMULATION');assert.equal(engine.settings.liveArmed,false);
});
...
test('R59 RGM4 reports real process resources with a 500 MiB hard ceiling while trading authority remains untouched',async()=>{
  ...
  assert.equal(RUNTIME_RESOURCE_GOVERNOR.version,'RGM5');
...
test('R58 EAC2 preserves deterministic base clock waits before the entry queue',()=>{
  ...
  d=entryAdmissionDecision({quote:{eventTicker:'EV',gameClockState:{version:'GCA2',eventTicker:'EV',phase:'UNKNOWN',observedActivityStartMs:now-10*60_000}},mode:'SIMULATION',minGameMinutes:min,now});
  assert.equal(d.action,'BLOCK');assert.equal(d.reason,'observed_activity_aging_wait');
  d=entryAdmissionDecision({quote:{eventTicker:'EV',gameClockState:{version:'GCA2',eventTicker:'EV',phase:'UNKNOWN',observedActivityStartMs:now-80*60_000}},mode:'SIMULATION',minGameMinutes:min,maxGameMinutes:90,now});
  assert.equal(d.action,'PROBE');assert.equal(d.reason,'observed_activity_due_for_authority_probe');
  d=entryAdmissionDecision({quote:{eventTicker:'EV',gameClockState:{version:'GCA2',eventTicker:'EV',phase:'UNKNOWN',observedActivityStartMs:now-80*60_000}},mode:'LIVE',minGameMinutes:min,maxGameMinutes:90,now});
  assert.equal(d.action,'PROBE');assert.equal(d.reason,'observed_activity_due_for_authority_probe','LIVE and SIM must schedule the same matured observed-activity evidence');
  d=entryAdmissionDecision({quote:{eventTicker:'EV',gameClockState:{version:'GCA2',eventTicker:'EV',phase:'UNKNOWN',observedActivityStartMs:now-95*60_000}},mode:'SIMULATION',minGameMinutes:min,maxGameMinutes:90,now});
  assert.equal(d.action,'BLOCK');assert.equal(d.reason,'maximum_game_time_exceeded','a shared observed-activity lower bound already beyond the maximum is certainly too late');
...
test('HF7 EAC1 quote-flood admission keeps predictable waits out of execution workers and rate-limits matured shared authority probes',()=>{
  const now=Date.now();
  const e=Object.create(SagittariusEngine.prototype);
  ...
  q={ticker:'T',eventTicker:'EV',gameClockState:{version:'GCA2',eventTicker:'EV',phase:'UNKNOWN',observedActivityStartMs:now-80*60_000}};
...
test('HF5 state database fanout and Atomic Thunder composite reads are explicitly bounded',async()=>{
  ...
  assert.match(engineSource,/mapLimit\(jobs,STATE_DB_FANOUT_MAX/);
  assert.match(engineSource,/operatorTelemetrySnapshot/);
