...
import { originalSettings, sanitizeRuntimeSettings, applyJusticeArrowPostScarletV3Migration, EDITABLE_BOOLEAN_SETTINGS, CANONICAL_NUMERIC_SETTINGS, CANONICAL_BOOLEAN_SETTINGS, RELEASE } from '../src/config.mjs';
import {
...
import { validateAthenaFireCommand, anotherDimensionQualification, justiceArrowSignalState, StrategyEngine } from '../src/strategy.mjs';
...
function memoryDb(initial=[]){
  ...
  return {
    ...
    async entriesByConceptTicker(_systemName,conceptName,ticker,{limit=200}={}){return [...rows.values()].filter(x=>x.conceptName===conceptName&&String(x.ticker)===String(ticker)).sort((a,b)=>Number(b.openedAtMs||0)-Number(a.openedAtMs||0)).slice(0,limit).map(x=>structuredClone(x));},
...
test('R60 Railway source package remains compact, source-started, and healthcheck-safe',async()=>{
  ...
  async function walk(u){let out=[];for(const ent of await readdir(u,{withFileTypes:true})){if(['node_modules','.git','package-lock.json'].includes(ent.name))continue;const x=new URL(ent.name+(ent.isDirectory()?'/':''),u);if(ent.isDirectory())out.push(...await walk(x));else out.push(x);}return out;}const all=await walk(root);assert.equal(all.length,50);
});

// R63 Gemini / Another Dimension / Sagittarius Justice Arrow + live-parity SIM.
test('SJA3 doctrine connects Justice Arrow directly after profitable Scarlet and preserves its own confirmation plus ATHENA-X1/Aurora',()=>{
  assert.equal(SHADOW_ATTACK_CONCEPTS.has('Another Dimension'),true);assert.equal(PORTFOLIO_CONCEPTS.has('Sagittarius Justice Arrow'),true);
  assert.equal(GEMINI_UNIVERSE.attack,'Another Dimension');assert.equal(ANOTHER_DIMENSION.brokerOrderAuthority,false);
  assert.equal(SAGITTARIUS_JUSTICE_ARROW.version,'SAGITTARIUS-JUSTICE-ARROW-V3');assert.equal(SAGITTARIUS_JUSTICE_ARROW.policyRevision,'SJA3-R2-DIRECT-POST-SCARLET-OWN-CONFIRMATION-ATHENA-X1');
  assert.equal(SAGITTARIUS_JUSTICE_ARROW.trigger,'POST_PROFITABLE_SCARLET_NEEDLE_CLOSE_DIRECT_OWN_CONFIRMATION');assert.deepEqual([...SAGITTARIUS_JUSTICE_ARROW.triggerSources],['SCARLET_NEEDLE_PROFIT_CLOSE']);
  assert.equal(SAGITTARIUS_JUSTICE_ARROW.independentCrashAuthority,false);assert.equal(SAGITTARIUS_JUSTICE_ARROW.requiresCi1CrashEpisode,false);assert.equal(SAGITTARIUS_JUSTICE_ARROW.directObservationFromParentClose,true);assert.equal(SAGITTARIUS_JUSTICE_ARROW.postScarletContinuationAuthority,true);assert.equal(SAGITTARIUS_JUSTICE_ARROW.requiredParentConcept,'Scarlet Needle');
  assert.equal(SAGITTARIUS_JUSTICE_ARROW.athenaEntryAuthority,false);assert.equal(SAGITTARIUS_JUSTICE_ARROW.fixedStakeOnly,true);assert.equal(SAGITTARIUS_JUSTICE_ARROW.stakeMultiplierAllowed,false);assert.equal(SAGITTARIUS_JUSTICE_ARROW.oneEntryPerCrashEpisode,false);assert.equal(SAGITTARIUS_JUSTICE_ARROW.oneEntryPerParentScarlet,true);
...
test('R69.5 one-time migration enables post-Scarlet Justice once and later operator disable remains durable',()=>{
  assert.equal(CANONICAL_BOOLEAN_SETTINGS.includes('justiceArrowPostScarletV3Migrated'),true);
  assert.equal(EDITABLE_BOOLEAN_SETTINGS.includes('justiceArrowPostScarletV3Migrated'),false);
  const legacy={...originalSettings(),justiceArrowEnabled:false,justiceArrowPostScarletV3Migrated:false};
  const first=applyJusticeArrowPostScarletV3Migration(legacy);assert.equal(first.migrated,true);assert.equal(first.settings.justiceArrowEnabled,true);assert.equal(first.settings.justiceArrowPostScarletV3Migrated,true);
  const operatorDisabled={...first.settings,justiceArrowEnabled:false};const restart=applyJusticeArrowPostScarletV3Migration(operatorDisabled);assert.equal(restart.migrated,false);assert.equal(restart.settings.justiceArrowEnabled,false);assert.equal(restart.settings.justiceArrowPostScarletV3Migrated,true);
  assert.equal(legacy.justiceArrowEnabled,false,'pure migration must not mutate the loaded settings object');
});
...
test('SJA3 Strategy accepts only a durable profitable same-ticker Scarlet parent plus its own direct post-close confirmation',async()=>{
  const s=settings({justiceArrowEnabled:true,justiceArrowStakeCents:500,justiceArrowMinEntryCents:10,justiceArrowMaxEntryCents:50,justiceArrowMinCrashCents:1,justiceArrowMinReboundCents:1,justiceArrowMinUpwardTicks:1});
  const ticker='SJA3-OPEN',closedAtMs=Date.now()-10_000,parent={id:'scarlet-sja3-open',systemName:s.systemName,ownerId:s.ownerId,conceptName:'Scarlet Needle',ticker,eventTicker:ticker,marketTitle:ticker,side:'YES',mode:s.mode,status:'closed',remainingCount:0,count:10,entryPriceCents:30,exitPriceCents:45,pnlCents:120,closeReason:'infinity_break',openedAtMs:closedAtMs-60_000,closedAtMs,entryConfig:{profitAuthority:INFINITY_BREAK.version,scarletContinuation:{version:SCARLET_NEEDLE.version,policyRevision:SCARLET_NEEDLE.policyRevision,authorizationId:'SCARLET:SJA3-OPEN'}}};
  const db=memoryDb([parent]),m=market(quote(ticker,45,46)),st=new StrategyEngine({db,kalshi:{},market:m,learning:{},getSettings:()=>s,getLiveReady:()=>false,refreshGameClock:async()=>{throw new Error('SJA3 clock bypass expected');},random:()=>0});
  const watch={authorizationId:`JUSTICE-ARROW-V3:SCARLET:${parent.id}`,observationId:`JUSTICE-ARROW-V3:OBS:${parent.id}`,parentScarletEntryId:parent.id,postScarletContinuation:true,directPostScarletObservation:true,ticker,eventTicker:ticker,sourceKind:'POST_SCARLET_NEEDLE',observationStartedAtMs:closedAtMs,preCrashPeakCents:46,troughCents:44,troughAtMs:closedAtMs+2000,crashDepthCents:2,lastBidCents:44,upwardTicks:0,ownCrashArmed:true,authorizedAtMs:closedAtMs+1,crashStartedAtMs:closedAtMs+1000};
  const arrow=await st.executeJusticeArrowAttack(m.getQuote(ticker),watch);assert.ok(arrow,JSON.stringify({audits:db.audits,pipeline:st.entryPipelineSummary()}));assert.equal(arrow.conceptName,'Sagittarius Justice Arrow');assert.equal(arrow.sourceTradeId,watch.observationId);assert.equal(arrow.entryConfig.justiceArrow.parentScarletEntryId,parent.id);assert.equal(arrow.entryConfig.justiceArrow.directPostScarletObservation,true);assert.equal(arrow.entryConfig.justiceArrow.crashEpisodeId,undefined);assert.equal(arrow.entryConfig.profitAuthority,ATHENA_EXIT_INTELLIGENCE.version);assert.equal(arrow.entryConfig.infinityBreak,undefined);assert.equal(arrow.entryConfig.aurora.frozen,true);
  const forged={...watch,parentScarletEntryId:'missing-parent'};assert.equal(await st.executeJusticeArrowAttack(m.getQuote(ticker),forged),null);
});
test('SJA3 end-to-end chain runs Scarlet profit close -> direct own 1/1/1 confirmation -> Justice Arrow -> ATHENA-X1 full-position profit exit',async()=>{
  const s=settings({justiceArrowEnabled:true,justiceArrowStakeCents:500,justiceArrowMinEntryCents:10,justiceArrowMaxEntryCents:50,justiceArrowMinCrashCents:1,justiceArrowMinReboundCents:1,justiceArrowMinUpwardTicks:1,auroraDamageControlPercent:95});
  const ticker='SJA3-CHAIN',closedAtMs=Date.now()-10_000,parent={id:'scarlet-sja3-chain',systemName:s.systemName,ownerId:s.ownerId,conceptName:'Scarlet Needle',ticker,eventTicker:ticker,marketTitle:ticker,side:'YES',mode:s.mode,status:'closed',remainingCount:0,count:10,entryPriceCents:28,exitPriceCents:44,pnlCents:150,closeReason:'infinity_break',openedAtMs:closedAtMs-60_000,closedAtMs,entryConfig:{profitAuthority:INFINITY_BREAK.version,scarletContinuation:{version:SCARLET_NEEDLE.version,policyRevision:SCARLET_NEEDLE.policyRevision,authorizationId:'SCARLET:SJA3-CHAIN'}}};
  const db=memoryDb([parent]),m=market(quote(ticker,44,45)),st=new StrategyEngine({db,kalshi:{},market:m,learning:{},getSettings:()=>s,getLiveReady:()=>false,refreshGameClock:async()=>{throw new Error('SJA3 bypasses ordinary Game Clock');},random:()=>0});
  const engine=Object.create(SagittariusEngine.prototype);engine.settings=s;engine.db=db;engine.market=m;engine.strategy=st;engine.crystalWallWatches=new Map();engine.justiceArrowWatches=new Map();engine.justiceArrowInFlight=new Set();engine.justiceArrowStats=null;engine.justiceArrowQueue={enqueue(){return true;},snapshot(){return{maxConcurrency:1,active:0,pending:0};}};engine.recoveryPriorityTickers=new Set();engine.simulationMutationGate={blocked:false};engine.entryExecutionGate=()=>({allowed:true,reason:'simulation'});engine.health={degraded:false};engine.recomputeHealth=()=>engine.health;
  const armed=await engine.handleJusticeArrowContinuation(parent);assert.equal(armed.status,'OBSERVING');const id=armed.authorizationId;assert.equal(engine.justiceArrowWatches.get(id)?.directPostScarletObservation,true);assert.equal(engine.justiceArrowWatches.get(id)?.ownCrashArmed,false);
  m.setQuote({yesBid:45,yesAsk:46});engine.observeJusticeArrowQuote(m.getQuote(ticker));assert.equal(engine.justiceArrowWatches.get(id)?.preCrashPeakCents,45);
  m.setQuote({yesBid:44,yesAsk:45});engine.observeJusticeArrowQuote(m.getQuote(ticker));assert.equal(engine.justiceArrowWatches.get(id)?.ownCrashArmed,true);assert.equal(engine.justiceArrowWatches.get(id)?.qualified,false);
  m.setQuote({yesBid:45,yesAsk:46});engine.observeJusticeArrowQuote(m.getQuote(ticker));assert.equal(engine.justiceArrowWatches.get(id)?.qualified,true);
  const opened=await engine.attemptJusticeArrowAttack(id);assert.equal(opened.status,'OPENED',JSON.stringify({opened,audits:db.audits}));const arrow=opened.entry;assert.equal(arrow.entryConfig.profitAuthority,'ATHENA-X1');assert.equal(arrow.entryConfig.infinityBreak,undefined);assert.equal(arrow.entryConfig.aurora.frozen,true);assert.equal(arrow.entryConfig.justiceArrow.directPostScarletObservation,true);assert.equal(arrow.entryConfig.justiceArrow.crashEpisodeId,undefined);
  const learning={async onHardStop(){},async stopGuardProfile(){return null;},async lossWatchdogProfile(){return null;},profitLearningState(){return null;},async observeProfitOpportunity(){return null;},profitRetentionProfileCached(){return{retentionRatio:.92,specificity:'cold_start',promoted:false,totalObservations:0,oneTickPullbacks:0,oneTickRecoveries:0,continuationRate:.5,collapseRate:.25,confidence:'low'};},async profitRetentionProfile(){return this.profitRetentionProfileCached();},async markProfitExit(){}};
  const guard=new ProfitGuard({db,kalshi:{},market:m,learning,getSettings:()=>s});let out=null,closedAt=null;for(const bid of [60,58,55,52,49,46,43,40,37,34,31,28]){await new Promise(r=>setTimeout(r,2));m.setQuote({yesBid:bid,yesAsk:Math.min(100,bid+1)});out=await guard.protect(await db.entryById(arrow.id));if(out.closed===true){closedAt=bid;break;}}
  assert.ok(closedAt!=null,'ATHENA-X1 must eventually harvest the profitable runner after confirmed deterioration');const finalArrow=await db.entryById(arrow.id);assert.equal(finalArrow.status,'closed');assert.equal(finalArrow.closeReason,'athena_x1_exit');assert.ok(finalArrow.pnlCents>0);assert.equal(finalArrow.remainingCount,0);
});
test('SJA3 failure chain preserves the frozen Aurora/U-SG1 loss authority after direct post-Scarlet Justice entry',async()=>{
  const s=settings({justiceArrowEnabled:true,justiceArrowStakeCents:500,justiceArrowMinEntryCents:10,justiceArrowMaxEntryCents:50,justiceArrowMinCrashCents:1,justiceArrowMinReboundCents:1,justiceArrowMinUpwardTicks:1,auroraDamageControlPercent:45});
  const ticker='SJA3-AURORA',closedAtMs=Date.now()-10_000,parent={id:'scarlet-sja3-aurora',systemName:s.systemName,ownerId:s.ownerId,conceptName:'Scarlet Needle',ticker,eventTicker:ticker,marketTitle:ticker,side:'YES',mode:s.mode,status:'closed',remainingCount:0,count:10,entryPriceCents:28,exitPriceCents:44,pnlCents:150,closeReason:'infinity_break',openedAtMs:closedAtMs-60_000,closedAtMs,entryConfig:{profitAuthority:INFINITY_BREAK.version,scarletContinuation:{version:SCARLET_NEEDLE.version,policyRevision:SCARLET_NEEDLE.policyRevision,authorizationId:'SCARLET:SJA3-AURORA'}}};
  const db=memoryDb([parent]),m=market(quote(ticker,44,45)),st=new StrategyEngine({db,kalshi:{},market:m,learning:{},getSettings:()=>s,getLiveReady:()=>false,refreshGameClock:async()=>{throw new Error('SJA3 bypasses ordinary Game Clock');},random:()=>0});
  const engine=Object.create(SagittariusEngine.prototype);engine.settings=s;engine.db=db;engine.market=m;engine.strategy=st;engine.crystalWallWatches=new Map();engine.justiceArrowWatches=new Map();engine.justiceArrowInFlight=new Set();engine.justiceArrowStats=null;engine.justiceArrowQueue={enqueue(){return true;},snapshot(){return{maxConcurrency:1,active:0,pending:0};}};engine.recoveryPriorityTickers=new Set();engine.simulationMutationGate={blocked:false};engine.entryExecutionGate=()=>({allowed:true,reason:'simulation'});engine.health={degraded:false};engine.recomputeHealth=()=>engine.health;
  const armed=await engine.handleJusticeArrowContinuation(parent),id=armed.authorizationId;m.setQuote({yesBid:45,yesAsk:46});engine.observeJusticeArrowQuote(m.getQuote(ticker));m.setQuote({yesBid:44,yesAsk:45});engine.observeJusticeArrowQuote(m.getQuote(ticker));m.setQuote({yesBid:45,yesAsk:46});engine.observeJusticeArrowQuote(m.getQuote(ticker));
  const opened=await engine.attemptJusticeArrowAttack(id);assert.equal(opened.status,'OPENED');const arrow=opened.entry,danger=Number(arrow.entryConfig.aurora.dangerPriceCents);assert.ok(danger>0);assert.equal(Number(arrow.entryConfig.aurora.damageControlPercent),45);assert.equal(arrow.entryConfig.profitAuthority,ATHENA_EXIT_INTELLIGENCE.version);
  m.setQuote({yesBid:Math.max(1,danger-1),yesAsk:Math.max(2,danger)});
  const learning={async onHardStop(){},async stopGuardProfile(){return null;},async lossWatchdogProfile(){return null;},profitLearningState(){return null;},async observeProfitOpportunity(){return null;},profitRetentionProfileCached(){return{retentionRatio:.92,specificity:'cold_start',promoted:false,totalObservations:0,oneTickPullbacks:0,oneTickRecoveries:0,continuationRate:.5,collapseRate:.25,confidence:'low'};},async profitRetentionProfile(){return this.profitRetentionProfileCached();},async markProfitExit(){}};
  const guard=new ProfitGuard({db,kalshi:{},market:m,learning,getSettings:()=>s}),out=await guard.protect(await db.entryById(arrow.id));assert.equal(out.protected,true);assert.equal(out.action,'stop_guard');assert.match(String(out.guardState||''),/^USG1_/);const row=await db.entryById(arrow.id);assert.equal(row.entryConfig.aurora.frozen,true);assert.equal(Number(row.entryConfig.aurora.dangerPriceCents),danger);
});
test('R63 shared SIM execution uses visible IOC depth deterministically, so Scarlet is not randomly rejected after hard safety passes',async()=>{
  ...
  const parent={id:'scarlet-parent-r63',systemName:s.systemName,ownerId:s.ownerId,conceptName:SCARLET_NEEDLE.requiredParentConcept,ticker:'SCAR63',eventTicker:'SCAR63',marketTitle:'SCAR63',side:'YES',mode:s.mode,status:'closed',remainingCount:0,entryPriceCents:55,exitPriceCents:84,pnlCents:500,closeReason:CRYSTAL_WALL.profitableCloseReason,closedAtMs:Date.now()-1000,openedAtMs:Date.now()-60_000,entryConfig:{side:'YES',crystalWall:{version:CRYSTAL_WALL.version,policyRevision:CRYSTAL_WALL.policyRevision,crashEpisodeId:'CRASH-SCAR63'},virtualInfinity:{version:INFINITY_BREAK.version,minimumNetPerOriginalContractCents:15}}};
  ...
  const opened=await st.executeScarletContinuation(m.getQuote('SCAR63'),parent,{authorizationId:'SCARLET-THIRD-PROOF:cw-first-proof:scarlet-parent-r63',rootEntryId:'cw-first-proof',repeatIndex:1,maxRepeats:1,authorizedAtMs:Date.now(),thirdProof:seedScarletThirdProof(db,parent)});
...
test('R69.4 complete chain validates three durable Crystal Wall proofs -> Scarlet SIM entry -> Infinity profit exit',async()=>{
  const s=settings({scarletNeedleEnabled:true,scarletNeedleStakeCents:20_000,scarletNeedleMinEntryCents:70,scarletNeedleMaxEntryCents:89,scarletNeedleInfinityNetPerOriginalContractCents:1,scarletNeedleMaxRepeats:1,simFillProbability:0,galacticExplosionEnabled:true,infinityBreakMinNetPerOriginalContractCents:25,infinityBreakRequiredConfirmations:2});
  const parent={id:'scarlet-full-parent-r63',systemName:s.systemName,ownerId:s.ownerId,conceptName:SCARLET_NEEDLE.requiredParentConcept,ticker:'SCARFULL',eventTicker:'SCARFULL',marketTitle:'SCARFULL',side:'YES',mode:s.mode,status:'closed',remainingCount:0,entryPriceCents:55,exitPriceCents:84,pnlCents:500,closeReason:CRYSTAL_WALL.profitableCloseReason,closedAtMs:Date.now()-1000,openedAtMs:Date.now()-60_000,entryConfig:{side:'YES',crystalWall:{version:CRYSTAL_WALL.version,policyRevision:CRYSTAL_WALL.policyRevision,crashEpisodeId:'CRASH-SCARFULL'},virtualInfinity:{version:INFINITY_BREAK.version,minimumNetPerOriginalContractCents:15}}};
  const firstProof={...structuredClone(parent),id:'scarlet-full-first-r63',openedAtMs:Date.now()-210_000,closedAtMs:Date.now()-150_000,pnlCents:100,entryConfig:{...structuredClone(parent.entryConfig),crystalWall:{...structuredClone(parent.entryConfig.crystalWall),crashEpisodeId:'CRASH-SCARFULL-FIRST'}}};
  const secondProof={...structuredClone(parent),id:'scarlet-full-second-r63',openedAtMs:Date.now()-140_000,closedAtMs:Date.now()-80_000,pnlCents:100,entryConfig:{...structuredClone(parent.entryConfig),crystalWall:{...structuredClone(parent.entryConfig.crystalWall),crashEpisodeId:'CRASH-SCARFULL-SECOND'}}};
  const db=memoryDb([firstProof,secondProof,parent]),m=market(quote('SCARFULL',84,85));
...
test('SJA3 dashboard exposes direct post-Scarlet own confirmation with no CI1 prerequisite and ATHENA-X1 exit',async()=>{
  ...
  assert.ok(app.includes("legacy:'Sagittarius Justice Arrow'"));assert.ok(html.includes('/app.js?v=SJA3DIRECT-R696'));assert.ok(html.includes('ATHENA-X1 smart peak-runner authority'));assert.ok(app.includes("['justiceArrowMinCrashCents','Min crash','c']"));assert.ok(app.includes('POST-SCARLET authority: profitable Scarlet Needle Infinity close'));assert.ok(app.includes('CI1 crash NOT required'));assert.equal(app.includes('NEW same-ticker CI1 crash'),false);assert.equal(app.includes('Independent authority: CI1 crash episode'),false);
});
test('R63 Gemini and Justice Arrow settings migrate safely and PostgreSQL readback verifies the independent controls',async()=>{
  ...
  assert.equal(CANONICAL_NUMERIC_SETTINGS.includes('geminiReferenceStakeCents'),true);assert.equal(CANONICAL_NUMERIC_SETTINGS.includes('geminiMinPriceCents'),true);assert.equal(CANONICAL_NUMERIC_SETTINGS.includes('geminiMaxPriceCents'),true);assert.equal(CANONICAL_NUMERIC_SETTINGS.includes('justiceArrowStakeCents'),true);assert.equal(CANONICAL_NUMERIC_SETTINGS.includes('justiceArrowMinEntryCents'),true);assert.equal(CANONICAL_NUMERIC_SETTINGS.includes('justiceArrowMaxEntryCents'),true);for(const k of ['justiceArrowMinCrashCents','justiceArrowMinReboundCents','justiceArrowMinUpwardTicks'])assert.equal(CANONICAL_NUMERIC_SETTINGS.includes(k),true,k);
  ...
  const out=await engine.applySettingsPatch({geminiEnabled:true,geminiReferenceStakeCents:21000,geminiMinPriceCents:40,geminiMaxPriceCents:87,justiceArrowEnabled:true,justiceArrowStakeCents:23456,justiceArrowMinEntryCents:71,justiceArrowMaxEntryCents:88,justiceArrowMinCrashCents:31,justiceArrowMinReboundCents:6,justiceArrowMinUpwardTicks:3});
  assert.equal(out.geminiEnabled,true);assert.equal(out.geminiReferenceStakeCents,21000);assert.equal(out.geminiMinPriceCents,40);assert.equal(out.geminiMaxPriceCents,87);assert.equal(out.justiceArrowEnabled,true);assert.equal(out.justiceArrowStakeCents,23456);assert.equal(out.justiceArrowMinEntryCents,71);assert.equal(out.justiceArrowMaxEntryCents,88);assert.equal(out.justiceArrowMinCrashCents,31);assert.equal(out.justiceArrowMinReboundCents,6);assert.equal(out.justiceArrowMinUpwardTicks,3);
  ...
  await assert.rejects(()=>engine.applySettingsPatch({justiceArrowMinCrashCents:30.5}),/justiceArrowMinCrashCents must be an integer from 1 to 99/);
  await assert.rejects(()=>engine.applySettingsPatch({justiceArrowMinReboundCents:0}),/justiceArrowMinReboundCents must be an integer from 1 to 99/);
  await assert.rejects(()=>engine.applySettingsPatch({justiceArrowMinUpwardTicks:21}),/justiceArrowMinUpwardTicks must be an integer from 1 to 20/);
  assert.equal(persisted.justiceArrowMinCrashCents,31,'invalid settings must fail before persistence');
});

test('R69.6 Justice direct watch trails the post-Scarlet peak and qualifies on own 1c crash + 1c rebound + 1 upward tick without CI1',()=>{
  const s=settings({justiceArrowEnabled:true,justiceArrowMinCrashCents:1,justiceArrowMinReboundCents:1,justiceArrowMinUpwardTicks:1,justiceArrowMinEntryCents:10,justiceArrowMaxEntryCents:92,maxSpreadCents:3});
  const watch={authorizationId:'JA-DIRECT',observationId:'JA-DIRECT-OBS',parentScarletEntryId:'SCARLET-DIRECT',postScarletContinuation:true,directPostScarletObservation:true,ticker:'JA-DIRECT',eventTicker:'JA-DIRECT',parentScarletClosedAtMs:1000,parentScarletExitPriceCents:80,observationStartedAtMs:1000,preCrashPeakCents:80,troughCents:80,troughAtMs:1000,lastBidCents:80,ownCrashArmed:false,upwardTicks:0};
  let x=justiceArrowSignalState(watch,quote('JA-DIRECT',82,83),s,2000);assert.equal(x.qualified,false);assert.equal(x.ownCrashArmed,false);assert.equal(x.preCrashPeakCents,82);
  x=justiceArrowSignalState(x,quote('JA-DIRECT',81,82),s,3000);assert.equal(x.qualified,false);assert.equal(x.ownCrashArmed,true);assert.equal(x.crashDepthCents,1);assert.equal(x.troughCents,81);
  x=justiceArrowSignalState(x,quote('JA-DIRECT',82,83),s,4000);assert.equal(x.qualified,true);assert.equal(x.reboundCents,1);assert.equal(x.upwardTicks,1);
});

test('SJA3 rejects non-Scarlet parent callbacks and raw crash activity cannot create an independent Justice watch',async()=>{
  const s=settings({justiceArrowEnabled:true}),engine=Object.create(SagittariusEngine.prototype);engine.settings=s;engine.justiceArrowWatches=new Map();engine.justiceArrowInFlight=new Set();engine.justiceArrowStats=null;engine.recoveryPriorityTickers=new Set();engine.db={audit:async()=>{}};
  const out=await engine.handleJusticeArrowContinuation({id:'old-ad',conceptName:'Another Dimension'});assert.equal(out.status,'IGNORED');assert.equal(out.reason,'not_scarlet_parent');assert.equal(engine.justiceArrowWatches.size,0);
  assert.equal(engine.observeJusticeArrowQuote(quote('RAW-SJA3',20,21)),false);assert.equal(engine.justiceArrowWatches.size,0);
});
