...
test('CW3 legacy shared aliases migrate normal models while Crystal Wall is reset once to its safe fixed profile',async()=>{const C=await import('../src/config.mjs');const s=C.sanitizeRuntimeSettings({hunterStakeCents:21000,stakeCents:22000,feederStakeCents:3100,recoveryBaseStakeCents:11000});assert.equal(s.momentumStakeCents,21000);assert.equal(s.waveStakeCents,22000);assert.equal(s.pegasusReferenceStakeCents,3100);assert.equal(s.recoveryStakeCents,500);assert.equal(s.recoveryMinEntryCents,10);assert.equal(s.recoveryMaxEntryCents,50);assert.equal(s.crystalWallMinCrashCents,15);for(const k of ['hunterStakeCents','stakeCents','feederStakeCents','recoveryBaseStakeCents'])assert.equal(Object.hasOwn(s,k),false,k);});
...
test('CW3 retired Recovery runtime is inert; direct Crystal Wall owns crash/rebound entry and Aurora remains the loss authority',async()=>{
  const stopped=row({id:'loss',conceptName:'Momentum Hunter',ticker:'R',eventTicker:'REV',status:'closed',closeReason:'hard_stop_loss',exitPriceCents:20,closedAtMs:Date.now()-60000,remainingCount:0});const db=memoryDb([stopped]);
  const s=settings({recoveryHunterEnabled:true,recoveryStakeCents:500,recoveryMinEntryCents:10,recoveryMaxEntryCents:50,crystalWallMinCrashCents:15,crystalWallMinReboundCents:5,crystalWallMinUpwardTicks:2});const st=strategyFor(s,db);
  assert.deepEqual(await st.evaluateRecovery(new Map([['R',q('R',25,26,'REV')]]),{legacyCompatibility:true}),[]);
  const D=await import('../src/doctrine.mjs');assert.equal(D.CRYSTAL_WALL.virtualLossAuthority,'AURORA_EXECUTION');assert.equal(D.CRYSTAL_WALL.stakeMultiplierAllowed,false);
});
...
test('R66 GCA2 shared observed-activity fallback ages conservatively and authorizes identical LIVE/SIM entry evidence',async()=>{
  let now=1_800_000_000_000;
  const event='R66-SHARED-ACTIVITY-EV';
  const authority=new GameClockAuthority({kalshi:{async getMilestonesForEvent(){return[];}},now:()=>now});
  const market=q('R66-SHARED-ACTIVITY',88,89,event);
  ...
  const first=await authority.resolveEvent({eventTicker:event,quotes:[market],now,allowObservedActivityClock:true,minimumElapsedMs,forceFresh:true});
  assert.equal(first.phase,'UNKNOWN');
  assert.equal(first.reason,'observed_market_activity_aging');
  assert.equal(first.observedActivityStartMs,now);
  assert.equal(first.startTimeMs,null,'a provisional observed-activity lower bound is not yet a confirmed game clock');
  let rolling=first;
  for(let i=0;i<11;i+=1){
    now+=5*60*1000; market.recentTradesObservedAtMs=now;
    rolling=await authority.resolveEvent({eventTicker:event,quotes:[market],priorState:rolling,now,allowObservedActivityClock:true,minimumElapsedMs,forceFresh:true});
    assert.equal(rolling.phase,'UNKNOWN');
    assert.equal(rolling.observedActivityStartMs,first.observedActivityStartMs);
  }
  now+=5*60*1000; market.recentTradesObservedAtMs=now;
  const authorized=await authority.resolveEvent({eventTicker:event,quotes:[market],priorState:rolling,now,allowObservedActivityClock:true,minimumElapsedMs,forceFresh:true});
  assert.equal(authorized.phase,'CONFIRMED');
  assert.equal(authorized.source,'observed_market_activity');
  assert.equal(authorized.sourceStrength,'bounded_observed_activity');
  assert.equal(authorized.startTimeMs,first.observedActivityStartMs,'the lower bound is the first sustained observed activity, never earlier');
  assert.equal(authorized.entryAuthorized,true);
  assert.equal(authorized.evidence.sharedLiveSimulationAuthority,true);
  assert.equal(isEntryAuthorizedGameClockState(authorized,event,now),true);

  // The clock authority is intentionally mode-agnostic. Replaying the exact
  // same fresh evidence from the same provenance must produce the same decision
  // for the LIVE and SIM engine callers.
  for(const mode of ['SIMULATION','LIVE']){
    market.recentTradesObservedAtMs=now+1000;
    const replay=await authority.resolveEvent({eventTicker:event,quotes:[market],priorState:authorized,now:now+1000,allowObservedActivityClock:true,minimumElapsedMs,forceFresh:true});
    assert.equal(replay.entryAuthorized,true,`${mode} lost shared observed-activity authority`);
    assert.equal(replay.authorizationReason,'observed_market_activity_aged_plus_fresh_exact_trade');
    assert.equal(replay.source,'observed_market_activity');
  }
});

test('R66 GCA2 migrates legacy SIM-only persisted activity provenance into shared LIVE/SIM authority',async()=>{
  const now=1_800_000_000_000;
  const event='R66-LEGACY-ACTIVITY-EV';
  const authority=new GameClockAuthority({kalshi:{async getMilestonesForEvent(){return[];}},now:()=>now});
  const market=q('R66-LEGACY-ACTIVITY',88,89,event);
  market.liveStatus='live'; market.recentTrades=20; market.recentTradesObservedAtMs=now; market.occurrenceTimeMs=now+4*60*60*1000;
  const legacy={version:'GCA2',eventTicker:event,phase:'CONFIRMED',confirmed:true,startTimeMs:now-70*60_000,source:'simulation_observed_activity',sourceStrength:'simulation_only',observedAtMs:now-70*60_000,entryAuthorized:true,evidenceObservedAtMs:now-1000,lastCheckedAtMs:now-1000,simulationActivityStartMs:now-70*60_000,simulationActivityLastEvidenceMs:now-1000,simulationActivityEvidenceCount:10};
  const migrated=await authority.resolveEvent({eventTicker:event,quotes:[market],priorState:legacy,now,allowObservedActivityClock:true,minimumElapsedMs:60*60_000,forceFresh:true});
  assert.equal(migrated.source,'observed_market_activity');
  assert.equal(migrated.sourceStrength,'bounded_observed_activity');
  assert.equal(migrated.entryAuthorized,true,'a LIVE process must not inherit the old SIM-only veto after deployment');
  assert.equal(migrated.observedActivityStartMs,legacy.simulationActivityStartMs);
  assert.equal(migrated.authorizationReason,'observed_market_activity_aged_plus_fresh_exact_trade');
});

test('R66 GCA2 shared observed-activity clock resets after a long evidence gap instead of inheriting stale pre-match activity',async()=>{
  let now=1_800_000_000_000;
  const event='R66-SHARED-GAP-EV';
  const authority=new GameClockAuthority({kalshi:{async getMilestonesForEvent(){return[];}},now:()=>now});
  const market=q('R66-SHARED-GAP',88,89,event);
  market.liveStatus='live'; market.recentTrades=20; market.recentTradesObservedAtMs=now; market.occurrenceTimeMs=now+4*60*60*1000;
  const first=await authority.resolveEvent({eventTicker:event,quotes:[market],now,allowObservedActivityClock:true,minimumElapsedMs:60*60*1000,forceFresh:true});
  now+=11*60*1000; market.recentTradesObservedAtMs=now;
  const reset=await authority.resolveEvent({eventTicker:event,quotes:[market],priorState:first,now,allowObservedActivityClock:true,minimumElapsedMs:60*60*1000,forceFresh:true});
  assert.equal(reset.phase,'UNKNOWN');
  assert.equal(reset.observedActivityStartMs,now,'a >10 minute evidence gap must restart the conservative lower bound');
  assert.equal(reset.observedActivityEvidenceCount,1);
  assert.equal(reset.evidence.continuityBroken,true);
});

test('R66 GCA2 explicit official pregame evidence outranks and blocks the shared observed-activity fallback in both modes',async()=>{
  const now=1_800_000_000_000;
  const event='R66-SHARED-PREGAME-EV';
  const authority=new GameClockAuthority({kalshi:{
    async getMilestonesForEvent(){return[{id:'r66-pre',category:'Sports',type:'tennis_match',primary_event_tickers:[event],related_event_tickers:[event]}];},
    async getLiveData(){return{milestone_id:'r66-pre',details:{status:'scheduled'}};},
    async getGameStats(){return null;},
  },now:()=>now});
  const market=q('R66-SHARED-PREGAME',88,89,event); market.liveStatus='live'; market.recentTrades=250; market.recentTradesObservedAtMs=now; market.occurrenceTimeMs=now+3*60*60*1000;
  const state=await authority.resolveEvent({eventTicker:event,quotes:[market],now,allowObservedActivityClock:true,minimumElapsedMs:0,forceFresh:true});
  assert.equal(state.phase,'UNKNOWN');
...
test('CW3 does not reuse parent Game Clock snapshots because crash/rebound timing is its independent authority',async()=>{
  const now=Date.now(),start=now-30*60000;const r17Loss=row({id:'r17-loss',conceptName:'Momentum Hunter',ticker:'RR',eventTicker:'RREV',status:'closed',closeReason:'hard_stop_loss',exitPriceCents:20,closedAtMs:now-60000,remainingCount:0,gameStartTimeMs:start,entryConfig:{release:RELEASE,gameClockAuthority:confirmedClock('RREV',start)}});const db=memoryDb([r17Loss]);const s=settings({recoveryHunterEnabled:true,recoveryStakeCents:500,recoveryMinEntryCents:10,recoveryMaxEntryCents:50,crystalWallMinCrashCents:15,crystalWallMinReboundCents:5,crystalWallMinUpwardTicks:2});const st=strategyFor(s,db);const rq=q('RR',25,26,'RREV');rq.gameStartTimeMs=null;rq.gameClockState={};assert.deepEqual(await st.evaluateRecovery(new Map([['RR',rq]]),{legacyCompatibility:true}),[],'retired parent-stop Recovery route must remain closed');const {readFile}=await import('node:fs/promises');const engineSource=await readFile(new URL('../src/engine.mjs',import.meta.url),'utf8');const strategySource=await readFile(new URL('../src/strategy.mjs',import.meta.url),'utf8');assert.ok(strategySource.includes('crystal_wall_crash_authority_exempt'));assert.equal(engineSource.includes('strategy.recoveryCandidateTickers()'),false);
});
...
test('R66 Engine wires the shared observed-activity authority identically for LIVE and SIMULATION force-fresh entry refresh',async()=>{
  const baseNow=Date.now();
  const event='R66-ENGINE-PARITY-EV';
  const ticker='R66-ENGINE-PARITY';
  const results=[];
  for(const mode of ['SIMULATION','LIVE']){
    let current={
      ticker,eventTicker:event,title:ticker,yesBid:79,yesAsk:80,recentTrades:20,recentTradesObservedAtMs:baseNow,
      volume24h:10000,updatedAtMs:baseNow,status:'active',result:'',occurrenceTimeMs:baseNow+4*60*60_000,
      closeTimeMs:baseNow+6*60*60_000,liveStatus:'live',gameStartTimeMs:null,gameClockState:{},
    };
    const prior={
      version:'GCA2',eventTicker:event,phase:'UNKNOWN',confirmed:false,startTimeMs:null,entryAuthorized:false,
      observedActivityStartMs:baseNow-40*60_000,observedActivityLastEvidenceMs:baseNow-1000,
      observedActivityEvidenceCount:8,lastCheckedAtMs:baseNow-1000,reason:'observed_market_activity_aging',
    };
    const engine=Object.create(SagittariusEngine.prototype);
    engine.settings={systemName:'SAGITTARIUS',mode,minGameMinutes:30};
    engine.gameClock=new GameClockAuthority({kalshi:{async getMilestonesForEvent(){return[];}}});
    engine.db={
      async gameClockStates(){return new Map([[event,structuredClone(prior)]]);},
      async upsertGameClockState(){},
      async audit(){},
    };
    engine.kalshi={async getRecentTradesForTicker(t,minutes){assert.equal(t,ticker);assert.equal(minutes,5);return{observedAtMs:Date.now(),count:20,lastPriceCents:79};}};
    engine.market={
      async refreshTicker(t){assert.equal(t,ticker);current={...current,updatedAtMs:Date.now()};return current;},
      getQuote(t){return t===ticker?current:null;},
    };
    engine.lastScanMarkets=[current];
    const candidate={...current,gameClockState:structuredClone(prior)};
    const refreshed=await engine.refreshGameClockForQuote(candidate,{forceFresh:true});
    assert.ok(refreshed?.gameClockState,`${mode} refresh did not return a clock`);
    assert.equal(refreshed.gameClockState.entryAuthorized,true,`${mode} lost shared entry authority`);
    assert.equal(refreshed.gameClockState.source,'observed_market_activity');
    assert.equal(refreshed.gameClockState.authorizationReason,'observed_market_activity_aged_plus_fresh_exact_trade');
    results.push({
      source:refreshed.gameClockState.source,
      sourceStrength:refreshed.gameClockState.sourceStrength,
      authorizationReason:refreshed.gameClockState.authorizationReason,
      entryAuthorized:refreshed.gameClockState.entryAuthorized,
      startTimeMs:refreshed.gameClockState.startTimeMs,
    });
  }
  assert.deepEqual(results[1],results[0],'LIVE and SIMULATION must make the same pre-execution Game Clock decision from the same evidence');
});
