...
import { ATHENA_BRAIN, ATHENA_B2, ATHENA_B2_R2_MODEL_VALIDATION, Athena, AthenaSoul, compileAthenaBrain, assessAthenaBrain, assessAthenaB2, athenaB2FeatureVector, athenaB2R2RiskFromFeatures, validateAthenaBrain } from '../src/athena.mjs';
import { ATHENA_SOUL, scarletHandoffGate } from '../src/doctrine.mjs';
...
test('S1 Athena Soul seals one story per bolt and refuses a second game on the same ticker',()=>{
  assert.equal(ATHENA_SOUL.shieldNeverBlocksBolt,true);
  const soul=new AthenaSoul();
  const a=soul.admitChapter1({ticker:'SON-SON',concept:'Wave Surfer',entryCents:82,boltId:'b1',source:'Pegasus'});
  assert.equal(a.ok,true);assert.equal(a.story.winSaint,'Scarlet Needle');assert.equal(a.story.woundSaint,'Crystal Wall');
  const same=soul.admitChapter1({ticker:'SON-SON',concept:'Momentum Hunter',entryCents:81,boltId:'b2',source:'Dragon'});
  assert.equal(same.ok,false);assert.equal(same.reason,'soul_same_board');
  const other=soul.admitChapter1({ticker:'SAK-SAK',concept:'Momentum Hunter',entryCents:66,boltId:'b3',source:'Dragon'});
  assert.equal(other.ok,true);assert.equal(soul.snapshot().openStories,2);
});

test('S1 Athena Soul refuses Grave and wrong rooms and does not use the shield as a gate',()=>{
  const soul=new AthenaSoul();
  assert.equal(soul.admitChapter1({ticker:'G',concept:'Wave Surfer',entryCents:12,boltId:'g'}).reason,'soul_grave');
  assert.equal(soul.admitChapter1({ticker:'C',concept:'Crash Recovery Hunter',entryCents:80,boltId:'c'}).reason,'soul_wrong_room');
  assert.equal(soul.admitChapter1({ticker:'P',concept:'Lightning Plasma',entryCents:40,boltId:'p'}).reason,'soul_not_opener');
  soul.ledger.realizedCents=-99_999;
  assert.equal(soul.snapshot().color,'RED');
  const still=soul.admitChapter1({ticker:'LIVE',concept:'Wave Surfer',entryCents:80,boltId:'ok',source:'Phoenix'});
  assert.equal(still.ok,true,'red shield must not refuse a new board');
});

test('S1 Athena Soul Chapter 2 only calls the sealed Saint',()=>{
  const soul=new AthenaSoul();
  soul.admitChapter1({ticker:'T',concept:'Wave Surfer',entryCents:82,boltId:'b',source:'Pegasus'});
  assert.equal(soul.admitChapter2({ticker:'T',concept:'Lightning Plasma',printCents:60}).reason,'soul_script_mismatch');
  const wall=soul.admitChapter2({ticker:'T',concept:'Crystal Wall',printCents:60});
  assert.equal(wall.ok,true);assert.equal(wall.reason,'soul_chapter2_wound');
  assert.equal(soul.admitChapter2({ticker:'T',concept:'Crystal Wall',printCents:62}).reason,'soul_second_reserve');
});

test('S2 Crash Recovery names Plasma as wound and Scarlet handoff may enter 50-99',()=>{
  const soul=new AthenaSoul();
  const cr=soul.admitChapter1({ticker:'R',concept:'Crash Recovery Hunter',entryCents:60,boltId:'cr',source:'Dragon'});
  assert.equal(cr.ok,true);assert.equal(cr.story.woundSaint,'Lightning Plasma');
  soul.closeChapter({ticker:'R',reason:'hard_stop_loss',printCents:58,pnlCents:-5000});
  const plasma=soul.admitChapter2({ticker:'R',concept:'Lightning Plasma',printCents:60});
  assert.equal(plasma.ok,true);
  const crown=new AthenaSoul();
  crown.admitChapter1({ticker:'C',concept:'Wave Surfer',entryCents:82,boltId:'w',source:'Phoenix'});
  const scarlet=crown.admitChapter2({ticker:'C',concept:'Scarlet Needle',printCents:90});
  assert.equal(scarlet.ok,true);
  assert.equal(crown.admitChapter2({ticker:'C',concept:'Scarlet Needle',printCents:40}).reason,'soul_wrong_room');
});

test('Soul ledger hydrates after a blank restart',()=>{
  const a=new AthenaSoul();
  a.admitChapter1({ticker:'P',concept:'Momentum Hunter',entryCents:66,boltId:'b1',source:'Pegasus'});
  const raw=a.serialize();
  const b=new AthenaSoul();
  b.hydrate(raw);
  assert.equal(b.snapshot().openStories,1);
  assert.equal(b.snapshot().ledger.opened,1);
  assert.equal(b.snapshot().stories[0].ticker,'P');
});

test('SN3 retires the Athena Soul Scarlet handoff veto so Crystal Wall wins are not re-vetoed',()=>{
  for(const input of [
    {soulEnabled:true,parentEntryCents:56,parentExitCents:66,askCents:67},
    {soulEnabled:true,parentEntryCents:81,parentExitCents:86,askCents:86},
    {soulEnabled:true,parentEntryCents:81,parentExitCents:86,askCents:87},
    {soulEnabled:false,parentEntryCents:56,parentExitCents:66,askCents:67},
  ]){const out=scarletHandoffGate(input);assert.equal(out.ok,true);assert.equal(out.reason,'sn3_legacy_handoff_gate_disabled');}
});
