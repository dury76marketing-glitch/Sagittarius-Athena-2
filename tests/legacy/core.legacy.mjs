...
import { KalshiClient, exchangeBalanceCents } from '../src/kalshi.mjs';
...
test('Kalshi V2 order supports explicit fill-or-kill while preserving IOC as the default', () => {
  const k=new KalshiClient({},'https://example.test/trade-api/v2','');
  const fok=k.buildOrder({ticker:'T',action:'buy',count:7,priceCents:21,clientOrderId:'fok',timeInForce:'fill_or_kill'});
  assert.equal(fok.time_in_force,'fill_or_kill');
  const normal=k.buildOrder({ticker:'T',action:'buy',count:7,priceCents:21,clientOrderId:'ioc'});
  assert.equal(normal.time_in_force,'immediate_or_cancel');
  assert.throws(()=>k.buildOrder({ticker:'T',action:'buy',count:1,priceCents:21,clientOrderId:'bad',timeInForce:'unknown'}),/invalid timeInForce/);
});

test('Kalshi shard routing uses the authoritative exchange index and balance preflight reads shard-local collateral',async()=>{
  const k=new KalshiClient({},'https://example.test/trade-api/v2','');
  const explicit=k.buildOrder({ticker:'T',action:'buy',count:2,priceCents:77,clientOrderId:'c',exchangeIndex:3});
  assert.equal(explicit.exchange_index,3);
  const balance={balance:45359,balance_breakdown:[{exchange_index:0,balance:'453.5994'},{exchange_index:3,balance:'0.0000'}]};
  assert.equal(exchangeBalanceCents(balance,0),45359.94);assert.equal(exchangeBalanceCents(balance,3),0);
  k.getBalance=async()=>balance;k.getMarket=async()=>({ticker:'T',exchangeIndex:3});
  let p=await k.preflightExchangeBalance({ticker:'T',requiredCents:100});
  assert.equal(p.ok,false);assert.equal(p.reason,'live_exchange_insufficient_balance');assert.equal(p.exchangeIndex,3);
  k.getBalance=async()=>({balance:45359,balance_breakdown:[{exchange_index:3,balance:'12.5000'}]});
  p=await k.preflightExchangeBalance({ticker:'T',exchangeIndex:3,requiredCents:100});assert.equal(p.ok,true);assert.equal(p.availableCents,1250);
});

test('Kalshi just-in-time shard funding transfers only the exact shortfall, deduplicates while pending, and requires fresh balance proof before entry',async()=>{
  const k=new KalshiClient({},'https://example.test/trade-api/v2','');
  ...
  const requests=[];
  k.getBalance=async()=>structuredClone(balance);
  k.request=async(method,path,body)=>{requests.push({method,path,body:structuredClone(body)});return{ok:true,status:200,data:{transfer_id:'xfer-1'}};};

  let p=await k.ensureExchangeBalance({ticker:'T',exchangeIndex:3,requiredCents:82});
  assert.equal(p.ok,false);assert.equal(p.reason,'live_exchange_rebalance_requested');assert.equal(p.retryable,true);assert.equal(p.exchangeIndex,3);assert.equal(p.fundingRequestedCents,82);assert.deepEqual(p.transferIds,['xfer-1']);
  assert.equal(requests.length,1);assert.equal(requests[0].method,'POST');assert.equal(requests[0].path,'/portfolio/intra_exchange_instance_transfer');
  assert.deepEqual(requests[0].body,{source:'event_contract',destination:'event_contract',amount:8200,source_exchange_shard:0,destination_exchange_shard:3,source_subaccount:0,destination_subaccount:0});

  p=await k.ensureExchangeBalance({ticker:'T',exchangeIndex:3,requiredCents:82});
  assert.equal(p.ok,false);assert.equal(p.reason,'live_exchange_rebalance_pending');assert.equal(p.retryable,true);assert.equal(requests.length,1,'pending async transfer must not be duplicated');

  balance={balance:45561,balance_breakdown:[{exchange_index:0,balance:'455.6145'},{exchange_index:1,balance:'0.0000'},{exchange_index:2,balance:'0.0000'},{exchange_index:3,balance:'0.8200'}]};
  p=await k.ensureExchangeBalance({ticker:'T',exchangeIndex:3,requiredCents:82});
  assert.equal(p.ok,true);assert.equal(p.reason,'live_exchange_balance_ready');assert.equal(p.availableCents,82);assert.equal(p.rebalanced,false);assert.equal(requests.length,1);
});

test('Kalshi just-in-time shard funding fails closed when aggregate transferable collateral cannot cover the entry',async()=>{
  const k=new KalshiClient({},'https://example.test/trade-api/v2','');let requests=0;
  k.getBalance=async()=>({balance:0,balance_breakdown:[{exchange_index:0,balance:'0.4000'},{exchange_index:1,balance:'0.1000'},{exchange_index:3,balance:'0.0000'}]});
  k.request=async()=>{requests+=1;throw new Error('must not transfer');};
  const p=await k.ensureExchangeBalance({ticker:'T',exchangeIndex:3,requiredCents:82});
  assert.equal(p.ok,false);assert.equal(p.reason,'live_account_insufficient_balance');assert.equal(p.exchangeIndex,3);assert.equal(p.transferableCents,50);assert.equal(requests,0);
});

test('Kalshi shard transfer retains broker error truth and never marks failed funding as spendable',async()=>{
  const k=new KalshiClient({},'https://example.test/trade-api/v2','');
  k.getBalance=async()=>({balance:45643,balance_breakdown:[{exchange_index:0,balance:'456.4345'},{exchange_index:3,balance:'0.0000'}]});
  k.request=async()=>({ok:false,status:409,data:{error:{code:'transfer_conflict',message:'transfer unavailable',details:{destination_exchange_shard:3}}}});
  const p=await k.ensureExchangeBalance({ticker:'T',exchangeIndex:3,requiredCents:82});
  assert.equal(p.ok,false);assert.equal(p.reason,'live_exchange_rebalance_failed');assert.equal(p.brokerErrorCode,'transfer_conflict');assert.equal(p.brokerErrorMessage,'transfer unavailable');assert.deepEqual(p.brokerErrorDetails,{destination_exchange_shard:3});assert.equal(k.exchangeFundingInFlight.size,0);
});

test('Kalshi ambiguous shard-transfer response is held fail-closed and cannot be blindly duplicated',async()=>{
  const k=new KalshiClient({},'https://example.test/trade-api/v2','');let requests=0;
  let balance={balance:45643,balance_breakdown:[{exchange_index:0,balance:'456.4345'},{exchange_index:3,balance:'0.0000'}]};
  k.getBalance=async()=>structuredClone(balance);
  k.request=async()=>{requests+=1;return{ok:false,status:503,data:{code:'temporarily_unavailable',message:'unknown transfer outcome'}};};
  let p=await k.ensureExchangeBalance({ticker:'T',exchangeIndex:3,requiredCents:82});
  assert.equal(p.ok,false);assert.equal(p.reason,'live_exchange_rebalance_ambiguous');assert.equal(p.retryable,true);assert.equal(p.ambiguous,true);assert.equal(requests,1);
  p=await k.ensureExchangeBalance({ticker:'T',exchangeIndex:3,requiredCents:82});
  assert.equal(p.ok,false);assert.equal(p.reason,'live_exchange_rebalance_pending');assert.equal(p.ambiguous,true);assert.equal(requests,1,'ambiguous mutation must not be retried while its outcome is unknown');
  balance={balance:45561,balance_breakdown:[{exchange_index:0,balance:'455.6145'},{exchange_index:3,balance:'0.8200'}]};
  p=await k.ensureExchangeBalance({ticker:'T',exchangeIndex:3,requiredCents:82});
  assert.equal(p.ok,true);assert.equal(p.reason,'live_exchange_balance_ready');assert.equal(p.availableCents,82);assert.equal(requests,1);
});

test('Kalshi multi-donor funding performs at most one asynchronous transfer per fresh evaluation',async()=>{
  const k=new KalshiClient({},'https://example.test/trade-api/v2','');const requests=[];
  let balance={balance:0,balance_breakdown:[{exchange_index:0,balance:'0.5000'},{exchange_index:1,balance:'0.5000'},{exchange_index:3,balance:'0.0000'}]};
  k.getBalance=async()=>structuredClone(balance);
  k.request=async(_method,_path,body)=>{requests.push(structuredClone(body));return{ok:true,status:200,data:{transfer_id:`xfer-${requests.length}`}};};
  let p=await k.ensureExchangeBalance({ticker:'T',exchangeIndex:3,requiredCents:82});
  assert.equal(p.reason,'live_exchange_rebalance_requested');assert.equal(p.fundingRequestedCents,50);assert.equal(requests.length,1);
  balance={balance:0,balance_breakdown:[{exchange_index:0,balance:'0.0000'},{exchange_index:1,balance:'0.5000'},{exchange_index:3,balance:'0.5000'}]};
  p=await k.ensureExchangeBalance({ticker:'T',exchangeIndex:3,requiredCents:82});
  assert.equal(p.reason,'live_exchange_rebalance_requested');assert.equal(p.fundingRequestedCents,32);assert.equal(requests.length,2);assert.equal(requests[1].source_exchange_shard,1);assert.equal(requests[1].amount,3200);
  balance={balance:0,balance_breakdown:[{exchange_index:0,balance:'0.0000'},{exchange_index:1,balance:'0.1800'},{exchange_index:3,balance:'0.8200'}]};
  p=await k.ensureExchangeBalance({ticker:'T',exchangeIndex:3,requiredCents:82});assert.equal(p.ok,true);assert.equal(requests.length,2);
});

test('Kalshi V2 error truth preserves broker code, message and details instead of collapsing objects to [object Object]',()=>{
  const k=new KalshiClient({},'https://example.test/trade-api/v2','');
  const r=k.resultFrom({error:{code:'market_not_found',message:'market unavailable',details:{exchange_index:3}}},'cid');
  assert.equal(r.brokerErrorCode,'market_not_found');assert.equal(r.brokerErrorMessage,'market unavailable');assert.deepEqual(r.brokerErrorDetails,{exchange_index:3});assert.match(r.brokerError,/market_not_found/);
});
...
test('CW3 RH1 keeps exact 2x configured Recovery stake and lets the normal buying-power gate decide',async()=>{const C=await import('../src/config.mjs');const D=await import('../src/doctrine.mjs');const {readFile}=await import('node:fs/promises');const strategy=await readFile(new URL('../src/strategy.mjs',import.meta.url),'utf8');const s=C.originalSettings();assert.equal(s.recoveryHunterEnabled,true);assert.equal(s.recoveryStakeCents,500);assert.equal(s.recoveryMinEntryCents,10);assert.equal(s.recoveryMaxEntryCents,50);assert.equal(s.crystalWallMinCrashCents,15);assert.equal(s.crystalWallMinReboundCents,5);assert.equal(s.crystalWallMinUpwardTicks,2);assert.equal(Object.hasOwn(s,'recoveryBaseStakeCents'),false);assert.equal(Object.hasOwn(s,'recoveryMinReboundCents'),false);assert.equal(D.CRYSTAL_WALL.version,'CRYSTAL-WALL-V3');assert.equal(D.CRYSTAL_WALL.athenaAuthority,false);assert.equal(D.CRYSTAL_WALL.stakeMultiplierAllowed,false);assert.equal(D.CRYSTAL_WALL.oneEntryPerCrashEpisode,true);assert.ok(strategy.includes("structuralRole:'INDEPENDENT_CRASH_REBOUND_ONLY'"));assert.ok(strategy.includes('crystalWallFireCommand'));assert.equal(strategy.includes('recoveryBaseStakeCents * 2'),false);});

test('CW3 removes the legacy Recovery evaluator so no hidden 2x or downsized fallback can execute', async () => {
  const source=entry({id:'rh1-cash-source',status:'closed',closeReason:'hard_stop_loss',exitPriceCents:20,closedAtMs:Date.now()-60000,pnlCents:0,remainingCount:0});
  const db=fakeDb([source]);
  const settings={...baseSettings(),recoveryHunterEnabled:true,recoveryStakeCents:500,recoveryMinEntryCents:10,recoveryMaxEntryCents:50,crystalWallMinCrashCents:15,crystalWallMinReboundCents:5,crystalWallMinUpwardTicks:2};
  const strategy=new StrategyEngine({db,kalshi:{},market:{},learning:{},getSettings:()=>settings,getLiveReady:()=>false,refreshGameClock:refreshClock,random:()=>0});
  const made=await strategy.evaluateRecovery(new Map([['T',q('T',24,25)]]),{legacyCompatibility:true});
  assert.deepEqual(made,[]);
  assert.equal(db.audits.some((x)=>x.event==='sim_entry_blocked_cash'&&x.data?.concept==='Recovery Hunter'),false);
});
...
test('CW3 Recovery uniqueness is per stopped source trade, not per ticker',async()=>{const C=await import('../src/config.mjs');const D=await import('../src/doctrine.mjs');const {readFile}=await import('node:fs/promises');const strategy=await readFile(new URL('../src/strategy.mjs',import.meta.url),'utf8');const s=C.originalSettings();assert.equal(s.recoveryHunterEnabled,true);assert.equal(s.recoveryStakeCents,500);assert.equal(s.recoveryMinEntryCents,10);assert.equal(s.recoveryMaxEntryCents,50);assert.equal(s.crystalWallMinCrashCents,15);assert.equal(s.crystalWallMinReboundCents,5);assert.equal(s.crystalWallMinUpwardTicks,2);assert.equal(Object.hasOwn(s,'recoveryBaseStakeCents'),false);assert.equal(Object.hasOwn(s,'recoveryMinReboundCents'),false);assert.equal(D.CRYSTAL_WALL.version,'CRYSTAL-WALL-V3');assert.equal(D.CRYSTAL_WALL.athenaAuthority,false);assert.equal(D.CRYSTAL_WALL.stakeMultiplierAllowed,false);assert.equal(D.CRYSTAL_WALL.oneEntryPerCrashEpisode,true);assert.ok(strategy.includes("structuralRole:'INDEPENDENT_CRASH_REBOUND_ONLY'"));assert.ok(strategy.includes('crystalWallFireCommand'));assert.equal(strategy.includes('recoveryBaseStakeCents * 2'),false);});
...
test('CW3 scanner priority contains only active Crystal Wall watches, not historical stopped trades', async()=>{
  const e=Object.create(SagittariusEngine.prototype);e.settings={recoveryHunterEnabled:true};e.strategy={recoveryCandidateTickers:async()=>{throw new Error('obsolete V2 query must not run');}};e.crystalWallWatches=new Map([['a',{ticker:'CW-A',status:'CRASH_ARMED'}],['b',{ticker:'CW-B',status:'EXPIRED',terminal:true}]]);e.recoveryPriorityTickers=new Set(['OLD']);
  const out=await e.refreshRecoveryPriorityTickers();assert.deepEqual([...out],['CW-A']);
});

test('CW3 quote event arms from a CI1 crash episode without requiring a stopped parent trade',()=>{
  const e=Object.create(SagittariusEngine.prototype);e.settings={recoveryHunterEnabled:true,crystalWallMinCrashCents:15,crystalWallMinReboundCents:5,crystalWallMinUpwardTicks:2,recoveryMinEntryCents:10,recoveryMaxEntryCents:50,maxSpreadCents:3};e.crystalWallWatches=new Map();e.recoveryPriorityTickers=new Set();e.crystalWallContinuationStats=null;e.queueCrystalWallWatch=()=>true;
  const ok=e.observeCrystalWallQuote({ticker:'CW',eventTicker:'EV',yesBid:20,yesAsk:21,status:'active'},{episodeId:'CRASH:CW:1',preCrashPeakCents:40,troughCents:20,crashDepthCents:20,troughAtMs:Date.now()});assert.equal(ok,true);assert.equal(e.crystalWallWatches.size,1);assert.equal(e.recoveryPriorityTickers.has('CW'),true);
});
...
test('CW3 CRH1 force-fresh execution revalidation blocks a captured signal that loses structure or spread before fill',async()=>{const C=await import('../src/config.mjs');const D=await import('../src/doctrine.mjs');const {readFile}=await import('node:fs/promises');const strategy=await readFile(new URL('../src/strategy.mjs',import.meta.url),'utf8');const s=C.originalSettings();assert.equal(s.recoveryHunterEnabled,true);assert.equal(s.recoveryStakeCents,500);assert.equal(s.recoveryMinEntryCents,10);assert.equal(s.recoveryMaxEntryCents,50);assert.equal(s.crystalWallMinCrashCents,15);assert.equal(s.crystalWallMinReboundCents,5);assert.equal(s.crystalWallMinUpwardTicks,2);assert.equal(Object.hasOwn(s,'recoveryBaseStakeCents'),false);assert.equal(Object.hasOwn(s,'recoveryMinReboundCents'),false);assert.equal(D.CRYSTAL_WALL.version,'CRYSTAL-WALL-V3');assert.equal(D.CRYSTAL_WALL.athenaAuthority,false);assert.equal(D.CRYSTAL_WALL.stakeMultiplierAllowed,false);assert.equal(D.CRYSTAL_WALL.oneEntryPerCrashEpisode,true);assert.ok(strategy.includes("structuralRole:'INDEPENDENT_CRASH_REBOUND_ONLY'"));assert.ok(strategy.includes('crystalWallFireCommand'));assert.equal(strategy.includes('recoveryBaseStakeCents * 2'),false);});

test('CW3 CRH1 consumes an exact Dragon Cosmo while independently freezing the current crash episode and normal execution controls',async()=>{const C=await import('../src/config.mjs');const D=await import('../src/doctrine.mjs');const {readFile}=await import('node:fs/promises');const strategy=await readFile(new URL('../src/strategy.mjs',import.meta.url),'utf8');const s=C.originalSettings();assert.equal(s.recoveryHunterEnabled,true);assert.equal(s.recoveryStakeCents,500);assert.equal(s.recoveryMinEntryCents,10);assert.equal(s.recoveryMaxEntryCents,50);assert.equal(s.crystalWallMinCrashCents,15);assert.equal(s.crystalWallMinReboundCents,5);assert.equal(s.crystalWallMinUpwardTicks,2);assert.equal(Object.hasOwn(s,'recoveryBaseStakeCents'),false);assert.equal(Object.hasOwn(s,'recoveryMinReboundCents'),false);assert.equal(D.CRYSTAL_WALL.version,'CRYSTAL-WALL-V3');assert.equal(D.CRYSTAL_WALL.athenaAuthority,false);assert.equal(D.CRYSTAL_WALL.stakeMultiplierAllowed,false);assert.equal(D.CRYSTAL_WALL.oneEntryPerCrashEpisode,true);assert.ok(strategy.includes("structuralRole:'INDEPENDENT_CRASH_REBOUND_ONLY'"));assert.ok(strategy.includes('crystalWallFireCommand'));assert.equal(strategy.includes('recoveryBaseStakeCents * 2'),false);});
...
test('HF7 A-to-Z SIM trade traverses EAC1 admission probe -> feeder -> GCA2 -> book -> Athena -> Galactic commit -> fill -> persistence -> ProfitGuard settlement', async () => {
  ...
  assert.equal(admission.shouldQueueInitialExposure('R35-E2E',{allowProbe:true}),true,'mature shared observed-activity lower-bound must earn one bounded authority probe');
  ...
  assert.equal(hunter.entryConfig.gameClockAuthority.source,'observed_market_activity');
  assert.equal(hunter.entryConfig.gameClockAuthority.sourceStrength,'bounded_observed_activity');
