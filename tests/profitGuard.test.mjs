...
test('MW HF6 profitable Athena PRI1-R2 close releases the strategic handoff only after the full durable close',async()=>{
  const handed=[];
  const mw={version:MEGA_WAVE.version,policyRevision:MEGA_WAVE.policyRevision,followUpAttacksAtEntry:6,enabledSaintsAtEntry:['Scarlet Needle']};
  const h=guardHarness({bid:75,onPositionClosed:async(entry)=>{handed.push(entry);},entryOverrides:{
    conceptName:'Athena Exclamation',entryPriceCents:70,currentPriceCents:70,peakPriceCents:76,count:10,remainingCount:10,pnlCents:0,entryFeeCents:20,
    entryConfig:{release:'TEST',profitAuthority:PROTECTED_RUNNER_INTELLIGENCE.version,profitAuthorityRevision:PROTECTED_RUNNER_INTELLIGENCE.policyRevision,pri1R2:{version:PROTECTED_RUNNER_INTELLIGENCE.version,policyRevision:PROTECTED_RUNNER_INTELLIGENCE.policyRevision,enabledAtEntry:true,triggerNetPerOriginalContractCents:2},megaWave:mw,athenaExclamation:{megaWaveAuthorization:mw}},
  }});
  const decision={action:'protected_runner_intelligence',reason:'protected_runner_intelligence',protectedRunnerIntelligence:PROTECTED_RUNNER_INTELLIGENCE.version,peakPriceCents:76,stopPriceCents:8};
  const first=await h.guard.applyExitFill(h.row(),decision,{fillCount:5,fillPriceCents:75,exitFeeCents:0});
  assert.equal(first.closed,false);
  await new Promise(resolve=>setImmediate(resolve));
  assert.equal(handed.length,0,'partial profitable fill cannot release downstream Saints');
  const second=await h.guard.applyExitFill(h.row(),decision,{fillCount:5,fillPriceCents:75,exitFeeCents:0});
  assert.equal(second.closed,true);
  await new Promise(resolve=>setImmediate(resolve));
  assert.equal(handed.length,1);
  assert.equal(handed[0].conceptName,'Athena Exclamation');
  assert.equal(handed[0].closeReason,'protected_runner_intelligence');
  assert.equal(handed[0].remainingCount,0);
  assert.ok(handed[0].pnlCents>0);
});

test('MW HF6 profitable Athena settlement releases Saints while settlement scratch/loss cannot',async()=>{
  const mw={version:MEGA_WAVE.version,policyRevision:MEGA_WAVE.policyRevision,followUpAttacksAtEntry:6,enabledSaintsAtEntry:['Scarlet Needle']};
  const make=()=>({conceptName:'Athena Exclamation',entryPriceCents:70,currentPriceCents:70,peakPriceCents:70,count:10,remainingCount:10,pnlCents:0,entryFeeCents:20,entryConfig:{release:'TEST',profitAuthority:PROTECTED_RUNNER_INTELLIGENCE.version,pri1R2:{version:PROTECTED_RUNNER_INTELLIGENCE.version,policyRevision:PROTECTED_RUNNER_INTELLIGENCE.policyRevision,enabledAtEntry:true,triggerNetPerOriginalContractCents:2},megaWave:mw,athenaExclamation:{megaWaveAuthorization:mw}}});
  const wins=[];const win=guardHarness({onPositionClosed:async(entry)=>wins.push(entry),entryOverrides:make()});
  const settled=await win.guard.closeAtSettlement(win.row(),{reason:'settlement_win',exitPriceCents:100});
  assert.equal(settled.closed,true);await new Promise(resolve=>setImmediate(resolve));assert.equal(wins.length,1);assert.ok(wins[0].pnlCents>0);
  const scratchHanded=[];const scratch=guardHarness({onPositionClosed:async(entry)=>scratchHanded.push(entry),entryOverrides:make()});
  await scratch.guard.closeAtSettlement(scratch.row(),{reason:'settlement_win',exitPriceCents:72});
  await new Promise(resolve=>setImmediate(resolve));assert.equal(scratch.row().pnlCents,0);assert.equal(scratchHanded.length,0);
  const lossHanded=[];const loss=guardHarness({onPositionClosed:async(entry)=>lossHanded.push(entry),entryOverrides:make()});
  await loss.guard.closeAtSettlement(loss.row(),{reason:'settlement_loss',exitPriceCents:0});
  await new Promise(resolve=>setImmediate(resolve));assert.ok(loss.row().pnlCents<0);assert.equal(lossHanded.length,0);
});
...
test('R36 Gate 2 ATHENA-X1 pure state machine holds a monotonic winner and ordinary peak noise with the full position intact',()=>{
  ...
  const path=[90,89,89,91,92,95,98,97,98,99,98,98,96];
  ...
  assert.equal(state.pullbackCents,3);
  assert.equal(state.phase,'X1_RECOVERY_WATCH');
});

test('R36 Gate 2 ATHENA-X1 does not panic-sell a single deep shock that immediately begins reclaiming',()=>{
  let state=null,t=1_000;
  for(const bid of [79,81,86,70,72,77,85,96,99]){
    const r=advanceAthenaExitState(state,{
    ...
    state=r.state;t+=5_000;
    assert.equal(r.decision,'HOLD',`recoverable shock path was exited at ${bid}c`);
  }
  assert.equal(state.peakExecutableBidCents,99);
});
...
test('R36 Gate 2 ATHENA-X1 commits only after confirmed structural deterioration, not the first pullback',()=>{
  ...
  for(const bid of [85,90,88,86,84,82]){
    ...
    if(bid===88||bid===86)assert.equal(r.decision,'HOLD',`X1 exited before deterioration was confirmed at ${bid}c`);
    if(exitAt!=null)break;
  }
  assert.equal(exitAt,84);
  assert.equal(state.phase,'X1_EXIT_COMMITTED');
  assert.ok(state.failureScore>=ATHENA_EXIT_INTELLIGENCE.capitalDefenseFailureScore);
  assert.ok(state.recoveryScore<=50);
});
...
test('R36 Gate 3 ATHENA-X1 integrated ProfitGuard holds normal 2-3c peak pullbacks instead of reproducing PRI1-R2 sensitivity',async()=>{
  ...
  h.setBid(89);out=await h.guard.protect(h.row());assert.equal(out.closed??false,false);
  h.setBid(88);out=await h.guard.protect(h.row());assert.equal(out.closed??false,false);
  h.setBid(87);out=await h.guard.protect(h.row());assert.equal(out.closed??false,false,'3c pullback must remain a runner');
  assert.equal(h.row().remainingCount,100);
...
test('R36 Gate 3 ATHENA-X1 integrated ProfitGuard exits exactly 100% after confirmed deterioration and keeps PLI1 research active',async()=>{
  ...
  let out=await h.guard.protect(h.row());assert.equal(out.closed??false,false);
  for(const bid of [90,88,86]){h.setBid(bid);out=await h.guard.protect(h.row());assert.equal(out.closed??false,false);}
  h.setBid(84);out=await h.guard.protect(h.row());
  assert.equal(out.closed,true);
...
test('R36 Gate 2 ATHENA-X1 high-price peak exhaustion protects Kaleido-like mature profit compression without reintroducing a generic tight trail',()=>{
  ...
  assert.equal(r.decision,'EXIT');
  assert.equal(r.state.decisionReason,'capital_defense');
  assert.equal(r.state.peakExhaustion,true);
...
test('R36 Gate 2 ATHENA-X1 may consume frozen ATHENA-B1 crash survival read-only to avoid panic-selling a historically recoverable 15c drawdown',async()=>{
  ...
  let out=await h.guard.protect(h.row());assert.equal(out.closed??false,false);
  for(const bid of [76,79,81,86,70,70,68]){h.setBid(bid);out=await h.guard.protect(h.row());assert.equal(out.closed??false,false,`supportive B1 drawdown history was ignored at ${bid}c`);}
  const st=h.row().profitGuardState;
  assert.equal(st.historicalDrawdown.available,true);
  assert.equal(st.historicalDrawdown.supportive,true);
  assert.equal(st.historicalDrawdown.thresholdCents,15);
  assert.ok(st.historicalDrawdown.totalObservations>=20);
...
test('R36 Gate 3 ATHENA-X1 integrated peak exhaustion exits 100% of a Kaleido-like position while Johannus-like retained profit stays open',async()=>{
  ...
  assert.equal(out.closed,true);assert.equal(out.filled,121);assert.equal(k.row().closeReason,'athena_x1_exit');assert.equal(k.row().profitGuardState.exitTrigger,'capital_defense');
  ...
  for(let path=0;path<250;path++){
    ...
    for(let step=0;step<80;step++){
      ...
      if(r.decision==='EXIT'){
        ...
        assert.equal(state.phase,'X1_EXIT_COMMITTED');
        assert.ok(['structural_deterioration','capital_defense','severe_crash_deterioration'].includes(state.exitTrigger));
        break;
      }
    }
  }
  assert.equal(evaluated>10_000,true);
  assert.equal(exits>0,true,'stress corpus never exercised the exit branch');
});
...
test('R36 Gate 5 missing or corrupt ATHENA-B1 read-only drawdown context is neutral to X1 and cannot break protection',async()=>{
  for(const athena of [null,{brain:{version:'corrupt',brainHash:'bad'}}]){
    ...
    let out=await h.guard.protect(h.row());assert.equal(out.closed??false,false);
    h.setBid(87);out=await h.guard.protect(h.row());assert.equal(out.closed??false,false);
...
test('R36 HARDENING ATHENA-X1 flat observations at the peak do not pre-load deterioration confirmation',()=>{
  ...
  r=advanceAthenaExitState(r.state,{observation:{observedAtMs:4_000,executableBidCents:86,executableNetCents:200,askCents:87},context});
...
test('R36 HARDENING ATHENA-X1 failure trajectory is isolated to the current post-peak cycle',()=>{
  ...
  for(const [t,bid,net] of [[2_000,88,1400],[3_000,86,1200],[4_000,94,2000]]){
  ...
  r=advanceAthenaExitState(r.state,{observation:{observedAtMs:5_000,executableBidCents:88,executableNetCents:1400,askCents:89},context});
...
test('R37 Golden Eye cashout queues behind in-flight per-entry protection instead of being silently coalesced into the protection promise',async()=>{
  ...
  const h=guardHarness({bid:90,entryOverrides:{entryConfig:geConfig}});
  let release;
  let startedResolve;
...
test('R37 Golden Eye SIM cashout never intentionally splits a position when only partial full-position depth is available',async()=>{
  const geConfig={profitAuthority:GOLDEN_EYE.version,profitAuthorityRevision:GOLDEN_EYE.policyRevision,profitLearning:'PLI1'};
  const h=guardHarness({bid:90,full:false,filled:40,entryOverrides:{entryConfig:geConfig}});
...
test('R37 Golden Eye LIVE persists an exit receipt before broker submit and requests the full owned count at the break-even-or-better limit',async()=>{
  const geConfig={profitAuthority:GOLDEN_EYE.version,profitAuthorityRevision:GOLDEN_EYE.policyRevision,profitLearning:'PLI1'};
  const h=guardHarness({bid:90,full:true,filled:100,entryOverrides:{mode:'LIVE',entryConfig:geConfig}});
  const before=h.row();
...
test('Atomic Thunder persistent stats do not treat null counterfactual fields as zero losses',async()=>{
  ...
  db.pool={async query(){
    ...
    if(call===2)return{rows:[{harvest_count:1,realized_pnl_cents:250,total_time_ms:5000,avoided_loss_cents:0,losses_avoided:0,forgone_upside_cents:0,research_complete:0}]};
    return{rows:[]};
  }};
...
// IB1-R2 industrial hotfix regression suite. These tests lock the economic
// covenant from full-book detection through the final SELL mutation boundary.
function ibR2Entry({mode='SIMULATION',ticker='IB1-R2',count=10,entryPriceCents=85,entryFeeCents=null,targetPerContract=1}={}){
  return {
    id:`ib-r2-${mode.toLowerCase()}`,systemName:'SAGITTARIUS',ownerId:'sagittarius-main',conceptName:'Momentum Hunter',
    ticker,eventTicker:ticker,mode,status:'open',entryPriceCents,currentPriceCents:entryPriceCents,peakPriceCents:entryPriceCents,
    stopPriceCents:1,stopLossCents:84,count,remainingCount:count,pnlCents:0,
    entryFeeCents:entryFeeCents==null?(mode==='SIMULATION'?2*count:0):entryFeeCents,
    exitFeeCents:0,exitFilledCount:0,exitNotionalCents:0,profitGuardState:{},apexProfitGuardState:{},stopGuardState:{},
    openedAtMs:Date.now()-60_000,updatedAtMs:Date.now()-1000,
    entryConfig:{infinityBreak:{version:INFINITY_BREAK.version,policyRevision:INFINITY_BREAK.policyRevision,enabledAtEntry:true,
      minimumNetPerOriginalContractCents:targetPerContract,requiredFreshConfirmations:1,maximumBookAgeMs:1000,confirmationWindowMs:3000,fullPositionOnly:true,lossAuthority:'AURORA_EXECUTION'},
      brokerRouting:{version:'LIVE-SHARD-R1',exchangeIndex:3}},
  };
}

function ibR2Db(entry){
  const rows=new Map([[entry.id,structuredClone(entry)]]),audits=[];
  return {
    rows,audits,
    async updateEntry(id,patch){const row=rows.get(id);if(row)Object.assign(row,structuredClone(patch));},
    async entryById(id){return rows.has(id)?structuredClone(rows.get(id)):null;},
    async openEntriesByTicker(_systemName,ticker){return [...rows.values()].filter(x=>x.ticker===ticker).map(x=>structuredClone(x));},
    async audit(level,event,data){audits.push({level,event,data:structuredClone(data)});},
  };
}

async function ibR2Market({ticker='IB1-R2',levels=[{priceCents:92,count:5},{priceCents:88,count:5}],onEnsure=null}={}){
  const {MarketHub}=await import('../src/market.mjs');
  const hub=new MarketHub({kalshi:{},wsUrl:'',fallbackWsUrl:'',getCredentials:()=>({})});
  const apply=(next)=>{
    const now=Date.now();
    hub.books.set(ticker,{ticker,source:'REST',sequenceValid:true,updatedAtMs:now,yesBids:next.map(x=>({...x})),noBids:[{priceCents:7,count:1000}]});
    hub.quotes.set(ticker,{ticker,eventTicker:ticker,yesBid:next[0]?.priceCents||0,yesAsk:94,updatedAtMs:now,bookInvalid:false,status:'active',result:'',exchangeIndex:3});
  };
  apply(levels);
  let ensures=0;
  hub.ensureFreshBook=async()=>{ensures++;if(onEnsure)await onEnsure({hub,apply,ensures});return hub.getBook(ticker);};
  return {hub,apply,get ensures(){return ensures;}};
}

test('IB1-R2 MarketHub full-position walk preserves exact consumed levels and lowest executable IOC price',async()=>{
  const {hub}=await ibR2Market({levels:[{priceCents:92,count:5},{priceCents:88,count:5},{priceCents:80,count:50}]});
  const x=hub.executableBid('IB1-R2',10,1);
  assert.equal(x.full,true);assert.equal(x.filled,10);assert.equal(x.avgCents,90);assert.equal(x.lowestConsumedCents,88);assert.equal(x.highestConsumedCents,92);
  assert.deepEqual(x.consumedLevels,[{priceCents:92,count:5},{priceCents:88,count:5}]);
});

test('IB1-R2 regression: aggregate SIM net may qualify even when only part of the position is above the old flat floor',async()=>{
  const entry=ibR2Entry();const db=ibR2Db(entry);const {hub}=await ibR2Market({levels:[{priceCents:92,count:5},{priceCents:88,count:5}]});
  const s={simFeeCents:2,systemName:'SAGITTARIUS',ownerId:'sagittarius-main',mode:'SIMULATION'};
  const guard=new ProfitGuard({db,kalshi:{},market:hub,learning:{},getSettings:()=>s});
  const floor=guard.atomicThunderExitFloorCents(entry,s);assert.equal(floor,90);
  const oldFlatDepth=hub.executableBid(entry.ticker,entry.count,floor);assert.equal(oldFlatDepth.full,false);assert.equal(oldFlatDepth.filled,5,'old implementation would refuse the exit');
  const assessment=await guard.assessInfinityBreakExecution(entry,hub.getQuote(entry.ticker),s,{ensureFresh:true});
  assert.equal(assessment.qualifies,true);assert.equal(assessment.executableBidCents,90);assert.equal(assessment.lowestConsumedCents,88);assert.equal(assessment.executableNetCents,10);assert.equal(assessment.targetNetCents,10);
  const out=await guard.simulationExit(entry,{action:'infinity_break',reason:'infinity_break',infinityBreak:INFINITY_BREAK.version,peakPriceCents:92,stopPriceCents:1},hub.getQuote(entry.ticker));
  assert.equal(out.closed,true);const closed=await db.entryById(entry.id);assert.equal(closed.remainingCount,0);assert.equal(closed.closeReason,'infinity_break');
  assert.equal(closed.profitGuardState.phase,'INFINITY_BREAK_EXIT_FILLED');assert.equal(closed.profitGuardState.lowestConsumedCents,88);assert.equal(closed.profitGuardState.targetNetCents,10);assert.equal(closed.profitGuardState.executableNetCents,10);
});

test('IB1-R2 LIVE uses the Kalshi taker estimator on actual consumed levels and submits IOC at the lowest qualifying consumed bid',async()=>{
  const {kalshiGeneralTakerFeeEstimateCents}=await import('../src/doctrine.mjs');
  const count=10;const entryFee=kalshiGeneralTakerFeeEstimateCents({count,priceCents:85});
  const entry=ibR2Entry({mode:'LIVE',entryFeeCents:entryFee});const db=ibR2Db(entry);const {hub}=await ibR2Market({levels:[{priceCents:92,count:5},{priceCents:84,count:5}]});
  const s={simFeeCents:2,systemName:'SAGITTARIUS',ownerId:'sagittarius-main',mode:'LIVE'};let order=null;
  const kalshi={async getPositions(){return[{ticker:entry.ticker,position_fp:count}];},buildClientOrderId:()=> 'ib-r2-live-exit',async placeOrder(o){order=structuredClone(o);return{orderId:'ib-r2-sell',fillCount:o.count,averageFillPriceCents:88,feePaidCents:kalshiGeneralTakerFeeEstimateCents({count:5,priceCents:92})+kalshiGeneralTakerFeeEstimateCents({count:5,priceCents:84})};}};
  const guard=new ProfitGuard({db,kalshi,market:hub,learning:{},getSettings:()=>s});
  const a=await guard.assessInfinityBreakExecution(entry,hub.getQuote(entry.ticker),s,{ensureFresh:true});
  const expectedExitFee=kalshiGeneralTakerFeeEstimateCents({count:5,priceCents:92})+kalshiGeneralTakerFeeEstimateCents({count:5,priceCents:84});
  assert.equal(a.qualifies,true);assert.equal(a.expectedExitFeeCents,expectedExitFee);assert.equal(a.lowestConsumedCents,84);assert.ok(a.executableNetCents>=a.targetNetCents);
  const theoreticalFlatFloor=guard.atomicThunderExitFloorCents(entry,s);assert.ok(theoreticalFlatFloor>84);assert.equal(hub.executableBid(entry.ticker,count,theoreticalFlatFloor).full,false,'old flat-floor rule would reject part of this profitable book');
  const out=await guard.liveExit(entry,{action:'infinity_break',reason:'infinity_break',infinityBreak:INFINITY_BREAK.version,peakPriceCents:92,stopPriceCents:1});
  assert.equal(out.closed,true);assert.ok(order);assert.equal(order.action,'sell');assert.equal(order.count,count);assert.equal(order.priceCents,84);assert.equal(order.exchangeIndex,3);
  const closed=await db.entryById(entry.id);assert.equal(closed.profitGuardState.phase,'INFINITY_BREAK_EXIT_FILLED');assert.equal(closed.profitGuardState.lowestConsumedCents,84);assert.ok(closed.profitGuardState.executableNetCents>=closed.profitGuardState.targetNetCents);assert.ok(closed.profitGuardState.realizedNetCents>0);
  const filled=db.audits.find(x=>x.event==='infinity_break_filled');assert.ok(filled);assert.equal(filled.data.lowestConsumedCents,84);assert.ok(filled.data.targetNetCents>0);assert.ok(filled.data.executableNetCents>0);
});

test('IB1-R2 final LIVE revalidation fails closed after durable intent if the aggregate profit window disappears before broker submission',async()=>{
  const {kalshiGeneralTakerFeeEstimateCents}=await import('../src/doctrine.mjs');
  const count=10;const entry=ibR2Entry({mode:'LIVE',entryFeeCents:kalshiGeneralTakerFeeEstimateCents({count,priceCents:85})});const db=ibR2Db(entry);
  const {hub}=await ibR2Market({levels:[{priceCents:92,count:5},{priceCents:84,count:5}],onEnsure:async({apply,ensures})=>{if(ensures===2)apply([{priceCents:86,count:10}]);}});
  const s={simFeeCents:2,systemName:'SAGITTARIUS',ownerId:'sagittarius-main',mode:'LIVE'};let submissions=0;
  const kalshi={async getPositions(){return[{ticker:entry.ticker,position_fp:count}];},buildClientOrderId:()=> 'ib-r2-window-loss',async placeOrder(){submissions++;throw new Error('must not submit after final revalidation fails');}};
  const guard=new ProfitGuard({db,kalshi,market:hub,learning:{},getSettings:()=>s});
  const out=await guard.liveExit(entry,{action:'infinity_break',reason:'infinity_break',infinityBreak:INFINITY_BREAK.version,peakPriceCents:92,stopPriceCents:1});
  assert.equal(out.closed,false);assert.equal(out.skipped,'infinity_break_revalidation_failed');assert.equal(submissions,0);
  const reopened=await db.entryById(entry.id);assert.equal(reopened.status,'open');assert.equal(reopened.exitClientOrderId,null);assert.equal(reopened.closeReason,null);assert.equal(reopened.profitGuardState.phase,'INFINITY_BREAK_WINDOW_LOST_BEFORE_SUBMIT');
  const lost=db.audits.find(x=>x.event==='infinity_break_exit_window_lost'&&x.data.stage==='immediately_before_submit');assert.ok(lost);
});

test('IB1-R2 near-target quote bypasses the 125ms debounce but remains inside the bounded/coalesced protection queue',async()=>{
  const {SagittariusEngine,CoalescingWorkQueue}=await import('../src/engine.mjs');
  const engine=Object.create(SagittariusEngine.prototype);let calls=0;
  engine.running=true;engine.protectedTickers=new Set(['FAST']);engine.quoteProtectionTimers=new Map();engine.quoteProtectionQueue=new CoalescingWorkQueue({maxConcurrency:3});
  engine.profitGuard={shouldFastWakeInfinity:()=>true,async protectTicker(){calls++;}};
  engine.market={quotes:new Map(),getQuote:()=>({yesBid:90,yesAsk:91,updatedAtMs:Date.now()})};
  engine.learning={async trackStopGuardRecovery(){}};engine.settings={};engine.db={async audit(){}};engine.markProtection=()=>{};engine.recomputeHealth=()=>{};
  engine.queueQuoteProtection('FAST');
  await new Promise(resolve=>setTimeout(resolve,20));
  assert.equal(calls,1,'near-target Infinity protection must not wait for the normal 125ms timer');assert.equal(engine.quoteProtectionTimers.has('FAST'),false);
  assert.ok(engine.quoteProtectionQueue.snapshot().maxObservedActive<=3,'fast wake must remain bounded by the existing protection queue');
});

test('IB1-R2 restart reconciliation preserves committed Infinity economics through an authoritative broker fill',async()=>{
  const {kalshiGeneralTakerFeeEstimateCents}=await import('../src/doctrine.mjs');
  const count=10;const entry=ibR2Entry({mode:'LIVE',entryFeeCents:kalshiGeneralTakerFeeEstimateCents({count,priceCents:85})});
  entry.status='exit_pending';entry.closeReason='infinity_break';entry.exitClientOrderId='ib-r2-restart-client';entry.updatedAtMs=Date.now()-1000;
  entry.profitGuardState={version:INFINITY_BREAK.version,policyRevision:INFINITY_BREAK.policyRevision,phase:'INFINITY_BREAK_EXIT_REVALIDATED',targetNetCents:10,executableNetCents:29,executableBidCents:89.5,lowestConsumedCents:87,expectedExitFeeCents:7,entryFeeCents:entry.entryFeeCents,bookMs:Date.now()-500,revalidatedAtMs:Date.now()-500,updatedAtMs:Date.now()-500};
  const db=ibR2Db(entry);const s={simFeeCents:2,systemName:'SAGITTARIUS',ownerId:'sagittarius-main',mode:'LIVE'};
  const kalshi={async inspectOrderByClientId(){return{found:true,authoritative:true,order:{status:'filled',orderId:'ib-r2-restart-fill',fillCount:count,averageFillPriceCents:89.5,feePaidCents:7}};}};
